...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  </div><div><RP miss={isDefault?MI.compre:null} isExample={isDefault} title="종합부동산세"
...
return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>증여세 계산기</h3>}
    ...
    <div>{g<=0&&<StepsGuide steps={[{label:"증여 재산가액 입력",filled:g>0},{label:"수증자 관계 선택",filled:true}]}/>}<RP miss={g>0?null:MI.gift} title="증여세"
...
function CalcInherit({isMo=false,onNav=()=>{}}){const[ta,sTa]=useState("");const[finAsset,sFA]=useState("");const[db,sDb]=useState("");const[fn,sFn]=useState("");const[hasSpouse,sHS]=useState("yes");const[children,sCh]=useState("2");const[cohabit,sCohab]=useState("");const[farm,sFarm]=useState("");const[priorGift,sPG]=useState("");const[onTime,sOT]=useState("yes");const t=tW(ta),finW=tW(finAsset),d=tW(db),f=Math.min(tW(fn),15000000),priorW=tW(priorGift),cohabW=tW(cohabit),farmW=tW(farm);const gross=t+priorW;const netEstate=gross-d-f;const childN=parseInt(children)||0;const basicDed=2e8;const personalDed=childN*5e7;const basicPlusPersonal=basicDed+personalDed;const lumpSum=5e8;const mainDed=Math.max(basicPlusPersonal,lumpSum);let spouseDed=0;if(hasSpouse==="yes"){const spouseShare=netEstate>0?netEstate*1.5/(1.5+childN):0;spouseDed=Math.min(Math.max(spouseShare,5e8),30e8);}let financialDed=0;if(finW>0){if(finW<=2e7)financialDed=finW;else if(finW<=1e8)financialDed=2e7;else financialDed=Math.min(Math.round(finW*0.20),2e8);}const cohabDed=cohabW>0?Math.min(cohabW,6e8):0;const farmDed=farmW>0?Math.min(farmW,15e8):0;const totalDed=mainDed+spouseDed+financialDed+cohabDed+farmDed;const tb=Math.max(0,netEstate-totalDed);const tx=pTx(tb,GB);const rebate=onTime==="yes"?0.03:0;const finalTax=Math.round(tx*(1-rebate));return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>상속세 계산기</h3>}
    ...
    <div>{t<=0&&<StepsGuide steps={[{label:"총 상속재산가액 입력",filled:t>0},{label:"가족 구성 확인",filled:true}]}/>}<RP miss={t>0?null:MI.inherit} title="상속세"
...
function CalcNetSale({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <RP miss={sellW>0?null:MI.netsale} title="매도 실수령액" total={Math.max(0,net)} sub={"매도가 "+fW(sellW)+" 기준"}
...
function CalcReg({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);const _regErr=!p||p==="0";
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>등기비용 통합 계산</h3>}<Tog label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Tog label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000" error={_regErr}/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div><div><RP miss={pW>0?null:MI.registration} title="등기비용 통합"
...
function CalcBond({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>국민주택채권 할인비용</h3>}<Inp label="매매 가격" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="지역" value={area} onChange={sArea} options={[{value:"seoul",label:"서울"},{value:"metro",label:"광역시"},{value:"etc",label:"기타 지역"}]}/><Tog label="용도" value={use} onChange={sUse} options={[{value:"house",label:"주택"},{value:"commercial",label:"상가·오피스텔"}]}/><Inp label="할인율 (실시세)" value={customRate} onChange={setCustomRate} suffix="%" note="증권사 당일 매도가. 일반 4~5%"/></div><RP miss={tW(p)>0?null:MI.bond} title="채권 할인비용"
...
function CalcYearEnd({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  </div><div>{salW<=0&&<RequiredGuide items={[{label:"연봉 (세전)",filled:salW>0}]}/>}<RP miss={salW>0?null:MI.yearend} title={refund>=0?"예상 환급액":"추가 납부 예상"}
...
function CalcRetire({isMo=false,onNav=()=>{}}){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
  ...
  </div><RP miss={(monthlyW>0&&y>0)?null:MI.retire} title="예상 퇴직금"
...
function CalcUnemploy({isMo=false,onNav=()=>{}}){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
  ...
  </div>{dailyW<=0&&<RequiredGuide items={[{label:"퇴직 전 평균임금",filled:dailyW>0}]}/>}<RP miss={dailyW>0?null:MI.unemploy} title="실업급여 예상"
...
function CalcCarTax({isMo=false,onNav=()=>{}}){const[carType,sCT]=useState("normal");const[cc,sCC]=useState("");const[year,sYear]=useState("0");const[isElec,sElec]=useState("no");
  ...
  </div><RP miss={(isElec==="yes"||ccVal>0)?null:MI.cartax} title="자동차세 (연간)"
...
function CalcMinWage({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}>
    ...
    <div><RP miss={parseFloat(weeklyH)>0?null:MI.minwage} title="예상 급여"
