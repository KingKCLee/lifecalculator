...
function CalcAcq({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?12:32,alignItems:"start"}}>
    <div>
      {!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>2026년 최신 세율 · 주택수·지역별 자동 계산</h3>}
...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  return(<div><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>보유기간·거주기간·장특공제 자동 반영</h3>}
...
const CX=[[3e8,.005],[6e8,.007],[12e8,.010],[25e8,.020],[50e8,.030],[94e8,.040],[Infinity,.050]];
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>공정시장가액비율 · 고령자·장기보유공제 적용</h3>}
...
let _setPanelContent=null;let _setPanelVisible=null;let _panelHideTimer=null;
function SidePanel(){
  const[visible,setVisible]=useState(false);const[content,setContent]=useState({title:"",body:null});const isMo=useIsMobile();
  useEffect(()=>{_setPanelContent=c=>setContent(c);_setPanelVisible=v=>setVisible(v);return()=>{_setPanelContent=null;_setPanelVisible=null;};},[]);
  if(isMo){if(!visible)return null;return createPortal(<div onClick={()=>setVisible(false)} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:10000,display:"flex",alignItems:"flex-end"}}><div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"16px 16px 0 0",padding:"20px 20px 40px",width:"100%",maxHeight:"70vh",overflowY:"auto",wordBreak:"keep-all"}}><div style={{width:36,height:4,background:"#dfe1e6",borderRadius:2,margin:"0 auto 16px"}}/><button onClick={()=>setVisible(false)} style={{position:"absolute",top:16,right:16,background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#666"}}>✕</button><h4 style={{fontWeight:700,marginBottom:12,fontSize:15,color:"#172B4D"}}>{content.title}</h4><div style={{fontSize:14,lineHeight:1.8,color:"#344563"}}>{content.body}</div></div></div>,document.body);}
  return createPortal(<div onMouseEnter={()=>{if(_panelHideTimer){clearTimeout(_panelHideTimer);_panelHideTimer=null;}}} onMouseLeave={()=>{_panelHideTimer=setTimeout(()=>{if(_setPanelVisible)_setPanelVisible(false);},200);}} style={{position:"fixed",top:"50%",right:visible?0:-360,transform:"translateY(-50%)",width:320,maxHeight:"60vh",overflowY:"auto",background:"#fff",borderRadius:"12px 0 0 12px",boxShadow:"-4px 0 20px rgba(0,0,0,0.12)",padding:20,zIndex:9999,transition:"right 0.25s ease",wordBreak:"keep-all",pointerEvents:visible?"auto":"none"}}><div style={{fontSize:11,fontWeight:700,color:"#0747A6",letterSpacing:.5,textTransform:"uppercase",marginBottom:8}}>{content.title}</div><div style={{fontSize:13,lineHeight:1.8,color:"#344563"}}>{content.body}</div></div>,document.body);
}
function TipModal({title,children}){
  const isMo=useIsMobile();const block=e=>{e.stopPropagation();e.preventDefault();};
  const showPanel=e=>{block(e);if(_setPanelContent)_setPanelContent({title,body:children});if(_setPanelVisible)_setPanelVisible(true);if(_panelHideTimer){clearTimeout(_panelHideTimer);_panelHideTimer=null;}};
  const hidePanel=()=>{if(!isMo){_panelHideTimer=setTimeout(()=>{if(_setPanelVisible)_setPanelVisible(false);},200);}};
  return(<span onClick={block} style={{display:"inline-flex",alignItems:"center",verticalAlign:"middle",marginLeft:4,flexShrink:0}}><span onClick={isMo?showPanel:block} onMouseEnter={isMo?undefined:showPanel} onMouseLeave={isMo?undefined:hidePanel} style={{cursor:"pointer",border:"1.5px solid #0747A6",borderRadius:"50%",width:16,height:16,minWidth:16,minHeight:16,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#0747A6",userSelect:"none",flexShrink:0,boxSizing:"border-box",lineHeight:1,fontWeight:700,transition:"all .15s"}}>?</span></span>);
}
...
function CalcMinWage({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div>
      {!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>2026년 시급 10,030원 · 주휴수당 포함 계산</h3>}
