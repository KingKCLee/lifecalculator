...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div>
      ...
      </div>:<div><RPFull miss={MI.transfer} expertGuide={CALC_NAV_CONTENT.transfer?.items} expertGuide={CALC_NAV_CONTENT.transfer?.items} title="양도소득세" total={0} isExample={true} items={[]}/></div>}
...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  </div><div><RPFull miss={isDefault?MI.compre:null} expertGuide={CALC_NAV_CONTENT.compre?.items} expertGuide={CALC_NAV_CONTENT.compre?.items} isExample={isDefault} title="종합부동산세"
...
return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div>{g<=0&&<StepsGuide steps={[{label:"증여 재산가액 입력",filled:g>0},{label:"수증자 관계 선택",filled:true}]}/>}<RPFull miss={g>0?null:MI.gift} expertGuide={CALC_NAV_CONTENT.gift?.items} expertGuide={CALC_NAV_CONTENT.gift?.items} title="증여세"
...
function CalcInherit({isMo=false,onNav=()=>{}}){const[ta,sTa]=useState("");const[finAsset,sFA]=useState("");const[db,sDb]=useState("");const[fn,sFn]=useState("");const[hasSpouse,sHS]=useState("yes");const[children,sCh]=useState("2");const[cohabit,sCohab]=useState("");const[farm,sFarm]=useState("");const[priorGift,sPG]=useState("");const[onTime,sOT]=useState("yes");const[showInheritLookup,setShowInheritLookup]=useState(false);const[autoInheritFill,setAutoInheritFill]=useState(false);const t=tW(ta),finW=tW(finAsset),d=tW(db),f=Math.min(tW(fn),15000000),priorW=tW(priorGift),cohabW=tW(cohabit),farmW=tW(farm);const gross=t+priorW;const netEstate=gross-d-f;const childN=parseInt(children)||0;const basicDed=2e8;const personalDed=childN*5e7;const basicPlusPersonal=basicDed+personalDed;const lumpSum=5e8;const mainDed=Math.max(basicPlusPersonal,lumpSum);let spouseDed=0;if(hasSpouse==="yes"){const spouseShare=netEstate>0?netEstate*1.5/(1.5+childN):0;spouseDed=Math.min(Math.max(spouseShare,5e8),30e8);}let financialDed=0;if(finW>0){if(finW<=2e7)financialDed=finW;else if(finW<=1e8)financialDed=2e7;else financialDed=Math.min(Math.round(finW*0.20),2e8);}const cohabDed=cohabW>0?Math.min(cohabW,6e8):0;const farmDed=farmW>0?Math.min(farmW,15e8):0;const totalDed=mainDed+spouseDed+financialDed+cohabDed+farmDed;const tb=Math.max(0,netEstate-totalDed);const tx=pTx(tb,GB);const rebate=onTime==="yes"?0.03:0;const finalTax=Math.round(tx*(1-rebate));return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div>{t<=0&&<StepsGuide steps={[{label:"총 상속재산가액 입력",filled:t>0},{label:"가족 구성 확인",filled:true}]}/>}<RPFull miss={t>0?null:MI.inherit} expertGuide={CALC_NAV_CONTENT.inherit?.items} expertGuide={CALC_NAV_CONTENT.inherit?.items} title="상속세"
...
function CalcConvert({isMo=false,onNav=()=>{}}){const[type,sType]=useState("m2j");const[deposit,sDep]=useState("");const[monthly,sMon]=useState("");const[rate,sRate]=useState("");const dep=tW(deposit),mon=tW(monthly),rt=pN(rate)/100;let result=0;if(type==="m2j"){result=rt>0?mon*12/rt+dep:0;}else{result=dep>0&&rt>0?(dep*rt)/12:0;}return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Tog label="전환 방향" value={type} onChange={sType} options={[{value:"m2j",label:"월세 → 전세"},{value:"j2m",label:"전세 → 월세"}]}/>{type==="m2j"?<><Inp label="보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 5000"/><Inp label="월세" value={monthly} onChange={sMon} suffix="만원" placeholder="예: 80"/></>:<Inp label="전세보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 30000"/>}<div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>전환율 <TipModal title=""><p>법정 상한: 기준금리(3%)+2% = 5.0% (2026년). 임대인이 초과 요구 불가.</p></TipModal></div><Inp label="" value={rate} onChange={sRate} suffix="%" note="2025 법정 전환율 상한: 기준금리+2%"/><RateTable title="전월세 전환율 기준" headers={["항목","기준"]} rows={[["법정 전환율 상한","기준금리+2%"],["현행 기준금리","3.0%"],["법정 상한","5.0%"],["시장 평균","약 4~6%"]]}/>
    </div><RPFull miss={(dep>0||mon>0)?null:MI.convert} expertGuide={CALC_NAV_CONTENT.convert?.items} expertGuide={CALC_NAV_CONTENT.convert?.items} title={type==="m2j"?"전세 환산금액":"월세 환산금액"}
...
function CalcNetSale({isMo=false,onNav=()=>{}}){
  ...
  return(<>{showLookup&&<AddressModal onClose={()=>setShowLookup(false)} onApplyPrice={v=>{setSellP(String(Math.round(v/10000)));setShowLookup(false);setAutoSaleFill(true);}}/>}<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <RPFull miss={sellW>0?null:MI.netsale} expertGuide={CALC_NAV_CONTENT.netsale?.items} expertGuide={CALC_NAV_CONTENT.netsale?.items} title="매도 실수령액" total={Math.max(0,net)} sub={"매도가 "+fW(sellW)+" 기준"}
...
function CalcReg({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);const _regErr=!p||p==="0";
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Tog label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Tog label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000" error={_regErr}/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div><div><RPFull miss={pW>0?null:MI.registration} expertGuide={CALC_NAV_CONTENT.registration?.items} expertGuide={CALC_NAV_CONTENT.registration?.items} title="등기비용 통합"
...
return(<>{showLookup&&<AddressModal onClose={()=>setShowLookup(false)} onApplyPrice={v=>{sPP(String(Math.round(v/10000)));setShowLookup(false);setAutoYieldFill(true);}}/>}<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>수익률 <TipModal title="임대수익률 계산법"><p><b>Gross:</b> 연임대료÷매입가 / <b>Net:</b> 순수익÷자기자본</p></TipModal></div>{!isMo&&<button onClick={()=>setShowLookup(true)} style={{fontSize:12,color:"#0141f9",background:"none",border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit",marginBottom:4,display:"flex",alignItems:"center",gap:4}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>주소로 실거래가 조회</button>}<Inp label="매입가" value={pp} onChange={v=>{sPP(v);setAutoYieldFill(false);}} suffix="만원" placeholder="예: 50000" error={!pp||pp==="0"}/>{autoYieldFill&&pp&&<div style={{fontSize:11,color:"#4285F4",marginTop:-8,marginBottom:8,fontWeight:500}}>📍 주소조회로 자동입력됨 · 직접 수정 가능</div>}{!pp&&<div style={{padding:"12px 14px",background:"#FFF4E6",border:"2px solid #FF8B00",borderRadius:10,fontSize:13,color:"#B8550A",marginTop:-4,marginBottom:12,fontWeight:700,boxShadow:"0 2px 8px rgba(255,139,0,0.15)",display:"flex",alignItems:"center",gap:8}}>💡 매매가격 입력 시 수익률이 계산됩니다</div>}<Inp label="보증금 (전세금)" value={dp} onChange={sDp} suffix="만원" placeholder="예: 10000"/><Inp label="월세" value={rn} onChange={sRn} suffix="만원" placeholder="예: 100"/><Inp label="월 관리비·경비" value={ex} onChange={sEx} suffix="만원" placeholder="예: 20"/><Inp label="대출금액" value={ln} onChange={sLn} suffix="만원" placeholder="예: 20000"/><Inp label="대출 금리" value={lr} onChange={sLr} suffix="%" placeholder="예: 4.0"/><RateTable title="수익률 기준" headers={["구분","수치","평가"]} rows={[["순수익률 10%↑","우수","적극 투자 고려"],["순수익률 5~10%","양호","시장 평균 수준"],["순수익률 3~5%","보통","개선 전략 필요"],["순수익률 3%↓","저조","대출 금리 대비 검토"]]}/>
    </div><RPFull miss={(!pp&&!dp&&!rn&&!ex&&!ln&&!lr)?MI.yield:null} expertGuide={CALC_NAV_CONTENT.yield?.items} expertGuide={CALC_NAV_CONTENT.yield?.items} isExample={(!pp&&!dp&&!rn&&!ex&&!ln&&!lr)} title="임대수익률"
...
function CalcJoint({isMo=false,onNav=()=>{}}){const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[showLookup,setShowLookup]=useState(false);const[autoJointFill,setAutoJointFill]=useState(false);const[r,sR]=useState("50");const pW=tW(p),rv=pN(r)/100;const sB=Math.max(0,(pW-12e8)*0.6);const j1B=Math.max(0,(pW*rv-9e8*rv)*0.6);const j2B=Math.max(0,(pW*(1-rv)-9e8*(1-rv))*0.6);const C2=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const sT=pTx(sB,C2),jT=pTx(j1B,C2)+pTx(j2B,C2),sv=sT-jT;return(<>{showLookup&&<AddressModal onClose={()=>setShowLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));setShowLookup(false);setAutoJointFill(true);}}/>}<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>공동명의 <TipModal title="공동명의 절세"><p>종부세: 단독 12억 vs 공동 각9억(합18억). 양도세: 각 250만 공제. 고령자·장기보유 공제 불가.</p></TipModal></div>{!isMo&&<button onClick={()=>setShowLookup(true)} style={{fontSize:12,color:"#0141f9",background:"none",border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit",marginBottom:4,display:"flex",alignItems:"center",gap:4}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>주소로 공시가격 조회</button>}<Inp label="부동산 공시가격" value={p} onChange={v=>{sP(v);setAutoJointFill(false);}} suffix="만원" placeholder="예: 150000" error={!p||p==="0"}/>{autoJointFill&&p&&<div style={{fontSize:11,color:"#4285F4",marginTop:-8,marginBottom:8,fontWeight:500}}>📍 주소조회로 자동입력됨 · 직접 수정 가능</div>}<div style={{marginBottom:16}}><label style={{display:"block",fontSize:12,fontWeight:600,color:P.mt,marginBottom:6}}>{"지분 비율 — 본인 "+r+"% : 배우자 "+(100-parseInt(r))+"%"}</label><input type="range" min="10" max="90" step="10" value={r} onChange={e=>sR(e.target.value)} style={{width:"100%",accentColor:P.pri}}/><div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:P.mt,marginTop:4}}><span>본인 10%</span><span>본인 90%</span></div></div><div style={{padding:"14px 16px",background:P.lt,borderRadius:10,fontSize:13,color:"#4a5568",lineHeight:1.6}}><b>단독명의:</b> 1주택 공제 12억, 고령자·장기보유 공제 가능<br/><b>공동명의:</b> 각 9억 공제 (합 18억), 고령자·장기보유 공제 불가</div><RateTable title="종합부동산세율 (2026)" headers={["과세표준","세율"]} rows={[["3억 이하","0.5%"],["6억 이하","0.7%"],["12억 이하","1.0%"],["25억 이하","1.3%"],["50억 이하","1.5%"],["94억 이하","2.0%"],["94억 초과","2.7%"]]}/>
    </div><RPFull miss={isDefault?MI.joint:null} expertGuide={CALC_NAV_CONTENT.joint?.items} expertGuide={CALC_NAV_CONTENT.joint?.items} isExample={isDefault} title={sv>0?"공동명의가 유리":"단독명의가 유리"}
...
function CalcHoldTax({isMo=false,onNav=()=>{}}){const[h,sH]=useState("1");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[showHoldLookup,setShowHoldLookup]=useState(false);const[selectedHoldInfo,setSelectedHoldInfo]=useState(null);const[autoHoldFill,setAutoHoldFill]=useState(false);const pW=tW(p);const n=parseInt(h);const ptBase=pW*0.6;const PTB=[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]];const ptTax=pTx(ptBase,PTB);const ptCity=Math.round(ptBase*0.0014);const ptEdu=Math.round(ptTax*0.2);const ptTotal=ptTax+ptCity+ptEdu;const cDed=n===1?12e8:9e8;const cBase=Math.max(0,(pW-cDed)*0.6);const CTB=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const cTax=pTx(cBase,CTB);const cFarm=Math.round(cTax*0.2);const cTotal=cTax+cFarm;const grand=ptTotal+cTotal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Radio label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>공시가격 <TipModal title=""><p>매년 6.1 기준. realtyprice.kr에서 확인. 재산세: 공시가×60%</p></TipModal></div>{!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowHoldLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showHoldLookup&&<AddressModal calcType="holding" onClose={()=>setShowHoldLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));markModified();setAutoHoldFill(true);}} onApplyInfo={info=>setSelectedHoldInfo(info)}/>}{selectedHoldInfo&&!isMo&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
      ...
      </div>}<Inp label="" value={p} onChange={v=>{sP(v);setAutoHoldFill(false);}} suffix="만원" placeholder="예: 150000" error={!p||p==="0"}/>{autoHoldFill&&p&&<div style={{fontSize:11,color:"#4285F4",marginTop:-8,marginBottom:8,fontWeight:500}}>📍 주소조회로 자동입력됨 · 직접 수정 가능</div>}</div><div><RPFull miss={isDefault?MI.holdtax:null} expertGuide={CALC_NAV_CONTENT.holdtax?.items} expertGuide={CALC_NAV_CONTENT.holdtax?.items} isExample={isDefault} title="연간 보유세 합계"
...
function CalcBond({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="매매 가격" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="지역" value={area} onChange={sArea} options={[{value:"seoul",label:"서울"},{value:"metro",label:"광역시"},{value:"etc",label:"기타 지역"}]}/><Tog label="용도" value={use} onChange={sUse} options={[{value:"house",label:"주택"},{value:"commercial",label:"상가·오피스텔"}]}/><Inp label="할인율 (실시세)" value={customRate} onChange={setCustomRate} suffix="%" note="증권사 당일 매도가. 일반 4~5%"/></div><RPFull miss={tW(p)>0?null:MI.bond} expertGuide={CALC_NAV_CONTENT.bond?.items} expertGuide={CALC_NAV_CONTENT.bond?.items} title="채권 할인비용"
...
function CalcDeposit({isMo=false,onNav=()=>{}}){const[amt,sAmt]=useState("");const[rt,sRt]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[mon,sMon]=useState("12");const[ty,sTy]=useState("saving");const[tax,sTax]=useState("normal");const amtW=tW(amt),r=pN(rt)/100,m=parseInt(mon);const taxRate=tax==="normal"?0.154:tax==="preferential"?0.095:0;let interest=0;if(ty==="saving"){interest=amtW*r*m/12;}else{interest=amtW*r*m*(m+1)/24;}const taxAmt=Math.round(interest*taxRate);const afterTax=interest-taxAmt;const total=(ty==="saving"?amtW:amtW*m)+afterTax;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label={ty==="saving"?"예금 원금":"월 납입액"} value={amt} onChange={sAmt} suffix="만원" placeholder="예: 1000" error={!amt||amt==="0"}/><Inp label="연이율" value={rt} onChange={sRt} suffix="%" placeholder="예: 3.5" error={!rt||rt==="0"} note="한국은행 기준금리: 3.00% (2026.02.27 동결)"/><Sel label="기간" value={mon} onChange={sMon} options={[3,6,12,18,24,36].map(m=>({value:String(m),label:m+"개월"}))}/><Tog label="상품 유형" value={ty} onChange={sTy} options={[{value:"saving",label:"예금 (거치식)"},{value:"installment",label:"적금 (적립식)"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>세금 유형 <TipModal title=""><p>일반 15.4% / 세금우대 9.5% / 비과세(ISA 등). 금융소득 2천만↑ 종합과세.</p></TipModal></div><Tog label="" value={tax} onChange={sTax} options={[{value:"normal",label:"일반 (15.4%)"},{value:"preferential",label:"세금우대 (9.5%)"},{value:"exempt",label:"비과세"}]}/><RateTable title="이자소득 세율" headers={["구분","세율"]} rows={[["일반과세","15.4%"],["세금우대","9.5%"],["비과세","0%"],["금융소득종합과세","2천만↑ 합산"]]}/>
    </div><RPFull miss={isDefault?MI.deposit:null} expertGuide={CALC_NAV_CONTENT.deposit?.items} expertGuide={CALC_NAV_CONTENT.deposit?.items} isExample={isDefault} title="세후 수령액"
...
function CalcAppraisal({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="감정가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="부가세 처리" value={vatType} onChange={setVatType} options={[{value:"include",label:"VAT 포함"},{value:"exclude",label:"VAT 별도"}]}/><div style={{padding:"14px 16px",background:P.lt,borderRadius:10,fontSize:12,color:"#4a5568",lineHeight:1.8,marginTop:8}}>• 5천만 이하: 기본 20만원<br/>• 5천만~2억: 20만 + 초과분×0.09%<br/>• 2~5억: 15.5만 + 초과분×0.08%<br/>• 5~10억: 39.5만 + 초과분×0.06%<br/>• 10~50억: 69.5만 + 초과분×0.04%</div><RateTable title="감정평가 수수료 기준" headers={["감정가액","수수료"]} rows={[["5천만 이하","20만원"],["5천~2억","20만+초과×0.09%"],["2~5억","15.5만+초과×0.08%"],["5~10억","39.5만+초과×0.06%"],["10~50억","69.5만+초과×0.04%"],["50억 초과","229.5만+초과×0.02%"]]}/>
    </div><RPFull miss={tW(p)>0?null:MI.appraisal} expertGuide={CALC_NAV_CONTENT.appraisal?.items} expertGuide={CALC_NAV_CONTENT.appraisal?.items} title="감정평가 수수료"
...
function CalcAuction({isMo=false,onNav=()=>{}}){const[bp,sBp]=useState("");const[ap,sAp]=useState("");const[fail,sFail]=useState("0");const bpW=tW(bp),apW=tW(ap),failN=parseInt(fail);const ratio=failN>0?Math.pow(0.8,failN):1;const minBid=Math.round(apW*ratio);const acqTax=Math.round(bpW*0.04);const regTax=Math.round(bpW*0.02*1.2);const legalFee=bpW<=3e8?220000:bpW<=5e8?310000:bpW<=10e8?420000:600000;const moveFee=3000000;const total=acqTax+regTax+legalFee+moveFee;const invested=bpW+total;const profit=apW>0?apW-invested:0;const roi=invested>0?profit/invested*100:0;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="낙찰가 (예상)" value={bp} onChange={sBp} suffix="만원" placeholder="예: 50000"/><Inp label="감정가" value={ap} onChange={sAp} suffix="만원" placeholder="예: 70000"/><Sel label="유찰 횟수" value={fail} onChange={sFail} options={[0,1,2,3,4,5].map(n=>({value:String(n),label:n===0?"신건 (유찰 없음)":n+"회 유찰 (최저가 "+(Math.pow(0.8,n)*100).toFixed(0)+"%)"}))}/><RateTable title="경매 취득 비용 기준" headers={["항목","기준"]} rows={[["취득세","주택 1~12%"],["등록면허세","취득세의 일부"],["법무사 보수","30~100만"],["명도비용","30~100만"],["점유이전비","약 300만"]]}/>
    </div><RPFull miss={bpW>0?null:MI.auction} expertGuide={CALC_NAV_CONTENT.auction?.items} expertGuide={CALC_NAV_CONTENT.auction?.items} title="경매 총비용"
...
function CalcRemodel({isMo=false,onNav=()=>{}}){const[cv,sCv]=useState("");const[cost,sCost]=useState("");const[ev,sEv]=useState("");const cvW=tW(cv),costW=tW(cost),evW=tW(ev);const profit=evW-cvW-costW;const roi=costW>0?profit/costW*100:0;const totalRoi=(cvW+costW)>0?profit/(cvW+costW)*100:0;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="기존 평가액 (현재 시세)" value={cv} onChange={sCv} suffix="만원" placeholder="예: 80000"/><Inp label="리모델링 분담금" value={cost} onChange={sCost} suffix="만원" placeholder="예: 15000"/><Inp label="리모델링 후 예상가" value={ev} onChange={sEv} suffix="만원" placeholder="예: 120000"/><RateTable title="리모델링 투자 기준" headers={["구분","기준"]} rows={[["수익률 20%↑","매우 우수"],["수익률 10~20%","양호"],["수익률 5~10%","보통"],["수익률 5%↓","재검토 필요"]]}/>
    </div><RPFull miss={(cvW>0&&costW>0)?null:MI.remodel} expertGuide={CALC_NAV_CONTENT.remodel?.items} title={profit>=0?"예상 수익":"예상 손실"}
...
function CalcIncTax({isMo=false,onNav=()=>{}}){const[incType,sIT]=useState("salary");const[gross,sGross]=useState("");const[deductions,sDed]=useState("");const[bizType,sBT]=useState("simple");const[bizRate,sBR]=useState("");
  ...
  </div><div><RPFull miss={tW(gross)>0?null:MI.inctax} expertGuide={CALC_NAV_CONTENT.inctax?.items} expertGuide={CALC_NAV_CONTENT.inctax?.items} title="종합소득세"
...
function CalcYearEnd({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  </div><div>{salW<=0&&<RequiredGuide items={[{label:"연봉 (세전)",filled:salW>0}]}/>}<RPFull miss={salW>0?null:MI.yearend} expertGuide={CALC_NAV_CONTENT.yearend?.items} expertGuide={CALC_NAV_CONTENT.yearend?.items} title={refund>=0?"예상 환급액":"추가 납부 예상"}
...
function CalcRetire({isMo=false,onNav=()=>{}}){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
  ...
  <RateTable title="퇴직소득 공제액" headers={["근속연수","공제액"]} rows={[["5년 이하","30만×근속"],["10년 이하","150만+50만×(근속-5)"],["20년 이하","400만+80만×(근속-10)"],["20년 초과","1,200만+120만×(근속-20)"]]}/>
    </div><RPFull miss={(monthlyW>0&&y>0)?null:MI.retire} expertGuide={CALC_NAV_CONTENT.retire?.items} expertGuide={CALC_NAV_CONTENT.retire?.items} title="예상 퇴직금"
...
function CalcUnemploy({isMo=false,onNav=()=>{}}){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
  ...
  <RateTable title="실업급여 수급기간" headers={["피보험기간","50세 미만","50세↑·장애"]} rows={[["1년 미만","120일","120일"],["1~3년","150일","180일"],["3~5년","180일","210일"],["5~10년","210일","240일"],["10년↑","240일","270일"]]}/>
    </div>{dailyW<=0&&<RequiredGuide items={[{label:"퇴직 전 평균임금",filled:dailyW>0}]}/>}<RPFull miss={dailyW>0?null:MI.unemploy} expertGuide={CALC_NAV_CONTENT.unemploy?.items} expertGuide={CALC_NAV_CONTENT.unemploy?.items} title="실업급여 예상"
...
function CalcCarTax({isMo=false,onNav=()=>{}}){const[carType,sCT]=useState("normal");const[cc,sCC]=useState("");const[year,sYear]=useState("0");const[isElec,sElec]=useState("no");
  ...
  <RateTable title="자동차세율표 (2026)" headers={["배기량","cc당 세액"]} rows={[["1,000cc 이하","80원"],["1,000~1,600cc","140원"],["1,600cc 초과","200원"],["전기차","10만원 (정액)"],["3년↑ 경감","매년 5% (최대 50%)"]]}/>
    </div><RPFull miss={(isElec==="yes"||ccVal>0)?null:MI.cartax} expertGuide={CALC_NAV_CONTENT.cartax?.items} expertGuide={CALC_NAV_CONTENT.cartax?.items} title="자동차세 (연간)"
...
function CalcMinWage({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}>
    ...
    <div><RPFull miss={parseFloat(weeklyH)>0?null:MI.minwage} expertGuide={CALC_NAV_CONTENT.minwage?.items} expertGuide={CALC_NAV_CONTENT.minwage?.items} title="예상 급여"
