...
function CalcAcq({isMo=false}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div>
      ...
      <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>시가표준액 <Tip text="공시가격. 미입력 시 취득가액으로 간주. 증여 시 조정지역 3억 초과 중과 판단, 다주택 1억 이하 중과 제외 판단에 사용. 부동산 공시가격 알리미에서 확인."/></div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        ...
        {showFirstDist&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={firstDistribution} onChange={e=>setFirstDistribution(e.target.checked)} style={{width:18,height:18}}/> 임대사업자 최초분양<Tip text="60㎡ 이하 공동주택·오피스텔 최초 분양 + 임대사업자 등록 시 취득세 면제/감면. 수도권 6억, 그 외 3억 이하."/></label>}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<Tip text="서울 전 지역, 경기 일부(과천·성남·하남 등). 2주택 8%, 3주택+ 12% 중과."/></label>}
        ...
        {showHeavyExclude&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={heavyTaxExclude} onChange={e=>setHeavyTaxExclude(e.target.checked)} style={{width:18,height:18}}/> 중과 배제<Tip text="가정어린이집, 농어촌주택, 공시가 1억 이하 등 중과 제외 대상."/></label>}
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:14}}>
        {showFirstOfLife&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={firstOfLife} onChange={e=>setFirstOfLife(e.target.checked)} style={{width:18,height:18}}/> 생애최초 구입<Tip text="무주택 세대가 12억 이하 주택을 생애 최초 구입 시 취득세 최대 200만원 감면 (2028년까지). 부부합산 소득 제한 없음."/></label>}
...
function CalcTrans({isMo=false}){
  ...
  return(<div><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
    <div style={{marginBottom:12}}><div style={{fontSize:isMo?14:12,fontWeight:600,color:"#505f79",marginBottom:6}}>양도물건 종류<Tip text="분양권: 1년미만 70% / 1년이상 60%. 입주권: 관리처분인가일 기준 분리 계산. 비사업토지: 기본세율+10%p 추가과세."/></div><div className="radio-grid" style={{display:"grid",gridTemplateColumns:isMo?"repeat(3,1fr)":"repeat(5,1fr)",gap:6}}>
...
function CalcYearEnd({isMo=false}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🧾 연말정산</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>신용카드 공제 <Tip text="총급여의 25% 초과 사용분부터 공제 시작. 신용카드 15%, 체크카드·현금영수증 30%, 전통시장 40%, 대중교통 80%."/></div>
