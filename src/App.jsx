...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div>
      ...
      </div>:<div><RPFull miss={MI.transfer} title="양도소득세" total={0} isExample={true} items={[]}/></div>}
...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <hr style={{border:"none",borderTop:"1px solid #E5E7EB",margin:"16px 0"}}/>
      ...
      total={total} sub={rateLabel+" ("+calcYear+"년 기준)"} items={rpItems}/></div></div>);}
...
function CalcProp({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Tog label="부동산 유형" value={pt} onChange={sPt} options={[{value:"house",label:"주택"},{value:"land",label:"토지"},{value:"com",label:"건축물"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>공시가격 <TipModal title=""><p>매년 1월 1일 기준 공시가격. realtyprice.kr에서 확인. 공정시장가액비율(60%)을 곱한 금액이 과세표준.</p></TipModal></div>{!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowPropLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showPropLookup&&<AddressModal calcType="holding" onClose={()=>setShowPropLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));markModified();setAutoPropFill(true);}} onApplyInfo={info=>setSelectedPropInfo(info)}/>}{selectedPropInfo&&!isMo&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
      ...
      total={total} sub={"공정시장가액비율 "+(fr*100).toFixed(0)+"% 적용"} items={rpItems}/></div></div>);}
...
function CalcLegal({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[p,sP]=useState("");const pW=tW(p);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Sel label="업무 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"소유권이전 (매매)"},{value:"gift",label:"소유권이전 (증여)"},{value:"inherit",label:"소유권이전 (상속)"},{value:"mortgage",label:"근저당 설정등기"},{value:"release",label:"말소등기"}]}/><Inp label="시가표준액 (과세표준)" value={p} onChange={sP} suffix="만원" placeholder="예: 90000" note="대한법무사협회 보수표 기준 (참고용)" error={!p||p==="0"}/><div style={{padding:"10px 14px",background:P.lt,borderRadius:8,fontSize:11,color:"#6b778c",lineHeight:1.5,marginTop:8}}>※ 증여/상속은 매매 대비 약 80% 수준. 실제 비용은 법무사와 협의에 따라 달라질 수 있습니다.</div></div><div><RPFull miss={pW>0?null:MI.legal} title="법무사 비용 총합계"
      ...
      alertType="info"
      total={total} sub="보수 + VAT + 부대비용" items={[{l:"업무 유형",v:tx==="sale"?"소유권이전(매매)":tx==="gift"?"소유권이전(증여)":tx==="inherit"?"소유권이전(상속)":tx==="mortgage"?"근저당 설정등기":"말소등기"},{l:"기본 보수",v:fW(bf),note:"법무사협회 기준"},{l:"교통비·일당",v:fW(travel)},{l:"서류발급비",v:fW(doc)},{l:"부가가치세 (10%)",v:fW(vat),note:"기본 보수 기준"},{l:"총 합계 (보수+경비+VAT)",v:fW(total)}]}/></div></div>);}
...
function CalcLoanMax({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="연 소득" value={inc} onChange={sInc} suffix="만원" placeholder="예: 5000" error={!inc||inc==="0"}/><Inp label="주택가격 (시가)" value={price} onChange={sPrice} suffix="만원" placeholder="예: 90000" note="입력 시 LTV·주담대 한도 적용"/><Tog label="주택 소재지" value={region} onChange={sRegion} options={[{value:"metro",label:"수도권·규제지역"},{value:"non",label:"비수도권"}]}/><Tog label="DSR 기준" value={dsrLim} onChange={sDsrLim} options={[{value:"40",label:"은행 40%"},{value:"50",label:"비은행 50%"}]}/><Tog label="대출 유형 (스트레스)" value={loanType} onChange={sLT} options={[{value:"fixed",label:"고정"},{value:"mixed",label:"혼합(+0.75p)"},{value:"variable",label:"변동(+1.5p)"}]}/><Inp label="대출 금리" value={rt} onChange={sRt} suffix="%" placeholder="예: 3.5" note={stressRate>0?"스트레스 가산 후 "+fP(adjRate*100)+" 적용":"고정금리 가산 없음"}/>{!isMo&&<BaseRateHint/>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Sel label="대출 기간" value={yr} onChange={sYr} options={[5,10,15,20,25,30,35,40].map(y=>({value:String(y),label:y+"년"}))}/><Sel label="차주 연령대" value={age} onChange={sAge} options={[{value:"20",label:"20대"},{value:"30",label:"30대"},{value:"40",label:"40대"},{value:"50",label:"50대"},{value:"60",label:"60대"}]}/></div><Inp label="기존 대출 월 상환액" value={ep} onChange={sEp} suffix="만원" placeholder="없으면 0"/>{region==="metro"&&pW>0&&<div style={{padding:"12px 16px",background:"#FFF8E1",border:"1px solid #FFE082",borderRadius:10,fontSize:13,color:"#F57F17",marginTop:12,lineHeight:1.6}}>수도권·규제지역 주담대 한도: 15억↓ 6억 / 25억↓ 4억 / 25억↑ 2억 (2025.6.28~ 시행)</div>}</div><div><RPFull miss={tW(inc)>0?null:MI.loanmax} title="최대 대출가능액"
      ...
      total={Math.max(0,maxLoan)} sub={"제한 요인: "+limitReason} items={[{l:"연 소득",v:fW(ai)},{l:"실제 적용 금리",v:fP(adjRate*100),note:stressRate>0?"스트레스 +"+fP(stressRate*100)+"p":"가산 없음"},{l:"적용 대출 기간",v:adjYear+"년",note:parseInt(yr)>maxTerm?"연령 최대 "+maxTerm+"년":""},{l:"DSR 기준 대출가능액",v:fW(Math.max(0,dsrLoan))}].concat(pW>0?[{l:"LTV 70% 기준",v:fW(ltvLoan)}]:[]).concat(region==="metro"&&pW>0?[{l:"수도권 주담대 한도",v:metroCap===Infinity?"제한없음":fW(metroCap)}]:[]).concat([{l:"최대 대출가능액",v:fW(Math.max(0,maxLoan)),note:limitReason+" 적용"},{l:"적용 DSR",v:fP(actualDsr)}])}/></div></div>);}
...
function CalcHoldTax({isMo=false,onNav=()=>{}}){const[h,sH]=useState("1");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[showHoldLookup,setShowHoldLookup]=useState(false);const[selectedHoldInfo,setSelectedHoldInfo]=useState(null);const[autoHoldFill,setAutoHoldFill]=useState(false);const pW=tW(p);const n=parseInt(h);const ptBase=pW*0.6;const PTB=[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]];const ptTax=pTx(ptBase,PTB);const ptCity=Math.round(ptBase*0.0014);const ptEdu=Math.round(ptTax*0.2);const ptTotal=ptTax+ptCity+ptEdu;const cDed=n===1?12e8:9e8;const cBase=Math.max(0,(pW-cDed)*0.6);const CTB=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const cTax=pTx(cBase,CTB);const cFarm=Math.round(cTax*0.2);const cTotal=cTax+cFarm;const grand=ptTotal+cTotal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Radio label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>공시가격 <TipModal title=""><p>매년 6.1 기준. realtyprice.kr에서 확인. 재산세: 공시가×60%</p></TipModal></div>{!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowHoldLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showHoldLookup&&<AddressModal calcType="holding" onClose={()=>setShowHoldLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));markModified();setAutoHoldFill(true);}} onApplyInfo={info=>setSelectedHoldInfo(info)}/>}{selectedHoldInfo&&!isMo&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
      ...
      total={grand} sub={"공시가 "+fW(pW)+" 기준 / 재산세+종부세"} items={[{l:"▼ 재산세 (7·9월 납부)",v:fW(ptTotal),note:"지방세"},{l:"  과세표준 (공정가 60%)",v:fW(ptBase)},{l:"  재산세 본세",v:fW(ptTax)},{l:"  도시지역분 (0.14%)",v:fW(ptCity)},{l:"  지방교육세 (20%)",v:fW(ptEdu)},{l:"▼ 종합부동산세 (12월 납부)",v:fW(cTotal),note:"국세"},{l:"  공제금액",v:fW(cDed)},{l:"  과세표준 (공정가 60%)",v:fW(cBase)},{l:"  종부세",v:fW(cTax)},{l:"  농어촌특별세 (20%)",v:fW(cFarm)},{l:"연간 보유세 총합",v:fW(grand),note:"재산세+종부세"}]}/></div></div>);}
...
function CalcBond({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="매매 가격" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="지역" value={area} onChange={sArea} options={[{value:"seoul",label:"서울"},{value:"metro",label:"광역시"},{value:"etc",label:"기타 지역"}]}/><Tog label="용도" value={use} onChange={sUse} options={[{value:"house",label:"주택"},{value:"commercial",label:"상가·오피스텔"}]}/><Inp label="할인율 (실시세)" value={customRate} onChange={setCustomRate} suffix="%" note="증권사 당일 매도가. 일반 4~5%"/></div><RPFull miss={tW(p)>0?null:MI.bond} title="채권 할인비용"
      ...
      alertType="info"
      total={discountCost} sub={"매입률 "+fP(bondRate*100)+" · 할인율 "+fP(discountRate*100)} items={[{l:"매매가격",v:fW(pW)},{l:"적용 지역·용도",v:(area==="seoul"?"서울":area==="metro"?"광역시":"기타")+" / "+(use==="house"?"주택":"상가")},{l:"채권매입률",v:fP(bondRate*100)},{l:"채권매입액",v:fW(bondAmt),note:"매매가×매입률"},{l:"할인율 (시세)",v:fP(discountRate*100)},{l:"실제 부담 할인비용",v:fW(discountCost),note:"매입액×할인율"}]}/></div>);}
    ...
    <>
      ...
      <div className="calc-grid page-layout" style={{padding:isMo?"16px":24}}>
        ...
        <div>
          ...
          {!isMo&&calc!=="acquisition"&&<div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:28}}><div/><NextStep calcId={calc} onNav={navigateCalc} isMo={false}/></div>}
