...
function CalcAcq({isMo=false}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div>
      ...
      <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>시가표준액 <TipModal title="시가표준액 (공시가격)"><p>미입력 시 취득가액을 시가표준액으로 간주합니다.</p><ul style={{paddingLeft:20}}><li>취득가액보다 시가표준액이 크면 시가표준액이 과세표준</li><li>시가표준액 1억 미만이면 다주택 중과 제외</li><li>조정대상지역 증여 시 시가표준액 3억 초과하면 12% 중과</li></ul><p>부동산 공시가격 알리미(realtyprice.kr)에서 확인 가능합니다.</p></TipModal></div>
...
function CalcTrans({isMo=false}){
  ...
  return(<div><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
    ...
    <div className="calc-inner" style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:12,marginBottom:12}}>
      <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>필요경비 <Tip text="취득세·등기비·중개수수료·인테리어비(자본적지출) 등. 대출이자·재산세는 필요경비 불인정."/></div>
...
const CX=[[3e8,.005],[6e8,.007],[12e8,.010],[25e8,.020],[50e8,.030],[94e8,.040],[Infinity,.050]];
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>공시가격 <Tip text="매년 6월 1일 기준 공시가격 합계. 공정시장가액비율 60%를 곱한 금액이 과세표준."/></div>
...
function CalcYearEnd({isMo=false}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🧾 연말정산</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>신용카드 공제 <TipModal title="신용카드 등 사용금액 소득공제"><p>총급여의 25%를 초과 사용한 금액부터 공제가 시작됩니다.</p><ul style={{paddingLeft:20}}><li><b>신용카드:</b> 15%</li><li><b>체크카드·현금영수증:</b> 30%</li><li><b>전통시장:</b> 40%</li><li><b>대중교통:</b> 80%</li></ul><p>공제 한도: 총급여 7천만원 이하 300만, 1.2억 이하 250만, 1.2억 초과 200만</p></TipModal></div>
...
function CalcNetSalary({isMo=false}){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💰 연봉 실수령액</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2}}>비과세 <Tip text="식대 월 20만원 등 비과세 항목. 비과세 금액은 세금 계산에서 제외됨. 근로계약서/급여명세서에서 확인."/></div>
