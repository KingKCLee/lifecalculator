...
acquisition:{title:"🏠 부동산 거래 통계",items:["서울 아파트 평균 매매가: 약 12.5억원 (2025년 기준)","전국 주택 거래량 월평균: 약 5.2만건","생애최초 취득세 감면 수혜자: 연 약 15만명"]},
...
/* ── 세무 캘린더 ── */
const TAX_CALENDAR={
1:[{day:"1.1~1.31",title:"자동차세 연납 신청",desc:"연납 시 약 5% 할인",icon:"🚗",calc:{cat:"life",id:"cartax"},urgent:true},{day:"1.25",title:"부가가치세 확정신고",desc:"7~12월분 (일반과세자)",icon:<IconClip/>},{day:"1.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
2:[{day:"2.1~2.28",title:"연말정산 서류 제출",desc:"직장인 소득·세액공제 신고",icon:"🧾",calc:{cat:"tax",id:"yearend"},urgent:true},{day:"2.28",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
3:[{day:"3.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>},{day:"3.31",title:"법인세 신고·납부",desc:"12월 결산법인",icon:<IconBank/>}],
4:[{day:"4.1~4.30",title:"종합소득세 미리 준비",desc:"5월 신고 전 경비 정리",icon:"📊",calc:{cat:"tax",id:"inctax"}},{day:"4.25",title:"부가가치세 예정신고",desc:"1~3월분 (일반과세자)",icon:<IconClip/>},{day:"4.30",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
5:[{day:"5.1~5.31",title:"종합소득세 확정신고",desc:"프리랜서·사업자 필수",icon:"💰",calc:{cat:"tax",id:"inctax"},urgent:true},{day:"5.31",title:"개인지방소득세 신고",desc:"종합소득세와 동시 신고",icon:"🏛️"},{day:"5.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
6:[{day:"6.1",title:"종합부동산세·재산세 기준일",desc:"6.1 보유자에게 과세",icon:"🏘️",calc:{cat:"tax",id:"compre"}},{day:"6.30",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
7:[{day:"7.1~7.31",title:"재산세 1기분 납부",desc:"건물분+토지분 절반",icon:"🏡",calc:{cat:"tax",id:"property"},urgent:true},{day:"7.25",title:"부가가치세 확정신고",desc:"1~6월분 (일반과세자)",icon:<IconClip/>},{day:"7.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
8:[{day:"8.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>},{day:"8.31",title:"주민세(균등분) 납부",desc:"개인 사업장 주민세",icon:"🏛️"}],
9:[{day:"9.1~9.30",title:"재산세 2기분 납부",desc:"토지분 나머지 절반",icon:"🏡",calc:{cat:"tax",id:"property"},urgent:true},{day:"9.30",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
10:[{day:"10.25",title:"부가가치세 예정신고",desc:"7~9월분 (일반과세자)",icon:<IconClip/>},{day:"10.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
11:[{day:"11.15~12.1",title:"종합부동산세 고지·납부",desc:"12.15까지 납부",icon:"🏘️",calc:{cat:"tax",id:"compre"}},{day:"11.30",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
12:[{day:"12.1~12.15",title:"종합부동산세 납부 마감",desc:"분납 가능 (250만원 초과)",icon:"🏘️",calc:{cat:"tax",id:"compre"},urgent:true},{day:"12.1~12.31",title:"자동차세 2기분 납부",desc:"7~12월분",icon:"🚗",calc:{cat:"life",id:"cartax"},urgent:true},{day:"12.31",title:"원천세 신고·납부",desc:"전월분 근로·사업소득",icon:<IconUser/>}],
};
const UTILITY_DATES=[{name:"아파트 관리비",day:"매월 25일 전후",icon:<IconBank/>},{name:"전기요금",day:"매월 말일",icon:"⚡"},{name:"가스요금",day:"매월 말일",icon:"🔥"},{name:"수도요금",day:"홀수월 말일",icon:"💧"},{name:"통신요금",day:"매월 말일",icon:"📱"},{name:"건강보험료",day:"매월 10일",icon:"🏥"}];
...
function getConsultFunnel(calcId,total){
  ...
  if(calcId==="acquisition"&&total>=3e7)return{label:"💡 절세 전략 상담받기",color:"#FF8B00",subtitle:"취득세 3천만 이상 — 전문가 상담으로 생애최초·중과배제 포인트 확인"};
  if(calcId==="transfer"&&total>=1e8)return{label:"🏛 세무사 무료 상담",color:"#0747A6",subtitle:"양도세 1억 이상 — 장특공·중과유예·비과세 전략으로 수백만원 절감 가능"};
  if(calcId==="transfer"&&total>=3e7)return{label:"📊 양도세 절세 상담",color:"#0747A6",subtitle:"양도세 부담이 큽니다. 필요경비·기본공제 최적화 상담"};
  if((calcId==="property"||calcId==="compre"||calcId==="holdtax")&&total>=5e6)return{label:"💰 보유세 절감 전략 상담",color:"#FF8B00",subtitle:"보유세 500만 이상 — 합산배제·세부담상한·고령자공제 전략 안내"};
  if(calcId==="gift"&&total>=5e7)return{label:"🎁 증여 타이밍 상담",color:"#6554C0",subtitle:"증여세 5천만 이상 — 10년 합산·분할증여 전략 상담"};
  if(calcId==="inherit"&&total>=1e8)return{label:"⚖️ 상속 전략 상담",color:"#6554C0",subtitle:"상속세 1억 이상 — 배우자공제·연부연납·가업상속 상담"};
  return null;
}
  ...
  <div style={{background:"linear-gradient(180deg,#0747A6 0%,#0052CC 100%)",borderRadius:20,padding:"28px 24px",color:"#fff",position:isMo?"relative":"sticky",top:isMo?0:80,alignSelf:"start",boxShadow:"0 8px 28px rgba(7,71,166,.22)",width:"100%",minWidth:isMo?"auto":320,boxSizing:"border-box"}}>
    ...
    {total>0&&<div style={{marginTop:10,padding:tier==="strong"?"14px 16px":tier==="medium"?"12px 14px":"10px 12px",background:tier==="strong"?"rgba(255,139,0,.22)":tier==="medium"?"rgba(255,255,255,.14)":"rgba(255,255,255,.08)",border:tier==="strong"?"1.5px solid #FFC400":"1px solid rgba(255,255,255,.22)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
      <div style={{flex:"1 1 auto",minWidth:0}}>
        <div style={{fontSize:tier==="strong"?13:12,fontWeight:tier==="strong"?800:700,color:tier==="strong"?"#FFC400":"#fff",marginBottom:2}}>{tier==="strong"?"⚡ 이 계산 결과가 복잡하신가요?":"이 계산 결과가 복잡하신가요?"}</div>
...
function MarketIntel({isMo}){
  ...
  return(<div style={{marginBottom:16,padding:isMo?"18px 16px":"24px 28px",background:"#fff",borderRadius:14,border:`1px solid ${P.bd}`}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:6}}>
      <div style={{fontSize:13,fontWeight:700,color:P.tx}}>📊 Market Intelligence</div>
...
function CalcAcq({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <button onClick={()=>onNav("realestate","realprice")} style={{width:"100%",marginTop:12,padding:"12px 16px",background:"#fff",border:"1.5px solid #0747A6",borderRadius:10,fontSize:14,fontWeight:700,color:"#0747A6",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🔍 근처 실거래가 조회 →</button>
    </div>
    <div>
...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>양도소득세 계산기</h3>}
    ...
    {result&&<div style={{marginTop:24}}>
      ...
      <div style={{marginTop:12,padding:"10px 14px",background:"#f8f9fc",border:"1px solid #dfe1e6",borderRadius:8,fontSize:12,color:"#505f79",lineHeight:1.6,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}><span>📅 신고기한: 양도일이 속한 달 말일로부터 2개월 이내</span><a href="https://hometax.go.kr" target="_blank" rel="noopener noreferrer" style={{color:"#0747A6",fontWeight:700,textDecoration:"none"}}>홈택스 신고 →</a></div>
      ...
      {(result.totalTax>=1e8||((result.own==="two"||result.own==="more")&&result.conArea))&&<button onClick={()=>window.dispatchEvent(new CustomEvent('lc-consult',{detail:{title:"양도소득세",total:result.totalTax}}))} style={{display:"block",width:"100%",marginTop:12,padding:"14px 16px",background:result.totalTax>=1e8?"#0747A6":"#FF8B00",color:"#fff",border:"none",borderRadius:12,fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 12px rgba(0,0,0,.15)",textAlign:"left",lineHeight:1.5}}>
        <div style={{fontSize:15,fontWeight:800,marginBottom:2}}>{result.totalTax>=1e8?"🏛 세무사 무료 상담 →":"📊 다주택 절세 전략 보기 →"}</div>
    ...
    {/* 2026.04.14 실거래가 조회 연동 */}
    <button onClick={()=>onNav("realestate","realprice")} style={{width:"100%",marginTop:12,padding:"12px 16px",background:"#fff",border:"1.5px solid #0747A6",borderRadius:10,fontSize:14,fontWeight:700,color:"#0747A6",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🔍 근처 실거래가 조회 →</button><RateTable title="양도세율표 (2년 이상 보유)" headers={["과세표준","세율","누진공제"]} rows={[["1,400만 이하","6%","-"],["5,000만 이하","15%","126만"],["8,800만 이하","24%","576만"],["1.5억 이하","35%","1,544만"],["3억 이하","38%","1,994만"],["5억 이하","40%","2,594만"],["10억 이하","42%","3,594만"],["10억 초과","45%","6,594만"]]}/>
...
function CalcDSR({isMo=false,onNav=()=>{}}){const[inc,sInc]=useState("5000");const[nl,sNl]=useState("30000");const[nr,sNr]=useState("3.5");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[ny,sNy]=useState("30");const[ep,sEp]=useState("");const[loanType,sLT]=useState("fixed");const[age,sAge]=useState("30");const ai=tW(inc),nlW=tW(nl),epW=tW(ep);const stressRate=loanType==="fixed"?0:loanType==="mixed"?0.0075:0.015;const adjRate=pN(nr)/100+stressRate;const nrV=adjRate/12;const ageVal=parseInt(age);let maxTerm=40;if(ageVal>=50)maxTerm=20;else if(ageVal>=40)maxTerm=30;else if(ageVal>=30)maxTerm=35;const adjYear=Math.min(parseInt(ny),maxTerm);const nn=adjYear*12;let nm=0;if(nlW>0&&nrV>0&&nn>0)nm=nlW*nrV*Math.pow(1+nrV,nn)/(Math.pow(1+nrV,nn)-1);const tot=nm*12+epW*12,dsr=ai>0?tot/ai*100:0;let st="양호",sc="#00875A";if(dsr>40){st="한도 초과";sc="#DE350B";}else if(dsr>30){st="주의";sc="#FF8B00";}
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>DSR 계산기</h3>}<Inp label="연 소득" value={inc} onChange={sInc} suffix="만원" placeholder="예: 5000"/><Inp label="신규 대출금액" value={nl} onChange={sNl} suffix="만원" placeholder="예: 30000"/><Inp label="신규 대출 금리" value={nr} onChange={sNr} suffix="%" placeholder="예: 3.5"/><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>대출 유형 <TipModal title="스트레스 DSR"><p>스트레스 DSR: 변동금리 +1.5%p, 혼합형 +0.75%p 가산금리 추가. 고정금리는 가산 없음.</p></TipModal></div><Tog label="대출 유형" value={loanType} onChange={sLT} options={[{value:"fixed",label:"고정금리"},{value:"mixed",label:"혼합형 (5년 고정)"},{value:"variable",label:"변동금리"}]}/><Sel label="대출 기간" value={ny} onChange={sNy} options={[5,10,15,20,25,30,35,40].map(y=>({value:String(y),label:y+"년"}))}/><Sel label="차주 연령대" value={age} onChange={sAge} options={[{value:"20",label:"20대"},{value:"30",label:"30대"},{value:"40",label:"40대"},{value:"50",label:"50대"},{value:"60",label:"60대"}]}/><Inp label="기존 대출 월 상환액 합계" value={ep} onChange={sEp} suffix="만원" placeholder="없으면 0"/></div><div>{ai>0&&nlW>0?<><div style={{background:"linear-gradient(135deg,#0747A6 0%,#0065FF 100%)",borderRadius:16,padding:"28px 24px",color:"#fff",textAlign:"center"}}><div style={{fontSize:11,letterSpacing:1.5,opacity:.8,marginBottom:16}}>DSR 진단 결과 (스트레스 DSR 반영)</div><div style={{fontSize:52,fontWeight:800,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{fP(dsr)}</div><div style={{fontSize:16,fontWeight:600,marginTop:4,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{st}</div><div style={{background:"rgba(255,255,255,.2)",borderRadius:8,height:12,margin:"12px 0 4px"}}><div style={{height:"100%",borderRadius:8,width:Math.min(dsr,100)+"%",background:dsr>40?"#ef4444":dsr>30?"#f59e0b":"#22c55e",transition:"width .4s"}}/></div><div style={{display:"flex",justifyContent:"space-between",fontSize:10,opacity:.7,marginBottom:8}}><span>0%</span><span>40% 한도</span><span>100%</span></div><div style={{fontSize:12,opacity:.7,marginTop:8}}>은행권 40% / 비은행권 50% 기준</div><div style={{borderTop:"1px solid rgba(255,255,255,.2)",marginTop:16,paddingTop:12,textAlign:"left",fontSize:13}}><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>신규 월 상환액</span><span style={{fontWeight:600}}>{fW(nm)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>연간 총 원리금</span><span style={{fontWeight:600}}>{fW(tot)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>스트레스 가산</span><span style={{fontWeight:600}}>{stressRate>0?"+"+fP(stressRate*100)+"p":"없음 (고정)"}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>실제 적용 금리</span><span style={{fontWeight:600}}>{fP(pN(nr)+stressRate*100)} {stressRate>0?"(+"+(stressRate*100).toFixed(2)+"p)":"(가산 없음)"}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>연령별 최대 만기</span><span style={{fontWeight:600}}>{maxTerm}년{parseInt(ny)>maxTerm?" ⚠️":"" }</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>은행권 40% 기준 최대 대출</span><span style={{fontWeight:600}}>{fW(dsrLimitAmt40)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>비은행 50% 기준 최대 대출</span><span style={{fontWeight:600}}>{fW(dsrLimitAmt50)}</span></div></div></div>{(dsr>30||true)&&<div style={{marginTop:12,padding:"10px 14px",background:dsr>40?"#FFEBE6":dsr>30?"#FFF8E1":"#f8f9fc",border:"1px solid "+(dsr>40?"#FFBDAD":dsr>30?"#FFE082":"#dfe1e6"),borderRadius:8,fontSize:12,color:dsr>40?"#BF2600":dsr>30?"#F57F17":"#505f79",lineHeight:1.6}}>{dsr>40?"⚠️ DSR 한도 초과: 대출 승인 어려울 수 있습니다":dsr>30?"⚠️ DSR 30% 초과: 추가 대출 시 주의":"ℹ️ DSR 규제: 은행 40% / 비은행 50% / 스트레스DSR 변동금리 +1.5%p 가산"}</div>}<div style={{marginTop:8,padding:"10px 14px",background:"#f8f9fc",border:"1px solid #dfe1e6",borderRadius:8,fontSize:12,color:"#505f79",lineHeight:1.6,display:"flex",alignItems:"center",gap:6}}><span>📅</span><span>은행 주담대 심사 통상 3~7영업일 · 본 계산은 2026년 스트레스 DSR 기준</span></div></>:<StepsGuide steps={[{label:"연 소득 입력",filled:ai>0},{label:"신규 대출금액 입력",filled:nlW>0},{label:"대출 금리 입력",filled:pN(nr)>0}]}/>}{dsr>40&&<button onClick={()=>window.dispatchEvent(new CustomEvent('lc-consult',{detail:{title:"DSR 한도 초과",total:Math.round(nlW)}}))} style={{display:"block",width:"100%",marginTop:12,padding:"14px 16px",background:"#FF8B00",color:"#fff",border:"none",borderRadius:12,fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 12px rgba(0,0,0,.15)",textAlign:"left",lineHeight:1.5}}><div style={{fontSize:15,fontWeight:800,marginBottom:2}}>🏦 대출 전문가 연결 →</div><div style={{fontSize:11,fontWeight:500,opacity:.92}}>DSR 40% 초과 — 보금자리·디딤돌·정책자금 대안 상담</div></button>}<NextStep calcId="dsr" onNav={onNav} isMo={isMo}/><RateTable title="DSR 규제 기준" headers={["구분","DSR"]} rows={[["은행권","40%"],["비은행권(보험·저축은행)","50%"],["스트레스 DSR 가산","변동+1.5%p / 혼합+0.75%p"]]}/></div></div>);}
...
function CalcComm({isMo=false,onNav=()=>{}}){const[ty,sTy]=useState("res");const[tx,sTx]=useState("sale");const[p,sP]=useState("90000");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[dep,sDep]=useState("5000");const[mon,sMon]=useState("50");const[mult,sMult]=useState("100");const[showRateTable,setShowRateTable]=useState(false);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>중개수수료 계산기</h3>}<div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>유형 <TipModal title="부동산 유형별 중개보수"><p>주택: 0.4~0.7% / 오피스텔: 0.5% / 상가: 0.9% 이내 협의</p></TipModal></div><Sel label="부동산 유형" value={ty} onChange={sTy} options={[{value:"res",label:"주택·아파트·빌라"},{value:"offi",label:"오피스텔"},{value:"com",label:"상가·토지·건물"},{value:"right",label:"분양권 전매"}]}/>{ty==="offi"&&<Tog label="오피스텔 용도" value={offiSub} onChange={setOffiSub} options={[{value:"res",label:"주거용 (주택요율)"},{value:"biz",label:"업무용 (0.5%)"}]}/>}<Tog label="거래 유형" value={tx} onChange={sTx} options={ty==="right"?[{value:"sale",label:"전매 (매매요율)"}]:[{value:"sale",label:"매매"},{value:"lease",label:"전세"},{value:"rent",label:"월세"}]}/>{tx==="rent"?(<><Inp label="보증금" value={dep} onChange={sDep} suffix="만원" placeholder="예: 5000"/><Inp label="월세" value={mon} onChange={sMon} suffix="만원" placeholder="예: 50"/><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>환산배수 <TipModal title="월세 환산배수"><p>보증금+월세×배수로 거래금액 환산. 일반: ×100 / 서울 등: ×70</p></TipModal></div><Tog label="월세 환산배수" value={mult} onChange={sMult} options={[{value:"100",label:"×100 (일반)"},{value:"70",label:"×70 (서울 등 특정지역)"}]}/><div style={{padding:"10px 14px",background:P.lt,borderRadius:8,fontSize:12,color:"#6b778c",lineHeight:1.6,marginBottom:16}}>환산보증금 = 보증금 + 월세 × {multiplier} = {fW(pW)}</div></>):(<Slider label="거래금액" value={p} onChange={v=>{sP(v);markModified();}} min={1000} max={500000} step={500}/>)}{ty!=="res"&&<div style={{padding:"10px 14px",background:"#FFF8E1",border:"1px solid #FFE082",borderRadius:10,fontSize:12,color:"#F57F17",marginTop:8,lineHeight:1.6}}>{ty==="offi"?"오피스텔 요율: 매매 0.5%, 전세·월세 0.4% (협의)":"상가·토지·건물: 0.9% 이내 협의 (부가세 별도)"}</div>}{rentBelowDeposit&&<div style={{padding:"10px 14px",background:"#eff6ff",border:"1px solid #93c5fd",borderRadius:10,fontSize:12,color:"#1e40af",marginTop:8,lineHeight:1.6}}>환산보증금({fW(convertedAmt)})이 보증금({fW(tW(dep))})보다 작아 보증금 기준 요율을 적용합니다.</div>}</div><div><RP isExample={isDefault} title="중개보수 (수수료)"
      ...
      {vt>0&&<div style={{padding:"8px 12px",marginTop:8}}><Tog label="VAT 처리" value={vatMode} onChange={setVatMode} options={[{value:"incl",label:"합계 포함"},{value:"excl",label:"별도 표기"}]}/></div>}<div style={{fontSize:12,color:"#64748b",padding:"8px 16px",lineHeight:1.6}}>💡 법정 상한요율이므로 실제 수수료는 협의로 낮출 수 있습니다</div><NextStep calcId="commission" onNav={onNav} isMo={isMo}/><button onClick={()=>setShowRateTable(!showRateTable)} style={{marginTop:16,width:"100%",padding:"10px 14px",background:"#f4f5f7",border:"1px solid #dfe1e6",borderRadius:8,fontSize:13,fontWeight:600,color:"#0747A6",cursor:"pointer",fontFamily:"inherit"}}>요율표 {showRateTable?"접기 ▲":"보기 ▼"}</button>{showRateTable&&<RateTable title="중개보수 상한요율 (매매)" headers={["거래금액","상한요율","한도액"]} rows={[["5천만 미만","0.6%","25만"],["2억 미만","0.5%","80만"],["9억 미만","0.4%","-"],["12억 미만","0.5%","-"],["15억 미만","0.6%","-"],["15억 이상","0.7%","-"]]}/>}</div></div>);}
...
function CalcConvert({isMo=false,onNav=()=>{}}){const[type,sType]=useState("m2j");const[deposit,sDep]=useState("");const[monthly,sMon]=useState("");const[rate,sRate]=useState("5");const dep=tW(deposit),mon=tW(monthly),rt=pN(rate)/100;let result=0;if(type==="m2j"){result=rt>0?mon*12/rt+dep:0;}else{result=dep>0&&rt>0?(dep*rt)/12:0;}return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>전월세전환 계산기</h3>}<Tog label="전환 방향" value={type} onChange={sType} options={[{value:"m2j",label:"월세 → 전세"},{value:"j2m",label:"전세 → 월세"}]}/>{type==="m2j"?<><Inp label="보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 5000"/><Inp label="월세" value={monthly} onChange={sMon} suffix="만원" placeholder="예: 80"/></>:<Inp label="전세보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 30000"/>}<div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>전환율 <TipModal title="전월세 전환율"><p>법정 상한: 기준금리(3%)+2% = 5.0% (2026년). 임대인이 초과 요구 불가.</p></TipModal></div><Inp label="전환율 (연 %)" value={rate} onChange={sRate} suffix="%" note="2025 법정 전환율 상한: 기준금리+2%"/></div><RP title={type==="m2j"?"전세 환산금액":"월세 환산금액"}
      ...
      alertMsg={pN(rate)>5?"⚠️ 법정 상한 5.0% 초과: 주택임대차보호법 위반":pN(rate)>=4.5?"법정 상한 근접: 임대인 요구 시 협의 권장":null}
...
function CalcRti({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <div style={{marginTop:12,padding:"10px 14px",background:pass?"#E3FCEF":"#FFEBE6",border:"1px solid "+(pass?"#57D9A3":"#FFBDAD"),borderRadius:8,fontSize:12,color:pass?"#006644":"#BF2600",lineHeight:1.6}}>{pass?"✓ 기준 충족 — 임대사업자 대출 심사 RTI 요건 통과":"⚠️ 기준 미달 — 대출금액 축소 또는 임대료 조정 필요. 기준 충족 최대대출: "+fW(gapLoan)}</div>
...
function CalcYearEnd({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>연말정산</h3>}
    ...
    {salW<=7e7&&<Inp label="월세 총액 (연간)" value={monthlyRent} onChange={sMR} suffix="만원" placeholder="예: 600" note="총급여 7천↓ 대상, 5500만↓17%·초과15%, 한도 1000만"/>}
      ...
      alertMsg={refund>=0?"🎉 환급 예상 "+fW(Math.abs(refund)):"⚠️ 추가 납부 "+fW(Math.abs(refund))}
...
function CalcMinWage({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}>
    <div>
      ...
      {isVio&&<div style={{padding:"10px 14px",background:"#FFEBE6",border:"1px solid #FFBDAD",borderRadius:10,fontSize:12,color:"#DE350B",marginTop:8}}>⚠️ 입력한 시급이 2026년 최저임금({MIN_HOURLY.toLocaleString()}원)보다 낮습니다.</div>}
      {hasPay&&<div style={{padding:"10px 14px",background:"#E3FCEF",border:"1px solid #57D9A3",borderRadius:10,fontSize:12,color:"#006644",marginTop:8}}>✅ 주 {weeklyH}시간 근무 → 주휴수당 적용 ({payH.toFixed(1)}시간/주)</div>}
    ...
    <div><RP title="예상 급여"
      ...
      alertMsg={isVio?"⚠️ 최저임금 위반: 시급 "+MIN_HOURLY+"원 미달":hasPay?"주휴수당 포함 주 "+payH.toFixed(1)+"시간 가산":"주 15시간 미만: 주휴수당 미적용"}
...
function EduSidebar({calc:calcId,eduTab,setEduTab}){
  ...
  return(
    <div className="edu-sidebar sidebar-left" style={{background:"#fff",borderRadius:16,border:`1px solid ${P.bd}`,padding:24,marginTop:16}}>
      ...
      {[{id:"rates",icon:"📊",l:"세율표·가이드"},{id:"regs",icon:<IconClip/>,l:"규정·법령"},{id:"tips",icon:<IconBulb/>,l:"절세 팁"},{id:"glossary",icon:"📖",l:"용어 사전"}].map(t=>(
...
`}</style>
    ...
    <>
      ...
      {!isMo&&<div style={{background:P.bg,padding:"48px 24px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          ...
          <div style={{fontSize:13,fontWeight:700,color:P.pri,marginBottom:10,padding:"0 4px"}}>📰 실시간 부동산 뉴스</div>
    ...
    {consultModal&&<div onClick={()=>!consultBusy&&setConsultModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:10002,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,padding:24,maxWidth:460,width:"100%",position:"relative"}}>
        ...
        <div style={{fontSize:18,fontWeight:800,color:"#172B4D",marginBottom:4}}>👨‍💼 전문가 상담 신청</div>
