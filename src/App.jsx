...
function shareKakao(title,total,sub){const text=`[생활계산기.com] ${title}\n산출 합계: ₩${Math.round(total).toLocaleString('ko-KR')}\n${sub||''}\n\n👉 직접 계산해보기: https://생활계산기.com`;if(navigator.share){navigator.share({title:title+' - 생활계산기.com',text,url:'https://xn--989a00a691bdfa717h.com'}).catch(()=>{});}else{navigator.clipboard.writeText(text).then(()=>alert('리포트 내용이 클립보드에 복사되었습니다.\n카카오톡이나 메신저에 붙여넣기 하세요.')).catch(()=>{const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);alert('리포트 내용이 복사되었습니다.');});}}
...
function RP({title,total,sub,items}){
  return(<div style={{background:"linear-gradient(180deg,#0747A6 0%,#0052CC 100%)",borderRadius:20,padding:"28px 24px",color:"#fff",position:"sticky",top:80}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:16}}>
      {[{fn:()=>generateReport(title,total,sub,items),icon:"📋",l:"리포트 보기"},{fn:()=>downloadPDF(title,total,sub,items),icon:"📄",l:"PDF 저장"},{fn:()=>shareKakao(title,total,sub),icon:"💬",l:"공유하기"}].map((b,i)=>(
