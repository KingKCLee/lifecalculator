...
function CalcComm({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <label style={{display:"flex",alignItems:"center",gap:8,fontSize:13,cursor:"pointer",marginBottom:16}}><input type="checkbox" checked={vatIncl} onChange={e=>setVatIncl(e.target.checked)} style={{width:16,height:16}}/> 부가세(10%) 포함 합계</label>
      ...
      items={[{l:"거래금액",v:fW(amt),note:tx==="rent"?"보증금+월세×"+(amt<5e7?"70":"100"):""},{l:"적용 요율",v:fP(r*100),note:useCustom?"직접 입력":rateLabel},{l:"중개보수",v:fW(fee)}].concat(mf&&amt*r>mf?[{l:"상한 적용",v:fW(mf)}]:[]).concat(ty!=="res"?[{l:"부가세 (10%)",v:fW(vat),note:vatIncl?"합계 포함":"별도"}]:[{l:"부가세",v:"해당없음 (개인거래 면세)"}]).concat([{l:"합계 (보수"+(vat>0&&vatIncl?"+VAT":"")+")  ",v:fW(total)}])}/>
