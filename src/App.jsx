...
transfer:{q:"양도소득세는 어떻게 계산되나요?",a:"부동산을 팔아 생긴 차익(양도가액-취득가액-필요경비-기본공제250만)에 6~45% 누진세율을 적용합니다. 1세대 1주택자가 2년 이상 보유(조정지역은 거주 포함) 시 비과세되나, 양도가 12억 초과분은 과세됩니다. 장기보유특별공제는 보유 연 4%+거주 연 4%(최대 80%)입니다. 다주택 중과(+20~30%p)는 2026.5.9까지 유예 중입니다.",
...
gift:{q:"증여세 공제한도는?",a:"재산을 무상으로 받으면 부과되는 국세입니다. 10년 합산 기준으로 배우자 6억, 성년자녀 5천만, 미성년자녀 2천만, 기타 친족 1천만원을 공제받습니다. 2024년부터 혼인·출산 시 1억원 추가공제가 신설되었습니다. 세율은 10~50% 5단계 누진이며, 기한 내 신고 시 3% 세액공제됩니다. 증여 취득세는 3.5%(조정지역 12%)가 별도 부과됩니다.",
...
inherit:{q:"상속세 계산 구조는?",a:"사망으로 재산이 이전될 때 부과됩니다. 기초공제 5억+일괄공제 5억으로 최소 10억이 공제되며, 배우자 상속공제는 실제 상속액의 법정상속분 내에서 5~30억까지 가능합니다. 배우자가 없으면 일괄공제 5억만 적용됩니다. 세율은 10~50%이며, 장례비용은 최대 1,500만원까지 공제. 신고기한은 사망일로부터 6개월, 연부연납 최대 10년 가능합니다.",
  rates:[["1억↓","10%","없음"],["~5억","20%","1천만"],["~10억","30%","6천만"],["~30억","40%","1.6억"],["30억↑","50%","4.6억"]],rh:["과세표준","세율","누진공제"]},
dsr:{q:"DSR 규제 기준은?",a:"연소득 대비 모든 금융권 대출의 연간 원리금 상환비율입니다. 은행 40%, 비은행 50% 이내여야 합니다. 주담대뿐 아니라 신용대출·학자금·자동차할부 등 모든 대출이 포함됩니다. 2024년부터 스트레스 DSR이 시행되어 금리 상승 시나리오(가산금리 0.38~1.5%p)가 반영됩니다.",
...
function CalcAcq(){
  const[price,sP]=useState("12500");const[own,sO]=useState("1");const[area,sA]=useState("84");const[reg,sR]=useState("spec");const[isFirst,sFirst]=useState("no");
  const pW=tW(price);let r=0.01;const n=parseInt(own),isR=reg!=="non";
  if(n===1||(n===2&&!isR)){r=pW<=6e8?0.01:pW<=9e8?Math.max(0.01,(pW*2/3e8-3)/100):0.03;}else if(n===2&&isR){r=0.08;}else if(n>=3){r=isR?0.12:0.08;}
  ...
  return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignItems:"start"}}>
    ...
    <RP title="예상 취득 비용" total={total} sub={"세율 "+fP(r*100)+" 적용"}
      items={[{l:"취득세 ("+fP(r*100)+")",v:fW(ac)},{l:"지방교육세",v:fW(ed)},{l:"농어촌특별세",v:fm>0?fW(fm):"해당없음"},{l:"인지세·증지대",v:fW(st)},{l:"생애최초 감면",v:firstDed>0?"-"+fW(firstDed):"해당없음"}]}/>
  </div>);
}
...
function CalcTrans(){
  ...
  const b=tW(bp),s=tW(sp),e=tW(ex),g=s-b-e;const hv=parseInt(hy),lv=Math.min(parseInt(ly),parseInt(hy)),nv=parseInt(hm);
  const is1h=nv===1&&hv>=2,overLimit=s>12e8;let exempt=is1h&&!overLimit,ltD=0;
...
function CalcGift(){const[rl,sRl]=useState("adult");const[p,sP]=useState("");const[isMarriage,sM]=useState("no");const g=tW(p),ds={spouse:6e8,adult:5e7,minor:2e7,other:1e7},dl={spouse:"배우자 6억",adult:"성년자녀 5천만",minor:"미성년 2천만",other:"기타 1천만"};let d=ds[rl];if(isMarriage==="yes")d+=1e8;const tb=Math.max(0,g-d),tx=pTx(tb,GB);return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🎁 증여세</h3><Sel label="수증자와의 관계" value={rl} onChange={sRl} options={[{value:"spouse",label:"배우자 (공제 6억)"},{value:"adult",label:"성년 자녀 (공제 5천만)"},{value:"minor",label:"미성년 자녀 (공제 2천만)"},{value:"other",label:"기타 친족 (공제 1천만)"}]}/><Tog label="혼인·출산 증여 해당" value={isMarriage} onChange={sM} options={[{value:"no",label:"아니오"},{value:"yes",label:"예 (+1억 추가공제)"}]}/><Inp label="증여 재산가액" value={p} onChange={sP} suffix="만원" placeholder="예: 100000" note={isMarriage==="yes"?"2024년~ 혼인·출산 시 1억 추가공제 적용":""}/></div>{g>0?<RP title="증여세" total={Math.round(tx*.97)} sub={"공제 "+fW(d)+" 적용"} items={[{l:"과세표준",v:fW(tb)},{l:"산출세액",v:fW(tx)},{l:"신고세액공제 (3%)",v:"-"+fW(tx*.03)},{l:"예상 납부세액",v:fW(tx*.97)}]}/>:<Empty icon="🎁"/>}</div>);}
