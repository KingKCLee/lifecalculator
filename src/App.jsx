...
function CalcTrans({isMo=false}){
  ...
  const shortTermRate=hv<1?0.70:hv<2?0.60:0;
  const needResidence=wasAdj==="yes";const is1h=nv===1&&hv>=2&&(!needResidence||adjLv>=2),overLimit=s>12e8;let exempt=is1h&&!overLimit&&shortTermRate===0,ltD=0;
  if(!exempt){if(nv===1&&hv>=3)ltD=Math.min(Math.min(hv,15)*.04+Math.min(adjLv,10)*.04,.80);else if(nv>1&&hv>=3)ltD=Math.min(hv*.02,.30);}
  if(surcharge>0||shortTermRate>0)ltD=0;
  let taxableGain=g;if(is1h&&overLimit){taxableGain=g*(s-12e8)/s;}
  const tg=exempt?0:Math.max(0,taxableGain*(1-ltD)-2500000);
  let baseTax,taxRateLabel;
  if(shortTermRate>0){baseTax=Math.round(tg*shortTermRate);taxRateLabel="단기보유 "+(shortTermRate*100)+"%";}
  else{baseTax=pTx(tg,IB);taxRateLabel="기본세율";}
  const surTax=surcharge>0&&shortTermRate===0?Math.round(tg*surcharge):0;const tx=baseTax+surTax;const loc=tx*.1;
  const baseItems=exempt?[{l:"양도차익",v:fW(g)},{l:"비과세 요건",v:"1주택, 2년↑, 12억↓"}]:[{l:"양도차익",v:fW(g)},{l:is1h&&overLimit?"과세 대상 차익 (12억 초과분)":"양도차익",v:is1h&&overLimit?fW(taxableGain):fW(g)},{l:"장기보유특별공제",v:(surcharge>0||shortTermRate>0)?"중과 배제":fP(ltD*100)},{l:"기본공제",v:"-₩250만"},{l:"과세표준",v:fW(tg)},{l:taxRateLabel+" 산출세액",v:fW(baseTax)}];
  const surItems=surcharge>0&&shortTermRate===0?[{l:"다주택 중과세율",v:"+"+(surcharge*100)+"%p (2026.5.10~ 시행)"},{l:"중과 가산세액",v:fW(surTax)}]:[];
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?4:8}}>
        <Sel label="보유기간" value={hy} onChange={sHy} options={[{value:"0",label:"1년 미만 (70%)"}].concat(Array.from({length:20},(_,i)=>({value:String(i+1),label:(i+1)+"년"})))}/><Sel label="거주기간" value={ly} onChange={sLy} options={Array.from({length:16},(_,i)=>({value:String(i),label:i+"년"}))}/><Sel label="주택 수" value={hm} onChange={sHm} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택+"}]}/>
      </div>
      ...
      {shortTermRate>0&&<div style={{padding:"12px 16px",background:"#FFEBE6",border:"1px solid #FFBDAD",borderRadius:10,fontSize:13,color:"#DE350B",marginTop:12,lineHeight:1.6}}>⚠️ 단기보유 중과세율 {shortTermRate*100}% 적용 중 ({hv<1?"1년 미만":"1~2년"} 보유). 장특공제·누진세 배제.</div>}
    </div>
...
function CalcCompre({isMo=false}){const[h,sH]=useState("1");const[p,sP]=useState("");const[age,sAge]=useState("0");const[hold,sHold]=useState("0");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    {n===1&&<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?8:12}}><Sel label="만 나이 (1주택자)" value={age} onChange={sAge} options={[{value:"0",label:"60세 미만"},{value:"60",label:"60~64세 (20%)"},{value:"65",label:"65~69세 (30%)"},{value:"70",label:"70세 이상 (40%)"}]}/><Sel label="보유기간 (1주택자)" value={hold} onChange={sHold} options={[{value:"0",label:"5년 미만"},{value:"5",label:"5~10년 (20%)"},{value:"10",label:"10~15년 (40%)"},{value:"15",label:"15년 이상 (50%)"}]}/></div>}
...
function CalcProp({isMo=false}){const[pt,sPt]=useState("house");const[p,sP]=useState("");const pp=tW(p),fr=pt==="house"?.6:.7,tb=pp*fr;let tx=0;if(pt==="house")tx=pTx(tb,[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]]);else if(pt==="land")tx=pTx(tb,[[5e7,.002],[1e8,.003],[Infinity,.005]]);else tx=tb*.0025;const ci=pt==="house"?Math.round(tb*.0014):0,ed=tx*.2;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏡 재산세</h3><Sel label="부동산 유형" value={pt} onChange={sPt} options={[{value:"house",label:"주택"},{value:"land",label:"토지"},{value:"com",label:"건축물 (상가 등)"}]}/><Inp label="공시가격" value={p} onChange={sP} suffix="만원" placeholder="예: 50000"/></div>{pp>0?<RP title="재산세" total={tx+ci+ed} items={[{l:"과세표준 (공시가×"+(fr*100)+"%)",v:fW(tb)},{l:"재산세",v:fW(tx)},{l:"도시지역분 (14%)",v:fW(ci)},{l:"지방교육세 (20%)",v:fW(ed)}]}/>:<Empty icon="🏡"/>}</div>);}

function CalcGift({isMo=false}){const[rl,sRl]=useState("adult");const[p,sP]=useState("");const[isMarriage,sM]=useState("no");const[priorGift,sPG]=useState("");const[skip,sSkip]=useState("no");const g=tW(p),priorW=tW(priorGift),ds={spouse:6e8,adult:5e7,minor:2e7,other:1e7};let d=ds[rl];if(isMarriage==="yes")d+=1e8;const totalGift=g+priorW;const tb=Math.max(0,totalGift-d);const priorTax=priorW>0?pTx(Math.max(0,priorW-d),GB):0;const fullTax=pTx(tb,GB);const skipRate=skip==="minor"?0.4:skip==="adult"?0.3:0;const surcharge=Math.round(fullTax*skipRate);const thisTax=Math.max(0,fullTax+surcharge-priorTax);return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🎁 증여세</h3><Sel label="수증자와의 관계" value={rl} onChange={sRl} options={[{value:"spouse",label:"배우자 (공제 6억)"},{value:"adult",label:"성년 자녀 (공제 5천만)"},{value:"minor",label:"미성년 자녀 (공제 2천만)"},{value:"other",label:"기타 친족 (공제 1천만)"}]}/><Tog label="혼인·출산 증여 해당" value={isMarriage} onChange={sM} options={[{value:"no",label:"아니오"},{value:"yes",label:"예 (+1억 추가공제)"}]}/><Sel label="세대생략 증여" value={skip} onChange={sSkip} options={[{value:"no",label:"해당 없음"},{value:"adult",label:"손자녀(성년) 30% 할증"},{value:"minor",label:"손자녀(미성년) 40% 할증"}]}/><Inp label="증여 재산가액" value={p} onChange={sP} suffix="만원" placeholder="예: 100000"/><Inp label="10년 내 기증여 금액" value={priorGift} onChange={sPG} suffix="만원" placeholder="없으면 0" note="동일인으로부터 10년 이내 증여받은 금액 합산"/></div>{g>0?<RP title="증여세" total={Math.round(thisTax*.97)} sub={"공제 "+fW(d)+" 적용"} items={[{l:"이번 증여액",v:fW(g)}].concat(priorW>0?[{l:"10년 내 기증여 합산",v:fW(priorW)}]:[]).concat([{l:"합산 과세표준",v:fW(tb)},{l:"합산 산출세액",v:fW(fullTax)}]).concat(skipRate>0?[{l:"세대생략 할증 ("+(skipRate*100)+"%)",v:"+"+fW(surcharge)}]:[]).concat(priorW>0?[{l:"기납부세액 차감",v:"-"+fW(priorTax)}]:[]).concat([{l:"신고세액공제 (3%)",v:"-"+fW(thisTax*.03)},{l:"이번 납부 증여세",v:fW(Math.round(thisTax*.97))}])}/>:<Empty icon="🎁"/>}</div>);}

function CalcInherit({isMo=false}){const[ta,sTa]=useState("");const[db,sDb]=useState("");const[fn,sFn]=useState("500");const[hasSpouse,sHS]=useState("yes");const[children,sCh]=useState("2");const t=tW(ta),d=tW(db),f=Math.min(tW(fn),15000000),net=t-d-f;const childN=parseInt(children)||0;const baseDed=5e8;let spouseDed=0;if(hasSpouse==="yes"){const spouseShare=net>0?net*1.5/(1.5+childN):0;spouseDed=Math.min(Math.max(spouseShare,5e8),30e8);}const ded=baseDed+spouseDed;const tb=Math.max(0,net-ded),tx=pTx(tb,GB);return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📜 상속세</h3><Inp label="총 상속재산가액" value={ta} onChange={sTa} suffix="만원" placeholder="예: 200000"/><Inp label="채무 (대출 등)" value={db} onChange={sDb} suffix="만원" placeholder="예: 10000"/><Inp label="장례비용" value={fn} onChange={sFn} suffix="만원" note="최대 1,500만원 공제"/><Tog label="배우자 유무" value={hasSpouse} onChange={sHS} options={[{value:"yes",label:"있음"},{value:"no",label:"없음"}]}/>{hasSpouse==="yes"&&<Sel label="자녀 수" value={children} onChange={sCh} options={[0,1,2,3,4,5].map(n=>({value:String(n),label:n+"명"}))}/>}</div>{t>0?<RP title="상속세" total={Math.round(tx*.97)} sub={"상속공제 "+fW(ded)} items={[{l:"순상속재산",v:fW(net)},{l:"일괄공제",v:fW(baseDed)}].concat(hasSpouse==="yes"?[{l:"배우자 법정상속분",v:fP(1.5/(1.5+childN)*100)},{l:"배우자 상속공제 (min 5억, max 30억)",v:fW(spouseDed)}]:[]).concat([{l:"과세표준",v:fW(tb)},{l:"산출세액",v:fW(tx)},{l:"예상 납부 (신고공제 후)",v:fW(tx*.97)}])}/>:<Empty icon="📜"/>}</div>);}
