...
function CalcAcq({isMo=false}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?12:32,alignItems:"start"}}>
    ...
    <RateTable title="주택 외 취득세율표" headers={["구분","취득세","교육세","농특세"]} rows={[["매매(토지·건물)","4%","0.4%","0.2%"],["증여","3.5%","0.3%","0.2%"],["상속","2.8%","0.16%","0.2%"],["원시취득","2.8%","0.16%","0.2%"],["농지 매매","3%","0.2%","0.2%"],["농지 자경","1.5%","0.1%","-"],["농지 상속","2.3%","0.06%","0.2%"]]}/></div>
  </div>);
}
...
/* DTI */
function CalcDTI({isMo=false}){const[inc,sInc]=useState("");const[la,sLa]=useState("");const[lr,sLr]=useState("");const[ly,sLy]=useState("30");const[ei,sEi]=useState("");const ai=tW(inc),laW=tW(la),lrV=pN(lr)/100/12,lN=parseInt(ly)*12,eiW=tW(ei);let na=0;if(laW>0&&lrV>0&&lN>0)na=laW*lrV*Math.pow(1+lrV,lN)/(Math.pow(1+lrV,lN)-1)*12;const tot=na+eiW*12,dti=ai>0?tot/ai*100:0;let st="양호",sc="#00875A";if(dti>60){st="한도 초과";sc="#DE350B";}else if(dti>50){st="주의";sc="#FF8B00";}return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📉 DTI 계산</h3>}<Inp label="연 소득" value={inc} onChange={sInc} suffix="만원" placeholder="예: 5000"/><Inp label="신규 대출금액" value={la} onChange={sLa} suffix="만원" placeholder="예: 30000"/><Inp label="대출 금리" value={lr} onChange={sLr} suffix="%" placeholder="예: 3.5"/><Sel label="대출 기간" value={ly} onChange={sLy} options={[5,10,15,20,25,30,35,40].map(y=>({value:String(y),label:y+"년"}))}/><Inp label="기존 대출 월이자 합계" value={ei} onChange={sEi} suffix="만원" placeholder="없으면 0" note="DTI는 기존 대출의 이자만 포함 (원금 미포함)"/></div><div>{ai>0&&laW>0?<div style={{background:"linear-gradient(135deg,#0747A6 0%,#0065FF 100%)",borderRadius:16,padding:"28px 24px",color:"#fff",textAlign:"center"}}><div style={{fontSize:11,letterSpacing:1.5,opacity:.8,marginBottom:16}}>DTI 진단 결과</div><div style={{fontSize:52,fontWeight:800,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{fP(dti)}</div><div style={{fontSize:16,fontWeight:600,marginTop:4,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{st}</div><div style={{fontSize:12,opacity:.7,marginTop:12}}>규제 기준: 60%</div><div style={{borderTop:"1px solid rgba(255,255,255,.2)",marginTop:16,paddingTop:12,textAlign:"left",fontSize:13}}><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>신규 연간 원리금</span><span style={{fontWeight:600}}>{fW(na)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>기존 연간 이자</span><span style={{fontWeight:600}}>{fW(eiW*12)}</span></div></div></div>:<Empty icon="📉" msg="소득과 대출 정보를 입력하세요."/>}<RateTable title="DTI 규제 비율" headers={["지역","DTI"]} rows={[["투기지역·투기과열","40%"],["조정대상지역","50%"],["기타지역","60%"]]}/></div></div>);}

/* LTV·대출한도 */
function CalcLTV({isMo=false}){const[pv,sPv]=useState("");const[a,sA]=useState("non");const[h,sH]=useState("1");const[ex,sEx]=useState("");const[first,sFirst]=useState("no");const[seomin,sSeomin]=useState("no");const pvW=tW(pv),exW=tW(ex),n=parseInt(h);
...
function CalcNetSalary({isMo=false}){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  </div>:<Empty icon="💰" msg="연봉을 입력하세요"/>}<RateTable title="4대보험 요율 (2025)" headers={["보험","근로자"]} rows={[["국민연금","4.75%"],["건강보험","3.595%"],["장기요양","건보의 12.95%"],["고용보험","0.9%"],["합계(근로자)","약 9.7%"]]}/></div></div>);}

/* 국민연금 수령액 */
function CalcPension({isMo=false}){const[age,sAge]=useState("35");const[salary,sSal]=useState("350");const[paidYears,sPY]=useState("10");const[retireAge,sRA]=useState("65");
...
/* 퇴직금 */
function CalcRetire({isMo=false}){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
...
/* 실업급여 */
function CalcUnemploy({isMo=false}){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
...
/* 자동차세 */
function CalcCarTax({isMo=false}){const[carType,sCT]=useState("normal");const[cc,sCC]=useState("2000");const[year,sYear]=useState("0");const[isElec,sElec]=useState("no");
...
/* 4대보험료 */
function CalcInsurance4({isMo=false}){const[type,sType]=useState("worker");const[salary,sSal]=useState("");const[bonus,sBonus]=useState("");
