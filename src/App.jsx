...
function CalcComm({isMo=false}){const[ty,sTy]=useState("res");const[tx,sTx]=useState("sale");const[p,sP]=useState("90000");const[dep,sDep]=useState("5000");const[mon,sMon]=useState("50");
...
/* 등기비용 */
function CalcReg({isMo=false}){const[tx,sTx]=useState("sale");const[p,sP]=useState("");const[includeAcq,sIA]=useState("yes");const pW=tW(p);const rr=tx==="sale"?0.02:tx==="inherit"?0.008:0.015;const rt=pW*rr,ed=rt*0.2;let stamp=0;if(pW>10e8)stamp=350000;else if(pW>5e8)stamp=150000;else if(pW>1e8)stamp=70000;const bondRate=pW>6e8?0.05:pW>1.6e8?0.04:0.03;const bondAmt=Math.round(pW*bondRate);const bondCost=Math.round(bondAmt*0.045);const court=15000;const acqR=tx==="sale"?0.01:tx==="gift"?0.035:0.028;const acqTax=includeAcq==="yes"?Math.round(pW*acqR):0;const acqEdu=Math.round(acqTax*0.1);let legal=pW<=1e8?160000:pW<=3e8?220000:pW<=5e8?310000:pW<=1e9?420000:600000;legal=Math.round(legal*1.1);const total=rt+ed+stamp+bondCost+court+acqTax+acqEdu+legal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📝 등기비용 통합 계산</h3><Sel label="등기 원인" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"inherit",label:"상속"},{value:"gift",label:"증여"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="취득세 포함" value={includeAcq} onChange={sIA} options={[{value:"yes",label:"포함 (전체 비용)"},{value:"no",label:"제외 (등기비용만)"}]}/></div>{pW>0?<RP title="등기비용 통합" total={total} sub={"등록면허세 "+(rr*100).toFixed(1)+"% + 취득세 "+(includeAcq==="yes"?(acqR*100).toFixed(1)+"%":"제외")} items={[].concat(includeAcq==="yes"?[{l:"취득세 ("+(acqR*100).toFixed(1)+"%)",v:fW(acqTax)},{l:"지방교육세 (취득세의 10%)",v:fW(acqEdu)}]:[]).concat([{l:"등록면허세 ("+(rr*100).toFixed(1)+"%)",v:fW(rt)},{l:"지방교육세 (등록세의 20%)",v:fW(ed)},{l:"인지세",v:fW(stamp)},{l:"국민주택채권 할인비용",v:fW(bondCost)},{l:"법무사 수수료(VAT포함)",v:fW(legal)},{l:"법원수수료 (증지대)",v:fW(court)}])}/>:<Empty icon="📝"/>}</div>);}

/* 법무사수수료 */
function CalcLegal({isMo=false}){const[tx,sTx]=useState("sale");const[p,sP]=useState("");const pW=tW(p);let bf=0;if(pW<=5e7)bf=110000;else if(pW<=1e8)bf=160000;else if(pW<=3e8)bf=220000;else if(pW<=5e8)bf=310000;else if(pW<=10e8)bf=420000;else if(pW<=20e8)bf=550000;else if(pW<=30e8)bf=660000;else bf=770000;if(tx==="inherit"||tx==="gift")bf=Math.round(bf*1.3);if(tx==="mortgage")bf=Math.round(bf*0.5);const vat=Math.round(bf*0.1);const doc=50000;const travel=30000;const total=bf+vat+doc+travel;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>⚖️ 법무사 수수료</h3><Sel label="업무 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매 (소유권이전)"},{value:"inherit",label:"상속 (약 30% 가산)"},{value:"gift",label:"증여 (약 30% 가산)"},{value:"mortgage",label:"근저당설정"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000" note="대한법무사협회 보수표 기준 (참고용)"/></div>{pW>0?<RP title="법무사 수수료" total={total} sub="실제 비용은 법무사에 따라 다를 수 있음" items={[{l:"기본 보수",v:fW(bf)},{l:"부가가치세 (10%)",v:fW(vat)},{l:"서류작성비",v:fW(doc)},{l:"교통비 등 부대비용",v:fW(travel)}]}/>:<Empty icon="⚖️"/>}</div>);}
