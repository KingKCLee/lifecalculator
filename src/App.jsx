...
function RP({title,total,sub,items}){
  return(<div style={{background:"linear-gradient(180deg,#0747A6 0%,#0052CC 100%)",borderRadius:20,padding:"28px 24px",color:"#fff",position:"sticky",top:80}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr 1fr",gap:8,marginTop:16}}>
...
function CalcAcq(){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}>
    <div>
...
function CalcTrans(){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}>
    <div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
      ...
      <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr 1fr",gap:8}}>
...
function CalcCompre(){const[h,sH]=useState("1");const[p,sP]=useState("");const[age,sAge]=useState("0");const[hold,sHold]=useState("0");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    {n===1&&<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:12}}><Sel label="만 나이 (1주택자)" value={age} onChange={sAge} options={[{value:"0",label:"60세 미만"},{value:"60",label:"60~64세 (20%)"},{value:"65",label:"65~69세 (30%)"},{value:"70",label:"70세 이상 (40%)"}]}/><Sel label="보유기간 (1주택자)" value={hold} onChange={sHold} options={[{value:"0",label:"5년 미만"},{value:"5",label:"5~10년 (20%)"},{value:"10",label:"10~15년 (40%)"},{value:"15",label:"15년 이상 (50%)"}]}/></div>}
...
function CalcIncTax(){const[incType,sIT]=useState("salary");const[gross,sGross]=useState("");const[deductions,sDed]=useState("");const[bizType,sBT]=useState("simple");const[bizRate,sBR]=useState("60");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💼 종합소득세</h3>
...
function CalcYearEnd(){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🧾 연말정산</h3>
...
function CalcNetSalary(){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💰 연봉 실수령액</h3>
    <Inp label="연봉 (세전)" value={salary} onChange={sSal} suffix="만원" placeholder="예: 5000"/>
    <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:12}}><Sel label="부양가족 (본인 포함)" value={family} onChange={sFamily} options={[1,2,3,4,5,6].map(n=>({value:String(n),label:n+"명"}))}/><Sel label="20세 이하 자녀" value={child} onChange={sChild} options={[0,1,2,3,4].map(n=>({value:String(n),label:n+"명"}))}/></div>
...
function CalcPension(){const[age,sAge]=useState("35");const[salary,sSal]=useState("350");const[paidYears,sPY]=useState("10");const[retireAge,sRA]=useState("65");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>👴 국민연금 예상수령액</h3>
    <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:12}}><Sel label="현재 나이" value={age} onChange={sAge} options={Array.from({length:45},(_,i)=>({value:String(i+20),label:(i+20)+"세"}))}/><Sel label="수령 시작 나이" value={retireAge} onChange={sRA} options={[{value:"60",label:"60세 (조기 -30%)"},{value:"62",label:"62세 (-18%)"},{value:"65",label:"65세 (정상)"},{value:"67",label:"67세 (+14.4%)"},{value:"70",label:"70세 (+36%)"}]}/></div>
...
function CalcRetire(){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💼 퇴직금 계산기</h3>
    <Inp label="최근 3개월 평균 월급 (세전)" value={monthly} onChange={sM} suffix="만원" placeholder="예: 400"/>
    <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:12}}><Inp label="근속 년수" value={years} onChange={sY} suffix="년" placeholder="예: 5"/><Sel label="추가 개월수" value={months} onChange={sMo} options={Array.from({length:12},(_,i)=>({value:String(i),label:i+"개월"}))}/></div>
...
function CalcUnemploy(){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏢 실업급여 계산기</h3>
    <Inp label="퇴직 전 1일 평균임금" value={daily} onChange={sD} suffix="원" placeholder="예: 150000" note="최근 3개월 급여총액 ÷ 근무일수"/>
    <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:12}}><Sel label="퇴직 시 나이" value={age} onChange={sAge} options={[{value:"30",label:"50세 미만"},{value:"50",label:"50세 이상"}]}/><Sel label="고용보험 가입기간" value={years} onChange={sY} options={[{value:"1",label:"1~3년 미만"},{value:"3",label:"3~5년"},{value:"5",label:"5~10년"},{value:"10",label:"10년 이상"}]}/></div>
...
function CalcCarTax(){const[carType,sCT]=useState("normal");const[cc,sCC]=useState("2000");const[year,sYear]=useState("0");const[isElec,sElec]=useState("no");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🚗 자동차세</h3>
...
function CalcInsurance4(){const[type,sType]=useState("worker");const[salary,sSal]=useState("");const[bonus,sBonus]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏥 4대보험료</h3>
  ...
  return(<div style={{position:"relative"}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
    <div style={{background:slide.bg,borderRadius:20,padding:isMo?"24px 20px":"32px",color:"#fff",minHeight:isMo?280:340,display:"flex",flexDirection:"column",justifyContent:"space-between",position:"relative",overflow:"hidden",transition:"background .5s"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:isMo?8:12,marginTop:isMo?16:20}}>
...
`}</style>
    ...
    {page==="home"?(<>
      ...
      <div style={{background:"#f8f9fc",padding:"80px 24px",maxWidth:"100%",overflow:"hidden"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:48,alignItems:"center"}}>
          <div>
    ...
    <>
      <div className="calc-grid" style={{maxWidth:1200,margin:"0 auto",padding:isMo?"16px":"32px 24px",display:"grid",gridTemplateColumns:"220px minmax(0,1fr) 280px",gap:isMo?16:24,alignItems:"start"}}>
        ...
        <div>
          ...
          {cat!=="pro"&&<div className="pro-cards" style={{display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr 1fr",gap:12}}>
    ...
    <footer style={{background:"#fff",borderTop:`1px solid ${P.bd}`,padding:"48px 24px"}}>
      <div className="footer-inner" style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:window.innerWidth<=768?"1fr":"1fr 1fr",gap:40}}>
        <div>
