...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>양도소득세 계산기</h3>}
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<TipModal title="조정대상지역 (양도)"><p>양도 시점에 조정대상지역이면 다주택 중과 대상. 단, 2022.5.10~2026.5.9 양도분은 중과 유예.</p></TipModal></label>}
      {showRealLive&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={realLive} onChange={e=>setRealLive(e.target.checked)} style={{width:18,height:18}}/> 2년 이상 거주<TipModal title="2년 이상 거주"><p>조정지역 주택은 2년 보유+2년 거주해야 1세대1주택 비과세. 비조정은 2년 보유만 필요.</p></TipModal></label>}
      {showRentBiz&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={rentBiz} onChange={e=>{setRentBiz(e.target.checked);if(!e.target.checked)setLongRentEx(false)}} style={{width:18,height:18}}/> 임대사업자<TipModal title="임대사업자"><p>등록요건·임대기간·임대료 조건 모두 만족한 경우 체크. 소득세법상 혜택이 적용됩니다.</p></TipModal></label>}
      {showLongRent&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={longRentEx} onChange={e=>setLongRentEx(e.target.checked)} style={{width:18,height:18}}/> 장기임대 조세특례</label>}
    </div>
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={unregistered} onChange={e=>setUnregistered(e.target.checked)} style={{width:18,height:18}}/> 미등기양도<TipModal title="미등기양도"><p>등기 없이 양도 시 70% 단일세율. 장기보유특별공제 적용 불가.</p></TipModal></label>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={inherited} onChange={e=>setInherited(e.target.checked)} style={{width:18,height:18}}/> 상속받은 자산<TipModal title="상속받은 자산"><p>상속받은 부동산 양도 시 세율은 피상속인 취득일부터, 장기보유특별공제는 상속개시일부터 기산합니다.</p></TipModal></label>
      {showNonBizLand&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={nonBizLand} onChange={e=>setNonBizLand(e.target.checked)} style={{width:18,height:18}}/> 비사업용토지<TipModal title="비사업용토지"><p>소유 기간 중 지목 본래 용도에 사용하지 않은 토지. 기본세율 + 10%p 추가과세.</p></TipModal></label>}
    </div>
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      {show1HouseExempt&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={is1HouseExempt} onChange={e=>setIs1HouseExempt(e.target.checked)} style={{width:18,height:18}}/> 1세대1주택 비과세 해당</label>}
      {showSangSaeng&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={sangSaeng} onChange={e=>setSangSaeng(e.target.checked)} style={{width:18,height:18}}/> 상생임대주택<TipModal title="상생임대주택"><p>임대료 5% 이내 인상 계약 임대인에 한해 조정지역 거주 2년 요건을 면제합니다. 장기보유특별공제는 보유공제만 적용되고 거주공제는 제외됩니다.</p></TipModal></label>}
      {showMixedHouse&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={mixedHouse} onChange={e=>setMixedHouse(e.target.checked)} style={{width:18,height:18}}/> 겸용주택(상가주택)<TipModal title="겸용주택"><p>2022.1.1 이후 양도분은 주택분과 상가분을 면적 비율로 분리하여 과세합니다. 주택분은 1세대1주택 비과세/장특공제, 상가분은 일반세율 적용.</p></TipModal></label>}
      {showHeavy2&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={isHeavy2} onChange={e=>setIsHeavy2(e.target.checked)} style={{width:18,height:18}}/> 조정 중과 2주택</label>}
      {showHeavy3&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={isHeavy3} onChange={e=>setIsHeavy3(e.target.checked)} style={{width:18,height:18}}/> 조정 중과 3주택</label>}
    </div>
    ...
    {result&&<div style={{marginTop:24}}>
      ...
      {inMoratoriumRender&&(own==="two"||own==="more")&&conArea&&<div style={{marginBottom:12,padding:"10px 14px",background:"#eff6ff",border:"1px solid #93c5fd",borderRadius:8,fontSize:13,color:"#1e40af",lineHeight:1.6}}>ℹ️ 다주택 중과 한시배제 (~2026.5.9): 기본세율 적용 중</div>}
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:isMo?14:13}}>
        ...
        <tbody>{result.items.map((item,i)=>{const isTot=item.l.includes("납부")||item.l.includes("합계")||item.l.includes("총 ");return(<tr key={i} style={{borderBottom:"1px solid #f1f5f9",background:isTot?"#eff6ff":i%2===0?"#fff":"#fafbfc"}}><td style={{padding:isTot?"11px 12px":"8px 12px",color:"#94a3b8",fontSize:11}}>{i+1}</td><td style={{padding:isTot?"11px 12px":"8px 12px",fontWeight:isTot?700:400,fontSize:isTot?14:13,color:isTot?"#1e40af":"#475569"}}>{item.l}</td><td style={{padding:isTot?"11px 12px":"8px 12px",textAlign:"right",fontWeight:isTot?800:600,fontSize:isTot?15:13,color:isTot?"#1e40af":item.v&&item.v.toString().includes("비과세")?"#15803d":"#1e293b"}}>{item.v}</td><td style={{padding:"8px 8px 8px 4px",fontSize:11,color:"#94a3b8"}}>{item.note||""}</td></tr>)})}</tbody>
      </table>
...
function CalcRti({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    {rentW>0&&loanW>0?<><div style={{background:"linear-gradient(135deg,#0747A6 0%,#0065FF 100%)",borderRadius:16,padding:"28px 24px",color:"#fff",textAlign:"center"}}>
...
function CalcSubscription({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <div style={{background:"linear-gradient(135deg,#0747A6 0%,#0065FF 100%)",borderRadius:16,padding:"28px 24px",color:"#fff",textAlign:"center"}}>
...
function CalcReg({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>{!isMo&&<h3 style={{fontSize:isMo?16:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>등기비용 통합 계산</h3>}<Sel label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Sel label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div><div><RP title="등기비용 통합"
