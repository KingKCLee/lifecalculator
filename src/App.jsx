...
function CalcCompre({isMo=false}){const[taxpayer,sTP]=useState("indi");const[h,sH]=useState("1");const[p,sP]=useState("");const[age,sAge]=useState("0");const[hold,sHold]=useState("0");const[prevTax,sPrev]=useState("");
  ...
  let ct=0,rateLabel="";
  if(isCorp){const r=n<=2?0.027:0.05;ct=Math.round(tb*r);rateLabel="법인 "+(r*100)+"% 단일세율";}
  else{const C1=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const CX=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.014],[50e8,.020],[94e8,.030],[Infinity,.050]];ct=Math.round(pTx(tb,n<=2?C1:CX));rateLabel=n<=2?"일반 누진세율":"3주택+ 중과 누진세율";}
  const ageV=parseInt(age),holdV=parseInt(hold);let ageCredit=0,holdCredit=0;
  if(!isCorp&&n===1){if(ageV>=70)ageCredit=.30;else if(ageV>=65)ageCredit=.25;else if(ageV>=60)ageCredit=.20;if(holdV>=15)holdCredit=.50;else if(holdV>=10)holdCredit=.40;else if(holdV>=5)holdCredit=.20;}
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    {!isCorp&&n===1&&<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?8:12}}><Sel label="만 나이" value={age} onChange={sAge} options={[{value:"0",label:"60세 미만"},{value:"60",label:"60~64세 (20%)"},{value:"65",label:"65~69세 (25%)"},{value:"70",label:"70세 이상 (30%)"}]}/><Sel label="보유기간" value={hold} onChange={sHold} options={[{value:"0",label:"5년 미만"},{value:"5",label:"5~10년 (20%)"},{value:"10",label:"10~15년 (40%)"},{value:"15",label:"15년 이상 (50%)"}]}/></div>}
...
function CalcReg({isMo=false}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📝 등기비용 통합 계산</h3><Sel label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Sel label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div>{pW>0?<RP title="등기비용 통합" total={total} sub={"취득세("+(acqR*100).toFixed(1)+"%)+채권+법무사+설정등기"} items={[{l:"▼ 취득세 합계",v:fW(acqTotal)},{l:"  취득세 ("+(acqR*100).toFixed(1)+"%)",v:fW(acqTax)},{l:"  지방교육세",v:fW(acqEdu)}].concat(acqFarm>0?[{l:"  농어촌특별세",v:fW(acqFarm)}]:[]).concat([{l:"▼ 채권할인료",v:fW(bondCost)},{l:"  매입률 "+fP(bondRate*100),v:"매입액 "+fW(bondAmt)},{l:"  할인율 4.5%",v:"실부담"},{l:"인지세",v:fW(stamp)},{l:"법원수수료",v:fW(court)},{l:"▼ 법무사 보수",v:fW(legalTotal)},{l:"  기본 보수",v:fW(legal)},{l:"  VAT (10%)",v:fW(legalVat)},{l:"  교통비·서류",v:fW(80000)}]).concat(loanW>0?[{l:"▼ 설정등기 (근저당)",v:fW(mortTotal)},{l:"  등록면허세 (0.2%)",v:fW(mortReg)},{l:"  지방교육세",v:fW(mortEdu)}]:[])}/>:<Empty icon="📝"/>}</div>);}
