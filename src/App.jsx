    ...
    <div>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div className="radio-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {[["sale","매매"],["gift","증여"],["inherit","상속"],["newbuild","원시"]].map(([v,l])=>(<button key={v} onClick={()=>sAT(v)} style={{padding:isMo?"8px 4px":"10px 4px",border:acqType===v?"2px solid #0747A6":"1.5px solid #dfe1e6",borderRadius:8,background:acqType===v?"#deebff":"#fff",color:acqType===v?"#0747A6":"#505f79",fontWeight:acqType===v?700:400,fontSize:isMo?12:13,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",textAlign:"center",minWidth:0,overflow:"hidden"}}>{l}</button>))}
        </div>
      </div>
      <div style={{marginBottom:16}}>
        ...
        <div className="radio-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {[["house","주택"],["officetel","오피스텔"],["farmLand","농지"],["building","그 외"]].map(([v,l])=>(<button key={v} onClick={()=>sRT(v)} style={{padding:isMo?"8px 4px":"10px 4px",border:realType===v?"2px solid #0747A6":"1.5px solid #dfe1e6",borderRadius:8,background:realType===v?"#deebff":"#fff",color:realType===v?"#0747A6":"#505f79",fontWeight:realType===v?700:400,fontSize:isMo?12:13,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",textAlign:"center",minWidth:0,overflow:"hidden"}}>{l}</button>))}
        </div>
      </div>
      {(realType==="house"||realType==="officetel")&&<div style={{marginBottom:16}}>
        ...
        <div className="radio-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {(isMo?[["40","40㎡↓"],["60","60㎡↓"],["85","85㎡↓"],["big","85㎡↑"]]:[["40","40㎡↓"],["60","60㎡↓"],["85","85㎡↓"],["big","85㎡↑"]]).map(([v,l])=>(<button key={v} onClick={()=>sAreaType(v)} style={{padding:isMo?"8px 4px":"10px 4px",minWidth:0,border:areaType===v?"2px solid #0747A6":"1.5px solid #dfe1e6",borderRadius:8,background:areaType===v?"#deebff":"#fff",color:areaType===v?"#0747A6":"#505f79",fontWeight:areaType===v?700:400,fontSize:isMo?13:12,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",display:"flex",alignItems:"center",justifyContent:"center",textAlign:"center",overflow:"hidden"}}>{l}</button>))}
        </div>
      </div>}
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<TipModal title="조정대상지역"><p>정부가 지정한 부동산 과열 지역입니다.</p><ul style={{paddingLeft:20}}><li>서울 전 지역 (25개 자치구)</li><li>경기 일부 (과천, 성남분당, 하남, 광명 등 12개)</li></ul><p>2주택자 취득세 8%, 3주택 이상 12% 중과가 적용됩니다.</p></TipModal></label>}
        {showMetro&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={metro} onChange={e=>{setMetro(e.target.checked);if(e.target.checked)setPopulationDecline(false)}} style={{width:18,height:18}}/> 수도권<TipModal title="수도권"><p>서울·인천·경기. 임대사업자 최초분양 감면 시 가격 한도(6억) 판단에 사용됩니다.</p></TipModal></label>}
        {showPopDecline&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={populationDecline} onChange={e=>{setPopulationDecline(e.target.checked);if(e.target.checked)setMetro(false)}} style={{width:18,height:18}}/> 인구감소지역<TipModal title="인구감소지역"><p>생애최초 감면 한도 200→300만원 확대. 수도권과 동시 선택 불가.</p></TipModal></label>}
        {showHeavyExclude&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={heavyTaxExclude} onChange={e=>setHeavyTaxExclude(e.target.checked)} style={{width:18,height:18}}/> 중과 배제<TipModal title="주택 유상거래 취득세 중과 제외"><p>지방세법 시행령 제28조의2에 따른 주택은 취득세가 중과세됩니다. 제외 요건을 확인하고 직접 체크해주셔야 합니다.</p><ul style={{paddingLeft:20}}><li>가정어린이집</li><li>노인복지주택</li><li>재개발 부지를 위해 멸실용으로 취득하는 주택</li><li>주택시공자가 공사대금으로 받은 미분양주택</li><li>저당권 실행으로 취득한 주택</li><li>국가등록문화재 주택</li><li>농어촌 주택</li><li>공시가격 1억 원 이하 주택 (재개발 구역 등 제외)</li><li>공공주택사업자(지방공사, LH 등)의 공공임대주택</li><li>주택도시기금 리츠가 환매 조건부로 취득하는 주택</li><li>사원용 주택(60㎡ 이하)</li><li>주택건설사업자가 신축한 미분양된 주택</li></ul></TipModal></label>}
        {showSpouseChild&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={spouseChildGive} onChange={e=>setSpouseChildGive(e.target.checked)} style={{width:18,height:18}}/> 1주택자 배우자·직계비속 증여<TipModal title="1주택자 배우자·직계비속 증여"><p>1세대 1주택자가 배우자·직계존비속에게 증여 시 조정지역 3억 초과여도 3.5% 일반세율 적용.</p></TipModal></label>}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:14}}>
        {showFirstOfLife&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={firstOfLife} onChange={e=>setFirstOfLife(e.target.checked)} style={{width:18,height:18}}/> 생애최초 구입<TipModal title="생애최초 주택구입 취득세 감면"><p>12억원 이하 주택에 한해 적용됩니다. (지방세특례제한법 제36조의3, 2028년 12월 31일까지)</p><p>인구감소지역에서 생애최초 주택을 취득하면 감면 한도만 300만원으로 확대됩니다.</p><ul style={{paddingLeft:20}}><li><b>취득세 200만원 이하:</b> 전액 면제</li><li><b>취득세 200만원 초과:</b> 취득세 - 200만원</li><li><b>인구감소지역, 300만원 이하:</b> 전액 면제</li><li><b>인구감소지역, 300만원 초과:</b> 취득세 - 300만원</li></ul></TipModal></label>}
        {showCultivation&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",wordBreak:"keep-all"}}><input type="checkbox" checked={cultivation} onChange={e=>setCultivation(e.target.checked)} style={{width:18,height:18}}/> 2년 이상 경작</label>}
      </div>
