...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  const moratoriumEnd=new Date('2026-05-09');const daysLeft=Math.ceil((moratoriumEnd-new Date())/(86400000));const showMoratoriumBanner=daysLeft>0&&daysLeft<=180;
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
      {showMoratoriumBanner&&<div style={{padding:"14px 16px",background:"linear-gradient(135deg,#FFF1F0,#FFEBE6)",border:"2px solid #FF5630",borderRadius:12,marginBottom:16,fontSize:13,lineHeight:1.7,color:"#BF2600"}}><div style={{fontWeight:800,fontSize:14,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>⚠ 다주택 중과 유예 2026.05.09 종료 (D-{daysLeft})</div><div>조정지역 다주택자는 <b>5월 9일까지 매도 완료</b> 시 기본세율 적용. 이후 2주택 +20%p, 3주택 +30%p 중과 + 장특공제 배제.</div><div style={{marginTop:4,fontSize:12,color:"#791F1F"}}>계약+계약금 지급 시 4~6개월 추가 유예 가능. 세무사 상담 권장.</div></div>}
      <div style={{marginBottom:16}}>
