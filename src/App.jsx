...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  const[items,setItems]=useState([{id:1,price:"",joint:false,jointRate:"50",exclude:false,aptInfo:null}]);const[showCompreLookup,setShowCompreLookup]=useState(null);
  ...
  const addItem=()=>{setItems([...items,{id:Date.now(),price:"",joint:false,jointRate:"50",exclude:false,aptInfo:null}]);markModified();};
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div style={{marginBottom:12}}>
      ...
      {items.map((it,idx)=>(<div key={it.id} style={{padding:12,border:"1px solid #dfe1e6",borderRadius:10,marginBottom:8,background:it.exclude?"#f8f9fc":"#fff"}}>
        ...
        <Inp label="공시가격" value={it.price} onChange={v=>{updateItem(it.id,{price:v});markModified();}} suffix="만원" placeholder="예: 100000" error={idx===0&&(!it.price||it.price==="0")}/>
...
function CalcProp({isMo=false,onNav=()=>{}}){
  const[pt,sPt]=useState("house");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[one,sOne]=useState("no");const[prevTax,sPT]=useState("");const[showPropLookup,setShowPropLookup]=useState(false);const[selectedPropInfo,setSelectedPropInfo]=useState(null);
  const pp=tW(p);
  let fr=0.70;
  if(pt==="house"){
    if(one==="yes"){
...
function CalcInherit({isMo=false,onNav=()=>{}}){const[ta,sTa]=useState("");const[finAsset,sFA]=useState("");const[db,sDb]=useState("");const[fn,sFn]=useState("");const[hasSpouse,sHS]=useState("yes");const[children,sCh]=useState("2");const[cohabit,sCohab]=useState("");const[farm,sFarm]=useState("");const[priorGift,sPG]=useState("");const[onTime,sOT]=useState("yes");const[showInheritLookup,setShowInheritLookup]=useState(false);const t=tW(ta),finW=tW(finAsset),d=tW(db),f=Math.min(tW(fn),15000000),priorW=tW(priorGift),cohabW=tW(cohabit),farmW=tW(farm);const gross=t+priorW;const netEstate=gross-d-f;const childN=parseInt(children)||0;const basicDed=2e8;const personalDed=childN*5e7;const basicPlusPersonal=basicDed+personalDed;const lumpSum=5e8;const mainDed=Math.max(basicPlusPersonal,lumpSum);let spouseDed=0;if(hasSpouse==="yes"){const spouseShare=netEstate>0?netEstate*1.5/(1.5+childN):0;spouseDed=Math.min(Math.max(spouseShare,5e8),30e8);}let financialDed=0;if(finW>0){if(finW<=2e7)financialDed=finW;else if(finW<=1e8)financialDed=2e7;else financialDed=Math.min(Math.round(finW*0.20),2e8);}const cohabDed=cohabW>0?Math.min(cohabW,6e8):0;const farmDed=farmW>0?Math.min(farmW,15e8):0;const totalDed=mainDed+spouseDed+financialDed+cohabDed+farmDed;const tb=Math.max(0,netEstate-totalDed);const tx=pTx(tb,GB);const rebate=onTime==="yes"?0.03:0;const finalTax=Math.round(tx*(1-rebate));return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    {!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowInheritLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showInheritLookup&&<AddressModal calcType="holding" onClose={()=>setShowInheritLookup(false)} onApplyStd={v=>{sTa(String(Math.round(v/10000)));}}/>}<Inp label="총 상속재산가액" value={ta} onChange={sTa} suffix="만원" placeholder="예: 200000" error={!ta||ta==="0"}/>
...
function CalcNetSale({isMo=false,onNav=()=>{}}){
  ...
  return(<>{showLookup&&<AddressModal onClose={()=>setShowLookup(false)} onApplyPrice={v=>{setSellP(String(Math.round(v/10000)));setShowLookup(false);}}/>}<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:32,alignItems:"start",minWidth:0}}><div>
    ...
    <Inp label="매도가격" value={sellP} onChange={setSellP} suffix="만원" placeholder="예: 100000" error={!sellP||sellP==="0"}/>
...
/* 보유세 통합 */
function CalcHoldTax({isMo=false,onNav=()=>{}}){const[h,sH]=useState("1");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[showHoldLookup,setShowHoldLookup]=useState(false);const[selectedHoldInfo,setSelectedHoldInfo]=useState(null);const pW=tW(p);const n=parseInt(h);const ptBase=pW*0.6;const PTB=[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]];const ptTax=pTx(ptBase,PTB);const ptCity=Math.round(ptBase*0.0014);const ptEdu=Math.round(ptTax*0.2);const ptTotal=ptTax+ptCity+ptEdu;const cDed=n===1?12e8:9e8;const cBase=Math.max(0,(pW-cDed)*0.6);const CTB=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const cTax=pTx(cBase,CTB);const cFarm=Math.round(cTax*0.2);const cTotal=cTax+cFarm;const grand=ptTotal+cTotal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Radio label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>공시가격 <TipModal title=""><p>매년 6.1 기준. realtyprice.kr에서 확인. 재산세: 공시가×60%</p></TipModal></div>{!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowHoldLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showHoldLookup&&<AddressModal calcType="holding" onClose={()=>setShowHoldLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));markModified();}} onApplyInfo={info=>setSelectedHoldInfo(info)}/>}{selectedHoldInfo&&!isMo&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
      ...
      </div>}<Inp label="" value={p} onChange={sP} suffix="만원" placeholder="예: 150000" error={!p||p==="0"}/></div><div><RP miss={isDefault?MI.holdtax:null} expertGuide={CALC_NAV_CONTENT.holdtax?.items} expertGuide={CALC_NAV_CONTENT.holdtax?.items} isExample={isDefault} title="연간 보유세 합계"
