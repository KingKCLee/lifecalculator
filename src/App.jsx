...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  const[items,setItems]=useState([{id:1,price:"",joint:false,jointRate:"50",exclude:false}]);const[showCompreLookup,setShowCompreLookup]=useState(false);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    {!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowCompreLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showCompreLookup&&<AddressModal calcType="holding" onClose={()=>setShowCompreLookup(false)} onApplyStd={v=>{const newItems=[...items];newItems[0]={...newItems[0],price:String(Math.round(v/10000))};setItems(newItems);markModified();}}/>}<Tog label="납세자 유형" value={taxpayer} onChange={sTP} options={[{value:"indi",label:"개인"},{value:"corp",label:"법인"}]}/>
    <hr style={{border:"none",borderTop:"1px solid #E5E7EB",margin:"16px 0"}}/>
    <div style={{marginBottom:12}}>
...
function CalcProp({isMo=false,onNav=()=>{}}){
  const[pt,sPt]=useState("house");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[one,sOne]=useState("no");const[prevTax,sPT]=useState("");const[showPropLookup,setShowPropLookup]=useState(false);
  const pp=tW(p);
  let fr=0.70;
  if(pt==="house"){
    if(one==="yes"){
...
/* 보유세 통합 */
function CalcHoldTax({isMo=false,onNav=()=>{}}){const[h,sH]=useState("1");const[p,sP]=useState("");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[showHoldLookup,setShowHoldLookup]=useState(false);const pW=tW(p);const n=parseInt(h);const ptBase=pW*0.6;const PTB=[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]];const ptTax=pTx(ptBase,PTB);const ptCity=Math.round(ptBase*0.0014);const ptEdu=Math.round(ptTax*0.2);const ptTotal=ptTax+ptCity+ptEdu;const cDed=n===1?12e8:9e8;const cBase=Math.max(0,(pW-cDed)*0.6);const CTB=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const cTax=pTx(cBase,CTB);const cFarm=Math.round(cTax*0.2);const cTotal=cTax+cFarm;const grand=ptTotal+cTotal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Radio label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><div style={{fontSize:13,color:"#374151",fontWeight:600,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>공시가격 <TipModal title=""><p>매년 6.1 기준. realtyprice.kr에서 확인. 재산세: 공시가×60%</p></TipModal></div>{!isMo&&<div style={{marginBottom:10}}><button type="button" onClick={()=>setShowHoldLookup(true)} style={{width:"100%",padding:"12px 16px",background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe",borderRadius:10,fontSize:13,fontWeight:700,color:"#1e40af",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📍 주소로 공시가격 자동조회</button></div>}{showHoldLookup&&<AddressModal calcType="holding" onClose={()=>setShowHoldLookup(false)} onApplyStd={v=>{sP(String(Math.round(v/10000)));markModified();}}/>}<Inp label="" value={p} onChange={sP} suffix="만원" placeholder="예: 150000" error={!p||p==="0"}/></div><div><RP miss={isDefault?MI.holdtax:null} expertGuide={CALC_NAV_CONTENT.holdtax?.items} expertGuide={CALC_NAV_CONTENT.holdtax?.items} isExample={isDefault} title="연간 보유세 합계"
