...
function CalcRetire({isMo=false,onNav=()=>{}}){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div style={{padding:"12px 16px",background:P.lt,borderRadius:10,fontSize:12,color:"#6b778c",lineHeight:1.6,marginTop:8}}>※ 퇴직금 = 1일 평균임금 × 30일 × (근속일수/365) / ※ 1년 이상 근무 시 수령 자격</div>
      ...
      total={severance} sub={y+"년 "+m+"개월 근속 기준"} items={[{l:"월 평균임금",v:fW(monthlyW),note:"최근 3개월 평균"},{l:"근속기간",v:y+"년 "+m+"개월 ("+totalDays+"일)"},{l:"1일 평균임금",v:fW(Math.round(monthlyW/30))},{l:"계산식",v:"월급×30일×근속일수÷365",note:"법정 공식"},{l:"퇴직금 (세전)",v:fW(severance)},{l:"퇴직소득세 (추정)",v:fW(Math.round(retireTax)),note:"간이 계산"},{l:"실수령 퇴직금 (추정)",v:fW(severance-Math.round(retireTax))}]} nextStep={<NextStep calcId="retire" onNav={onNav} isMo={isMo}/>}/></div>);}
...
function CalcUnemploy({isMo=false,onNav=()=>{}}){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div>
    ...
    <div style={{background:P.card,borderRadius:10,border:"1px solid "+P.bd,padding:"12px 14px",marginTop:12,fontSize:12}}>
      ...
      alertType="info"
      total={monthlyAmount} sub={"월 수령액 기준 · 최대 "+duration+"일"} items={[{l:"1일 평균임금",v:fmt(dailyW)+"원"},{l:"1일 실업급여액 (60%)",v:fmt(calcDaily)+"원",note:"평균임금 × 60%"},{l:"상한액",v:fmt(maxDaily)+"원",note:"2026년 기준"},{l:"하한액 (최저임금80%)",v:fmt(minDaily)+"원"},{l:"수급 기간",v:duration+"일 (약 "+(duration/30).toFixed(1)+"개월)",note:ageV+"세 "+yV+"년 가입"},{l:"월 수령액 (30일)",v:fmt(monthlyAmount)+"원"},{l:"총 수령액",v:fmt(totalAmount)+"원"}]} nextStep={<NextStep calcId="unemploy" onNav={onNav} isMo={isMo}/>}/></div>);}
