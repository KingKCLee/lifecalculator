...
function RP({title,total,sub,items,isExample=false}){
  const isMo=typeof window!=="undefined"&&window.innerWidth<=768;
  return(<div style={{background:"linear-gradient(180deg,#0747A6 0%,#0052CC 100%)",borderRadius:20,padding:"28px 24px",color:"#fff",position:isMo?"relative":"sticky",top:isMo?0:80}}>
    {isExample&&<div style={{background:"rgba(255,255,255,0.15)",borderRadius:8,padding:"6px 12px",marginBottom:12,fontSize:11,color:"rgba(255,255,255,0.9)",display:"flex",alignItems:"center",gap:6}}>📋 예시 계산 · 값을 직접 입력하면 실제 결과로 업데이트됩니다</div>}
    ...
    <div style={{display:"flex",flexWrap:"nowrap",gap:isMo?4:8,marginTop:16}}>
      {[{fn:()=>downloadPDF(title,total,sub,items),icon:"📄",l:"PDF 저장"},{fn:()=>downloadImage(title,total,sub,items),icon:"🖼",l:"사진 저장"},{fn:()=>shareKakao(title,total,sub,items),icon:"💬",l:"카카오톡"},{fn:()=>copyLink(),icon:"🔗",l:"링크 복사"}].map((b,i)=>(
        <button key={i} onClick={b.fn} style={{flex:"1 1 0",minWidth:0,padding:"10px 4px",background:"#fff",color:P.pri,border:"none",borderRadius:10,fontSize:isMo?11:12,fontWeight:700,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,transition:"all .2s",whiteSpace:"nowrap"}}
...
function CalcAcq({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?12:32,alignItems:"start"}}>
    ...
    <div><RP title="예상 취득 비용" total={total} sub={"세율 "+fP(r*100)+" 적용"}
      items={[{l:"취득세 ("+fP(r*100)+")",v:fW(ac)},{l:"지방교육세",v:fW(ed)},{l:"농어촌특별세",v:fm>0?fW(fm):"해당없음 (85㎡ 이하)"},{l:"인지세·증지대",v:fW(st)}].concat(firstDed>0?[{l:"생애최초 감면",v:"-"+fW(firstDed)}]:[])}/>
...
function CalcCompre({isMo=false,onNav=()=>{}}){const[taxpayer,sTP]=useState("indi");const[h,sH]=useState("1");const[p,sP]=useState("150000");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[age,sAge]=useState("0");const[hold,sHold]=useState("0");const[prevTax,sPrev]=useState("");
  const pp=tW(p),n=parseInt(h),isCorp=taxpayer==="corp";
  const d=isCorp?0:(n===1?12e8:9e8);const fr=0.60;const tb=Math.max(0,(pp-d)*fr);
  let ct=0,rateLabel="";
  const C1=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];// 종부세 3주택이상/법인 중과세율 (2023년 세법개정 후): 12억까지 일반과 동일, 25억 2.0%~94억↑5.0%
const CX=[[3e8,.005],[6e8,.007],[12e8,.010],[25e8,.020],[50e8,.030],[94e8,.040],[Infinity,.050]];
  ...
  if(!isCorp&&prev>0){const capRate=n<=2?1.5:3.0;const capped=Math.round(prev*capRate);if(finalCt>capped)finalCt=capped;}
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>종합부동산세 계산기</h3>}
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>공시가격 <TipModal title="공시가격"><p>매년 6월 1일 기준 공시가격 합계. 공정시장가액비율 60%를 곱한 금액이 과세표준이 됩니다.</p></TipModal></div>
    <Inp label="주택 공시가격 합계" value={p} onChange={sP} suffix="만원" placeholder="예: 150000" note={isCorp?"법인: 공제 없음":"공제금액: "+(n===1?"12억원":"9억원")}/>
...
function TipModal({title,children}){
  ...
  const openPop=e=>{block(e);if(_popoverTimer){clearTimeout(_popoverTimer);_popoverTimer=null;}const rect=btnRef.current?.getBoundingClientRect();if(!rect)return;const vw=window.innerWidth;const popW=280;let left=rect.right+8;if(left+popW>vw)left=rect.left-popW-8;let top=rect.top+rect.height/2-100;top=Math.max(8,Math.min(top,window.innerHeight-208));setPos({top,left});setShow(true);};
  ...
  return(<span onClick={block} style={{display:"inline-flex",alignItems:"center",verticalAlign:"middle",marginLeft:4,flexShrink:0,position:"relative"}}><span ref={btnRef} onMouseEnter={openPop} onMouseLeave={closePop} style={{cursor:"pointer",border:"1.5px solid #0747A6",borderRadius:"50%",width:16,height:16,minWidth:16,minHeight:16,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#0747A6",userSelect:"none",flexShrink:0,boxSizing:"border-box",lineHeight:1,fontWeight:700,transition:"all .15s"}}>?</span>{show&&createPortal(<div onMouseEnter={keepOpen} onMouseLeave={closePop} style={{position:"fixed",top:pos.top,left:pos.left,width:280,maxHeight:"50vh",overflowY:"auto",background:"#fff",borderRadius:12,boxShadow:"0 4px 24px rgba(0,0,0,0.14)",border:"1px solid #dfe1e6",padding:"14px 16px",zIndex:10000,wordBreak:"keep-all"}}><div style={{fontSize:11,fontWeight:700,color:"#0747A6",letterSpacing:.5,textTransform:"uppercase",marginBottom:8}}>{title}</div><div style={{fontSize:13,lineHeight:1.8,color:"#344563"}}>{children}</div></div>,document.body)}</span>);
}
...
/* DTI */
function CalcDTI({isMo=false,onNav=()=>{}}){const[inc,sInc]=useState("");const[la,sLa]=useState("");const[lr,sLr]=useState("");const[ly,sLy]=useState("30");const[ei,sEi]=useState("");const ai=tW(inc),laW=tW(la),lrV=pN(lr)/100/12,lN=parseInt(ly)*12,eiW=tW(ei);let na=0;if(laW>0&&lrV>0&&lN>0)na=laW*lrV*Math.pow(1+lrV,lN)/(Math.pow(1+lrV,lN)-1)*12;const tot=na+eiW*12,dti=ai>0?tot/ai*100:0;let st="양호",sc="#00875A";if(dti>60){st="한도 초과";sc="#DE350B";}else if(dti>50){st="주의";sc="#FF8B00";}return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>DTI 계산기</h3>}<Inp label="연 소득" value={inc} onChange={sInc} suffix="만원" placeholder="예: 5000"/><Inp label="신규 대출금액" value={la} onChange={sLa} suffix="만원" placeholder="예: 30000"/><Inp label="대출 금리" value={lr} onChange={sLr} suffix="%" placeholder="예: 3.5"/><Sel label="대출 기간" value={ly} onChange={sLy} options={[5,10,15,20,25,30,35,40].map(y=>({value:String(y),label:y+"년"}))}/><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>DTI vs DSR <TipModal title="DTI와 DSR의 차이"><p><b>DTI:</b> 신규 원리금 + 기존 이자만 포함</p><p><b>DSR:</b> 모든 대출 원금+이자 포함 (더 엄격)</p></TipModal></div><Inp label="기존 대출 월이자 합계" value={ei} onChange={sEi} suffix="만원" placeholder="없으면 0" note="DTI는 기존 대출의 이자만 포함 (원금 미포함)"/></div><div>{ai>0&&laW>0?<div style={{background:"linear-gradient(135deg,#0747A6 0%,#0065FF 100%)",borderRadius:16,padding:"28px 24px",color:"#fff",textAlign:"center"}}><div style={{fontSize:11,letterSpacing:1.5,opacity:.8,marginBottom:16}}>DTI 진단 결과</div><div style={{fontSize:52,fontWeight:800,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{fP(dti)}</div><div style={{fontSize:16,fontWeight:600,marginTop:4,color:sc==="#00875A"?"#57D9A3":sc==="#FF8B00"?"#FFC400":"#FF5630"}}>{st}</div><div style={{fontSize:12,opacity:.7,marginTop:12}}>규제 기준: 60%</div><div style={{borderTop:"1px solid rgba(255,255,255,.2)",marginTop:16,paddingTop:12,textAlign:"left",fontSize:13}}><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>신규 연간 원리금</span><span style={{fontWeight:600}}>{fW(na)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",opacity:.85}}><span>기존 연간 이자</span><span style={{fontWeight:600}}>{fW(eiW*12)}</span></div></div></div>:<Empty icon="📉" msg="소득과 대출 정보를 입력하세요."/>}<NextStep calcId="dti" onNav={onNav} isMo={isMo}/><RateTable title="DTI 규제 비율" headers={["지역","DTI"]} rows={[["투기지역·투기과열","40%"],["조정대상지역","50%"],["기타지역","60%"]]}/></div></div>);}

/* LTV·대출한도 */
function CalcLTV({isMo=false,onNav=()=>{}}){const[pv,sPv]=useState("");const[a,sA]=useState("non");const[h,sH]=useState("1");const[ex,sEx]=useState("");const[first,sFirst]=useState("no");const[seomin,sSeomin]=useState("no");const pvW=tW(pv),exW=tW(ex),n=parseInt(h);
// 2026년 LTV 규제비율 (금융위원회 은행권 기준)
// 생애최초: 비규제 80%, 규제지역(조정/투기) 70% (디딤돌 특례는 별도 80%)
// 서민실수요자 우대: 투기과열 60%, 조정 70%
let ltv=0.70;
...
function CalcReg({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📝 등기비용 통합 계산</h3>}<Sel label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Sel label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div><div>{pW>0?<RP title="등기비용 통합" total={total} sub={"취득세("+(acqR*100).toFixed(1)+"%)+채권+법무사+설정등기"} items={[{l:"▼ 취득세 합계",v:fW(acqTotal)},{l:"  취득세 ("+(acqR*100).toFixed(1)+"%)",v:fW(acqTax)},{l:"  지방교육세",v:fW(acqEdu)}].concat(acqFarm>0?[{l:"  농어촌특별세",v:fW(acqFarm)}]:[]).concat([{l:"▼ 채권할인료",v:fW(bondCost)},{l:"  매입률 "+fP(bondRate*100),v:"매입액 "+fW(bondAmt)},{l:"  할인율 4.5%",v:"실부담"},{l:"인지세",v:fW(stamp)},{l:"법원수수료",v:fW(court)},{l:"▼ 법무사 보수",v:fW(legalTotal)},{l:"  기본 보수",v:fW(legal)},{l:"  VAT (10%)",v:fW(legalVat)},{l:"  교통비·서류",v:fW(80000)}]).concat(loanW>0?[{l:"▼ 설정등기 (근저당)",v:fW(mortTotal)},{l:"  등록면허세 (0.2%)",v:fW(mortReg)},{l:"  지방교육세",v:fW(mortEdu)}]:[])}/>:<Empty icon="📝"/>}<NextStep calcId="registration" onNav={onNav} isMo={isMo}/><RateTable title="등기비용 구성" headers={["항목","내용"]} rows={[["등록면허세","취득세의 일부"],["지방교육세","등록세 20%"],["국민주택채권","매입가의 일정%"],["인지세","1~35만"],["법무사수수료","50~100만"]]}/></div></div>);}
...
function CalcIncTax({isMo=false,onNav=()=>{}}){const[incType,sIT]=useState("salary");const[gross,sGross]=useState("");const[deductions,sDed]=useState("");const[bizType,sBT]=useState("simple");const[bizRate,sBR]=useState("60");
  ...
  </div><div>{grossW>0?<RP title="종합소득세" total={Math.max(0,toPay)} sub={incType==="salary"?"근로소득 기준 간이세액":incType==="freelance"?"기납부 3.3% 차감 후":"사업소득 기준"} items={[{l:incType==="salary"?"총급여":"총수입금액",v:fW(grossW)},{l:incType==="salary"?"근로소득공제":"필요경비",v:fW(grossW-taxableIncome)},{l:"소득금액",v:fW(taxableIncome)},{l:"기본공제 (본인+가족 "+(1+spouseN+parentsN+childrenN)+"명)",v:"-"+fW(basicDed)}].concat(seniorN>0?[{l:"경로우대공제 ("+seniorN+"명)",v:"-"+fW(seniorDed)}]:[]).concat(disabledN>0?[{l:"장애인공제 ("+disabledN+"명)",v:"-"+fW(disabledDed)}]:[]).concat([{l:"인적공제 합계",v:"-"+fW(personalDed)},{l:"과세표준",v:fW(finalTaxable)},{l:"산출세액",v:fW(tax)},{l:"세액공제 (표준+자녀)",v:"-"+fW(stdDed+childCredit)},{l:"소득세",v:fW(finalTax)},{l:"지방소득세 (10%)",v:fW(finalTax*.1)}]).concat(incType==="freelance"?[{l:"기납부세액 (3.3%)",v:"-"+fW(prepaid)},{l:toPay>=0?"추가 납부":"환급 예상",v:fW(Math.abs(toPay))}]:[{l:"합계 (소득세+지방세)",v:fW(totalTax)}])}/>:<Empty icon="💼" msg="소득 금액을 입력하세요"/>}<RequiredGuide items={[{label:"월 급여 또는 평균임금",filled:false},{label:"근속기간 (1년 이상)",filled:false}]}/><NextStep calcId="inctax" onNav={onNav} isMo={isMo}/><RateTable title="종합소득세율표" headers={["과세표준","세율","누진공제"]} rows={[["1,400만 이하","6%","-"],["5,000만 이하","15%","126만"],["8,800만 이하","24%","576만"],["1.5억 이하","35%","1,544만"],["3억 이하","38%","1,994만"],["5억 이하","40%","2,594만"],["10억 이하","42%","3,594만"],["10억 초과","45%","6,594만"]]}/></div></div>);}
...
function CalcYearEnd({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  </div><div>{salW<=0&&<RequiredGuide items={[{label:"연봉 (세전)",filled:salW>0}]}/>}{salW>0?<RP title={refund>=0?"예상 환급액":"추가 납부 예상"} total={Math.abs(refund)} sub={refund>=0?"돌려받는 금액":"더 내야 할 금액"} items={[{l:"총급여",v:fW(salW)},{l:"근로소득공제",v:"-"+fW(workDed)},{l:"기본공제 (본인+가족 "+(1+spouseN+parentsN+childN)+"명)",v:"-"+fW(basicDed)}].concat(seniorN>0?[{l:"경로우대공제 ("+seniorN+"명)",v:"-"+fW(seniorDed)}]:[]).concat(disabledN>0?[{l:"장애인공제 ("+disabledN+"명)",v:"-"+fW(disabledDed)}]:[]).concat([{l:"인적공제 합계",v:"-"+fW(personalDed)},{l:"4대보험료 공제",v:"-"+fW(socialDed)},{l:"신용카드 소득공제",v:"-"+fW(cardDed)},{l:"과세표준",v:fW(taxableIncome)},{l:"산출세액",v:fW(tax)}]).concat(rentCredit>0?[{l:"월세 세액공제",v:"-"+fW(rentCredit)}]:[]).concat([{l:"세액공제 합계",v:"-"+fW(totalCredit)},{l:"결정세액",v:fW(finalTax)},{l:"지방소득세",v:fW(localTax)},{l:"기납부세액 (추정)",v:fW(prepaid)},{l:refund>=0?"예상 환급":"추가 납부",v:fW(Math.abs(refund))}])}/>:<Empty icon="🧾" msg="총급여를 입력하세요"/>}<RequiredGuide items={[{label:"연간 총급여액",filled:false}]}/><NextStep calcId="yearend" onNav={onNav} isMo={isMo}/><RateTable title="주요 공제 한도" headers={["항목","한도"]} rows={[["신용카드","7천↓300만 / 1.2억↓250만 / 1.2억↑200만"],["의료비","총급여 3% 초과분 15%"],["교육비","초중고 300만 / 대학 900만"],["기부금","법정 전액 / 지정 10~30%"],["연금저축","연 600만"],["IRP 합산","연 900만"],["월세(총급여 7천↓)","연 1000만, 5500↓17%·초과15%"]]}/></div></div>);}
...
function CalcMinWage({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    ...
    <div>{h>0&&d>0?<RP title="예상 급여" total={monthly} sub={"시급 "+hourly.toLocaleString()+"원 기준"} items={[{l:"시급",v:hourly.toLocaleString()+"원"},{l:"일급 ("+h+"시간)",v:daily.toLocaleString()+"원"},{l:"주급 (주휴수당 "+(hasPay?"포함":"미포함")+")",v:Math.round(hourly*totalWH).toLocaleString()+"원"},{l:"월급 (월 "+w+"주)",v:fW(monthly)},{l:"연봉 (월급×12)",v:fW(yearly)}]}/>:<Empty icon="💰" msg="근무시간을 입력하세요"/>}
