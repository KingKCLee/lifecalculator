...
function RP({title,total,sub,items,isExample=false,deadline,deadlineLink,deadlineLinkLabel,alertMsg,alertType="info"}){
  const isMo=typeof window!=="undefined"&&window.innerWidth<=768;
  const alertColors={success:{bg:"#f0fdf4",border:"#86efac",text:"#14532d",icon:"✓"},warning:{bg:"#fefce8",border:"#fde047",text:"#713f12",icon:"!"},danger:{bg:"#fef2f2",border:"#fca5a5",text:"#7f1d1d",icon:"✕"},info:{bg:"#eff6ff",border:"#93c5fd",text:"#1e3a8a",icon:"ℹ"}};
  const ac=alertColors[alertType]||alertColors.info;
  ...
  const isPositive=(v)=>typeof v==="string"&&(v.includes("비과세")||v.includes("✅")||v.includes("✓")||v.startsWith("-₩")||v.startsWith("-"));
  return(
  <div style={{background:"#fff",borderRadius:16,border:"1px solid #e2e8f0",overflow:"hidden",position:isMo?"relative":"sticky",top:isMo?0:80,boxShadow:"0 4px 20px rgba(0,0,0,.06)"}}>
    {/* 헤더 */}
    <div style={{background:"linear-gradient(135deg,#0747A6 0%,#1a66d4 100%)",padding:"18px 20px 16px"}}>
      {isExample&&<div style={{background:"rgba(255,255,255,0.18)",borderRadius:6,padding:"4px 10px",marginBottom:10,fontSize:11,color:"rgba(255,255,255,.9)",display:"inline-flex",alignItems:"center",gap:5}}>📋 예시값 · 직접 입력하면 즉시 업데이트</div>}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase",color:"rgba(255,255,255,.65)",marginBottom:4}}>{title}</div>
          <div style={{fontSize:34,fontWeight:800,color:"#fff",lineHeight:1.1}}>{fW(total)}</div>
        </div>
        {sub&&<div style={{background:"rgba(255,255,255,.15)",borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:600,color:"#fff",whiteSpace:"nowrap",marginTop:2}}>{sub}</div>}
      </div>
    </div>
    {/* 경고/안내 */}
    {alertMsg&&<div style={{padding:"8px 14px",background:ac.bg,borderBottom:`1px solid ${ac.border}`,fontSize:12,color:ac.text,display:"flex",gap:7}}><span style={{flexShrink:0,fontWeight:700}}>{ac.icon}</span><span>{alertMsg}</span></div>}
    {/* 단계별 계산과정 테이블 */}
    <table style={{width:"100%",borderCollapse:"collapse"}}>
      <tbody>
        {items.map((it,i)=>{
          const total_row=isTotal(it.l);
          const sub_row=isSub(it.l);
          const green_row=typeof it.v==="string"&&(it.v.includes("비과세")||it.v.includes("없음")||it.l.includes("감면")||(it.v.startsWith("-")&&!it.v.startsWith("-₩")));
          return(<tr key={i} style={{borderBottom:i<items.length-1?"1px solid #f1f5f9":"none",background:total_row?"#eff6ff":i%2===0?"#fff":"#fafbfc"}}>
            <td style={{padding:total_row?"12px 16px":"9px 16px",fontSize:total_row?14:12,fontWeight:total_row?700:400,color:sub_row?"#94a3b8":total_row?"#1e40af":"#475569",paddingLeft:sub_row?28:16}}>{it.l}</td>
            <td style={{padding:total_row?"12px 16px":"9px 16px",textAlign:"right",fontWeight:total_row?800:600,fontSize:total_row?15:13,color:total_row?"#1e40af":green_row?"#15803d":it.v&&it.v.toString().startsWith("+")||it.l.includes("할증")?"#dc2626":"#1e293b",whiteSpace:"nowrap"}}>{it.v}</td>
            {it.note&&<td style={{padding:"9px 12px 9px 4px",fontSize:11,color:"#94a3b8",whiteSpace:"nowrap"}}>{it.note}</td>}
          </tr>);
        })}
      </tbody>
    </table>
    {/* 신고기한/안내 */}
    {deadline&&<div style={{padding:"9px 16px",background:"#f8fafc",borderTop:"1px solid #e2e8f0",fontSize:12,color:"#64748b",display:"flex",alignItems:"center",gap:6}}>
      <span>📅</span><span>{deadline}</span>
      {deadlineLink&&<a href={deadlineLink} target="_blank" rel="noopener noreferrer" style={{marginLeft:"auto",color:"#2563eb",fontWeight:600,textDecoration:"none",fontSize:11}}>{deadlineLinkLabel||"바로가기 →"}</a>}
    </div>}
    {/* 액션 버튼 */}
    <div style={{display:"flex",gap:0,borderTop:"1px solid #e2e8f0"}}>
      {[{fn:()=>downloadPDF(title,total,sub,items),icon:"📄",l:"PDF"},{fn:()=>downloadImage(title,total,sub,items),icon:"🖼",l:"이미지"},{fn:()=>shareKakao(title,total,sub,items),icon:"💬",l:"카카오"},{fn:()=>copyLink(),icon:"🔗",l:"링크"}].map((b,i,arr)=>(
        <button key={i} onClick={b.fn} style={{flex:1,padding:"11px 4px",background:"#fff",color:"#475569",border:"none",borderRight:i<arr.length-1?"1px solid #e2e8f0":"none",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,transition:"background .15s"}}
          onMouseEnter={e=>{e.currentTarget.style.background="#f8fafc"}} onMouseLeave={e=>{e.currentTarget.style.background="#fff"}}>
          <span style={{fontSize:16}}>{b.icon}</span>{b.l}
        </button>))}
    </div>
    <div style={{padding:"8px 16px",fontSize:10,color:"#94a3b8",lineHeight:1.5,borderTop:"1px solid #f1f5f9"}}>※ 본 계산은 참고용이며 법적 효력이 없습니다. 실제 세금·수수료는 전문가에게 확인하세요. (v2026.04.06)</div>
  </div>);
}
...
function StepsGuide({steps}){const filled=steps.filter(s=>s.filled).length;if(filled>=steps.length)return null;return(<div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px",marginBottom:12}}><div style={{fontSize:12,fontWeight:600,color:"#64748b",marginBottom:8}}>📝 입력 단계 ({filled}/{steps.length})</div>{steps.map((s,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0",fontSize:13}}><span style={{width:18,height:18,borderRadius:"50%",background:s.filled?"#22c55e":"#e2e8f0",color:s.filled?"#fff":"#94a3b8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,flexShrink:0,fontWeight:700}}>{s.filled?"✓":i+1}</span><span style={{color:s.filled?"#94a3b8":"#1e293b",textDecoration:s.filled?"line-through":""}}>{s.label}</span></div>))}</div>);}
...
function CalcTrans({isMo=false,onNav=()=>{}}){
  ...
  useEffect(()=>{if(tW(buyAmt)&&tW(sellAmt)&&buyDate.length>=8&&sellDate.length>=8)calculate();else setResult(null);},[assetType,own,baseDeduct,jointOwn,jointRate,conArea,realLive,liveYear,rentBiz,longRentEx,unregistered,inherited,nonBizLand,is1HouseExempt,isHeavy2,isHeavy3,buyAmt,sellAmt,costTotal,buyDate,sellDate,inheritBuyDate,sangSaeng,mixedHouse,houseArea,shopArea,jointSimple]);
  function calculate(){
  ...
  return(<div><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>양도소득세 계산기</h3>}
...
function CalcCompre({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>종합부동산세 계산기</h3>}
  ...
  </div><div>{pp>0?<RP isExample={isDefault} title="종합부동산세"
      deadline="납부기한: 매년 12월 1~15일 / 250만원 초과 시 분납 가능"
      deadlineLink="https://hometax.go.kr" deadlineLinkLabel="홈택스 신고 →" total={total} sub={rateLabel+" ("+calcYear+"년 기준)"} items={rpItems}/>:<Empty icon="🏘️"/>}<NextStep calcId="compre" onNav={onNav} isMo={isMo}/><RateTable title="종부세율표 (일반·개인)" headers={["과세표준","세율"]} rows={[["3억 이하","0.5%"],["6억 이하","0.7%"],["12억 이하","1.0%"],["25억 이하","1.3%"],["50억 이하","1.5%"],["94억 이하","2.0%"],["94억 초과","2.7%"]]}/></div></div>);}
...
function CalcComm({isMo=false,onNav=()=>{}}){const[ty,sTy]=useState("res");const[tx,sTx]=useState("sale");const[p,sP]=useState("90000");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[dep,sDep]=useState("5000");const[mon,sMon]=useState("50");const[mult,sMult]=useState("100");const[showRateTable,setShowRateTable]=useState(false);
  ...
  let r=0,mf=Infinity;
  if(ty==="res"){
    if(tx==="sale"){if(pW<5e7){r=.006;mf=250000;}else if(pW<2e8){r=.005;mf=800000;}else if(pW<6e8)r=.004;else if(pW<9e8)r=.005;else if(pW<12e8)r=.006;else if(pW<15e8)r=.007;else r=.009;}
...
function CalcConvert({isMo=false,onNav=()=>{}}){const[type,sType]=useState("m2j");const[deposit,sDep]=useState("");const[monthly,sMon]=useState("");const[rate,sRate]=useState("5");const dep=tW(deposit),mon=tW(monthly),rt=pN(rate)/100;let result=0;if(type==="m2j"){result=rt>0?mon*12/rt+dep:0;}else{result=dep>0&&rt>0?(dep*rt)/12:0;}return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>전월세전환 계산기</h3>}<Tog label="전환 방향" value={type} onChange={sType} options={[{value:"m2j",label:"월세 → 전세"},{value:"j2m",label:"전세 → 월세"}]}/>{type==="m2j"?<><Inp label="보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 5000"/><Inp label="월세" value={monthly} onChange={sMon} suffix="만원" placeholder="예: 80"/></>:<Inp label="전세보증금" value={deposit} onChange={sDep} suffix="만원" placeholder="예: 30000"/>}<div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>전환율 <TipModal title="전월세 전환율"><p>법정 상한: 기준금리(3%)+2% = 5.0% (2026년). 임대인이 초과 요구 불가.</p></TipModal></div><Inp label="전환율 (연 %)" value={rate} onChange={sRate} suffix="%" note="2025 법정 전환율 상한: 기준금리+2%"/></div>{(dep>0||mon>0)?<RP title={type==="m2j"?"전세 환산금액":"월세 환산금액"}
      ...
      total={result} sub={"전환율 "+rate+"% 적용"} items={type==="m2j"?[{l:"기존 보증금",v:fW(dep)},{l:"월세",v:fW(mon),note:"월세×12÷전환율"},{l:"월세 환산 보증금",v:fW(mon*12/(rt||0.01))},{l:"전세 환산 합계",v:fW(result)},{l:"법정 상한 전환율",v:"5.0%",note:"기준금리+2%"}]:[{l:"전세보증금",v:fW(dep)},{l:"환산 월세",v:fW(result)},{l:"연 환산 임대료",v:fW(result*12)},{l:"법정 상한 전환율",v:"5.0%",note:"기준금리+2%"}]}/>:<Empty icon="🔄"/>}<NextStep calcId="convert" onNav={onNav} isMo={isMo}/></div>);}
...
function CalcReg({isMo=false,onNav=()=>{}}){const[tx,sTx]=useState("sale");const[propType,sPT]=useState("house");const[p,sP]=useState("");const[loan,sLoan]=useState("");const pW=tW(p),loanW=tW(loan);
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📝 등기비용 통합 계산</h3>}<Sel label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"gift",label:"증여"},{value:"inherit",label:"상속"}]}/><Sel label="부동산 유형" value={propType} onChange={sPT} options={[{value:"house",label:"주택"},{value:"land",label:"토지·상가·건물"}]}/><Inp label="부동산 가액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Inp label="대출금액 (설정등기용)" value={loan} onChange={sLoan} suffix="만원" placeholder="없으면 0" note="채권최고액 = 대출액 × 120%"/></div><div>{pW>0?<RP title="등기비용 통합"
...
function CalcJoint({isMo=false,onNav=()=>{}}){const[p,sP]=useState("150000");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const[r,sR]=useState("50");const pW=tW(p),rv=pN(r)/100;const sB=Math.max(0,(pW-12e8)*0.6);const j1B=Math.max(0,(pW*rv-9e8*rv)*0.6);const j2B=Math.max(0,(pW*(1-rv)-9e8*(1-rv))*0.6);const C2=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const sT=pTx(sB,C2),jT=pTx(j1B,C2)+pTx(j2B,C2),sv=sT-jT;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>공동명의 계산기</h3>}<div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>공동명의 <TipModal title="공동명의 절세"><p>종부세: 단독 12억 vs 공동 각9억(합18억). 양도세: 각 250만 공제. 고령자·장기보유 공제 불가.</p></TipModal></div><Inp label="부동산 공시가격" value={p} onChange={sP} suffix="만원" placeholder="예: 150000"/><div style={{marginBottom:16}}><label style={{display:"block",fontSize:12,fontWeight:600,color:P.mt,marginBottom:6}}>{"지분 비율 — 본인 "+r+"% : 배우자 "+(100-parseInt(r))+"%"}</label><input type="range" min="10" max="90" step="10" value={r} onChange={e=>sR(e.target.value)} style={{width:"100%",accentColor:P.pri}}/><div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:P.mt,marginTop:4}}><span>본인 10%</span><span>본인 90%</span></div></div><div style={{padding:"14px 16px",background:P.lt,borderRadius:10,fontSize:13,color:"#4a5568",lineHeight:1.6}}><b>단독명의:</b> 1주택 공제 12억, 고령자·장기보유 공제 가능<br/><b>공동명의:</b> 각 9억 공제 (합 18억), 고령자·장기보유 공제 불가</div></div>{pW>0?<RP isExample={isDefault} title={sv>0?"공동명의가 유리":"단독명의가 유리"}
      ...
      total={Math.abs(sv)} sub={"절세액 기준 비교"} items={[{l:"단독명의 (1주택 12억 공제)",v:fW(sT),note:sv<=0?"✓ 유리":""},{l:"공동명의 본인 ("+r+"%)",v:fW(pTx(j1B,C2))},{l:"공동명의 배우자 ("+(100-parseInt(r))+"%)",v:fW(pTx(j2B,C2))},{l:"공동명의 종부세 합계",v:fW(jT),note:sv>0?"✓ 유리":""},{l:"차이 (절세액)",v:fW(Math.abs(sv))},{l:"본인 지분 금액",v:fW(pW*rv)},{l:"배우자 지분 금액",v:fW(pW*(1-rv))}]}/>:<Empty icon="👥"/>}<NextStep calcId="joint" onNav={onNav} isMo={isMo}/></div>);}
...
function CalcHoldTax({isMo=false,onNav=()=>{}}){const[h,sH]=useState("1");const[p,sP]=useState("100000");const[isDefault,setIsDefault]=useState(true);const markModified=()=>setIsDefault(false);const pW=tW(p);const n=parseInt(h);const ptBase=pW*0.6;const PTB=[[6e7,.001],[1.5e8,.0015],[3e8,.0025],[Infinity,.004]];const ptTax=pTx(ptBase,PTB);const ptCity=Math.round(ptTax*0.14);const ptEdu=Math.round(ptTax*0.2);const ptTotal=ptTax+ptCity+ptEdu;const cDed=n===1?12e8:9e8;const cBase=Math.max(0,(pW-cDed)*0.6);const CTB=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const cTax=pTx(cBase,CTB);const cFarm=Math.round(cTax*0.2);const cTotal=cTax+cFarm;const grand=ptTotal+cTotal;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>보유세 계산기</h3>}<Radio label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>공시가격 <TipModal title="공시가격 (보유세)"><p>매년 6.1 기준. realtyprice.kr에서 확인. 재산세: 공시가×60%</p></TipModal></div><Inp label="주택 공시가격" value={p} onChange={sP} suffix="만원" placeholder="예: 150000"/></div><div>{pW>0?<RP isExample={isDefault} title="연간 보유세 합계"
      ...
      total={grand} sub={"공시가 "+fW(pW)+" 기준 / 재산세+종부세"} items={[{l:"▼ 재산세 (7·9월 납부)",v:fW(ptTotal),note:"지방세"},{l:"  과세표준 (공정가 60%)",v:fW(ptBase)},{l:"  재산세 본세",v:fW(ptTax)},{l:"  도시지역분 (0.14%)",v:fW(ptCity)},{l:"  지방교육세 (20%)",v:fW(ptEdu)},{l:"▼ 종합부동산세 (12월 납부)",v:fW(cTotal),note:"국세"},{l:"  공제금액",v:fW(cDed)},{l:"  과세표준 (공정가 60%)",v:fW(cBase)},{l:"  종부세",v:fW(cTax)},{l:"  농어촌특별세 (20%)",v:fW(cFarm)},{l:"연간 보유세 총합",v:fW(grand),note:"재산세+종부세"}]}/>:<Empty icon="🏛️"/>}<NextStep calcId="holdtax" onNav={onNav} isMo={isMo}/><RateTable title="보유세 구성" headers={["세목","기준","세율"]} rows={[["재산세","공시가×60%(공정가)","0.1~0.4%"],["종부세","9억(1주택12억) 초과분","0.5~2.7%"],["농특세","종부세의 20%","-"],["지방교육세","재산세의 20%","-"]]}/></div></div>);}
...
function CalcStamp({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>인지세 계산기</h3>}<div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>인지세 <TipModal title="인지세"><p>매매계약서 작성 시 부과. 매도·매수인 각 50% 부담. 1억↓ 비과세.</p></TipModal></div><Inp label="거래금액" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer",marginTop:8}}><input type="checkbox" checked={electronic} onChange={e=>setElectronic(e.target.checked)}/> 전자문서 인지세 (70% 할인)</label><div style={{padding:"14px 16px",background:P.lt,borderRadius:10,fontSize:13,color:"#4a5568",lineHeight:1.6,marginTop:8}}>• 1억 이하: 비과세<br/>• 1억~10억: 15만원<br/>• 10억~30억: 35만원<br/>• 30억 초과: 50만원<br/>※ 전자계약 시 70% 할인</div></div>{pW>0?<RP title="인지세"
      ...
      total={stamp} sub={stamp===0?"비과세 구간":"매도·매수인 각 50% 부담"} items={[{l:"거래금액",v:fW(pW)},{l:"기본 인지세액",v:fW(baseStamp)}].concat(electronic?[{l:"전자문서 할인 (70%)",v:"-"+fW(baseStamp-stamp)}]:[]).concat([{l:"납부 인지세",v:fW(stamp)},{l:"매도인 부담",v:fW(half)},{l:"매수인 부담",v:fW(half)}])}/>:<Empty icon="📃"/>}<NextStep calcId="stamp" onNav={onNav} isMo={isMo}/></div>);}
...
function CalcRemodel({isMo=false,onNav=()=>{}}){const[cv,sCv]=useState("");const[cost,sCost]=useState("");const[ev,sEv]=useState("");const cvW=tW(cv),costW=tW(cost),evW=tW(ev);const profit=evW-cvW-costW;const roi=costW>0?profit/costW*100:0;const totalRoi=(cvW+costW)>0?profit/(cvW+costW)*100:0;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏠 리모델링 수익 분석</h3>}<Inp label="기존 평가액 (현재 시세)" value={cv} onChange={sCv} suffix="만원" placeholder="예: 80000"/><Inp label="리모델링 분담금" value={cost} onChange={sCost} suffix="만원" placeholder="예: 15000"/><Inp label="리모델링 후 예상가" value={ev} onChange={sEv} suffix="만원" placeholder="예: 120000"/></div>{cvW>0&&costW>0&&evW>0?<RP title={profit>=0?"예상 수익":"예상 손실"}
      ...
      total={Math.abs(profit)} sub={"분담금 대비 수익률 "+fP(roi)} items={[{l:"기존 평가액",v:fW(cvW)},{l:"리모델링 분담금",v:fW(costW)},{l:"총 투자액",v:fW(cvW+costW)},{l:"리모델링 후 예상가",v:fW(evW)},{l:"예상 수익",v:fW(profit),note:profit>=0?"+":"-"},{l:"분담금 대비 수익률",v:fP(roi)},{l:"총투자 대비 수익률",v:fP(totalRoi)},{l:"손익분기점",v:profit>=0?"달성":fW(Math.abs(profit))+" 부족",note:"예상가 기준"}]}/>:<Empty icon="🏠"/>}</div>);}

/* 건물잔존가치 */
function CalcBldValue({isMo=false,onNav=()=>{}}){const[np,sNp]=useState("");const[ul,sUl]=useState("40");const[el,sEl]=useState("");const[mt,sMt]=useState("line");const npW=tW(np),ulV=parseInt(ul),elV=parseInt(el||"0");let remaining=0,depreciation=0;if(npW>0&&ulV>0){if(mt==="line"){const ratio=Math.max(0,Math.min(1,1-elV/ulV));remaining=Math.round(npW*ratio);depreciation=npW-remaining;}else{remaining=Math.round(npW*Math.pow(0.9,elV));depreciation=npW-remaining;}}const pct=npW>0?remaining/npW*100:0;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏚️ 건물 잔존가치</h3>}<Inp label="신축 가격 (재조달원가)" value={np} onChange={sNp} suffix="만원" placeholder="예: 50000"/><Sel label="내용연수" value={ul} onChange={sUl} options={[{value:"20",label:"20년 (경량철골)"},{value:"30",label:"30년 (조적·블록)"},{value:"40",label:"40년 (철근콘크리트)"},{value:"50",label:"50년 (SRC)"}]}/><Inp label="경과연수" value={el} onChange={sEl} placeholder="예: 15"/><Tog label="감가 방법" value={mt} onChange={sMt} options={[{value:"line",label:"정액법"},{value:"declining",label:"정률법 (10%)"}]}/></div>{npW>0&&elV>0?<RP title="건물 잔존가치"
      ...
      total={remaining} sub={"잔존률 "+fP(pct)} items={[{l:"신축 가격",v:fW(npW)},{l:"내용연수",v:ulV+"년"},{l:"경과연수",v:elV+"년"},{l:"감가방법",v:mt==="line"?"정액법":"정률법 (10%)"},{l:"정액법 기준 잔존률",v:fP(Math.max(0,Math.min(1,1-elV/ulV))*100)},{l:"정률법 기준 잔존률",v:fP(Math.pow(0.9,elV)*100)},{l:"감가상각 누계",v:fW(depreciation)},{l:"잔존가치",v:fW(remaining)},{l:"잔존률",v:fP(pct)}]}/>:<Empty icon="🏚️"/>}</div>);}
...
function CalcIncTax({isMo=false,onNav=()=>{}}){const[incType,sIT]=useState("salary");const[gross,sGross]=useState("");const[deductions,sDed]=useState("");const[bizType,sBT]=useState("simple");const[bizRate,sBR]=useState("60");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💼 종합소득세</h3>}
  ...
  </div><div>{grossW>0?<RP title="종합소득세"
...
function CalcYearEnd({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");const[monthlyRent,sMR]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🧾 연말정산</h3>}
  ...
  </div><div>{salW<=0&&<RequiredGuide items={[{label:"연봉 (세전)",filled:salW>0}]}/>}{salW>0?<RP title={refund>=0?"예상 환급액":"추가 납부 예상"}
...
function CalcNetSalary({isMo=false,onNav=()=>{}}){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💰 연봉 실수령액</h3>}
  ...
  </div>:<Empty icon="💰" msg="연봉을 입력하세요"/>}<NextStep calcId="netsalary" onNav={onNav} isMo={isMo}/><RateTable title="4대보험 요율 (2025)" headers={["보험","근로자"]} rows={[["국민연금","4.75%"],["건강보험","3.595%"],["장기요양","건보의 12.95%"],["고용보험","0.9%"],["합계(근로자)","약 9.7%"]]}/></div></div>);}
...
function CalcPension({isMo=false,onNav=()=>{}}){const[age,sAge]=useState("35");const[salary,sSal]=useState("350");const[paidYears,sPY]=useState("10");const[retireAge,sRA]=useState("65");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>👴 국민연금 예상수령액</h3>}
...
function CalcRetire({isMo=false,onNav=()=>{}}){const[monthly,sM]=useState("");const[years,sY]=useState("");const[months,sMo]=useState("0");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💼 퇴직금 계산기</h3>}
    ...
    <div style={{padding:"12px 16px",background:P.lt,borderRadius:10,fontSize:12,color:"#6b778c",lineHeight:1.6,marginTop:8}}>※ 퇴직금 = 1일 평균임금 × 30일 × (근속일수/365) / ※ 1년 이상 근무 시 수령 자격</div>
  </div>{monthlyW>0&&y>=1?<RP title="예상 퇴직금"
      ...
      total={severance} sub={y+"년 "+m+"개월 근속 기준"} items={[{l:"월 평균임금",v:fW(monthlyW),note:"최근 3개월 평균"},{l:"근속기간",v:y+"년 "+m+"개월 ("+totalDays+"일)"},{l:"1일 평균임금",v:fW(Math.round(monthlyW/30))},{l:"계산식",v:"월급×30일×근속일수÷365",note:"법정 공식"},{l:"퇴직금 (세전)",v:fW(severance)},{l:"퇴직소득세 (추정)",v:fW(Math.round(retireTax)),note:"간이 계산"},{l:"실수령 퇴직금 (추정)",v:fW(severance-Math.round(retireTax))}]}/>:<Empty icon="💼" msg="월급과 근속기간을 입력하세요"/>}</div>);}
...
function CalcUnemploy({isMo=false,onNav=()=>{}}){const[age,sAge]=useState("30");const[years,sY]=useState("3");const[daily,sD]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏢 실업급여 계산기</h3>}
    ...
    <div style={{background:P.card,borderRadius:10,border:"1px solid "+P.bd,padding:"12px 14px",marginTop:12,fontSize:12}}>
  ...
  </div>{dailyW<=0&&<RequiredGuide items={[{label:"퇴직 전 평균임금",filled:dailyW>0}]}/>}{dailyW>0?<RP title="실업급여 예상"
      ...
      alertType="info"
      total={monthlyAmount} sub={"월 수령액 기준 · 최대 "+duration+"일"} items={[{l:"1일 평균임금",v:fmt(dailyW)+"원"},{l:"1일 실업급여액 (60%)",v:fmt(calcDaily)+"원",note:"평균임금 × 60%"},{l:"상한액",v:fmt(maxDaily)+"원",note:"2026년 기준"},{l:"하한액 (최저임금80%)",v:fmt(minDaily)+"원"},{l:"수급 기간",v:duration+"일 (약 "+(duration/30).toFixed(1)+"개월)",note:ageV+"세 "+yV+"년 가입"},{l:"월 수령액 (30일)",v:fmt(monthlyAmount)+"원"},{l:"총 수령액",v:fmt(totalAmount)+"원"}]}/>:<Empty icon="🏢" msg="평균임금을 입력하세요"/>}</div>);}
...
function CalcCarTax({isMo=false,onNav=()=>{}}){const[carType,sCT]=useState("normal");const[cc,sCC]=useState("2000");const[year,sYear]=useState("0");const[isElec,sElec]=useState("no");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🚗 자동차세</h3>}
...
function CalcInsurance4({isMo=false,onNav=()=>{}}){const[type,sType]=useState("worker");const[salary,sSal]=useState("");const[bonus,sBonus]=useState("");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div>{!isMo&&<h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏥 4대보험료</h3>}
  ...
  </div>:<Empty icon="🏥" msg="월 급여를 입력하세요"/>}<RequiredGuide items={[{label:"월 급여 (세전)",filled:false}]}/></div>);}
