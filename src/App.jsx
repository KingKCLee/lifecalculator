...
function CalcCompre({isMo=false}){const[taxpayer,sTP]=useState("indi");const[h,sH]=useState("1");const[p,sP]=useState("");const[age,sAge]=useState("0");const[hold,sHold]=useState("0");const[prevTax,sPrev]=useState("");
  ...
  let ct=0,rateLabel="";
  const C1=[[3e8,.005],[6e8,.007],[12e8,.01],[25e8,.013],[50e8,.015],[94e8,.020],[Infinity,.027]];const CX=[[3e8,.012],[6e8,.016],[12e8,.022],[25e8,.036],[50e8,.050],[94e8,.056],[Infinity,.060]];
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    {isCorp&&<div style={{padding:"10px 14px",background:"#FFEBE6",border:"1px solid #FFBDAD",borderRadius:10,fontSize:12,color:"#DE350B",marginTop:8,lineHeight:1.5}}>법인: 2주택↓ 일반 누진세율 / 3주택↑ 중과 누진세율 (최대 6.0%) 적용. 공제·세부담상한 미적용.</div>}
...
function Tip({text}){const[show,sS]=useState(false);return(<span style={{position:"relative",display:"inline-block",marginLeft:4}}><span onClick={e=>{e.stopPropagation();sS(!show)}} style={{cursor:"pointer",color:"#888",border:"1px solid #ccc",borderRadius:"50%",width:16,height:16,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,verticalAlign:"middle"}}>?</span>{show&&<div onClick={e=>{e.stopPropagation();sS(false)}} style={{position:"absolute",bottom:"calc(100% + 8px)",left:"50%",transform:"translateX(-50%)",background:"#333",color:"#fff",padding:"10px 14px",borderRadius:8,fontSize:12,lineHeight:1.6,width:260,zIndex:999,boxShadow:"0 4px 12px rgba(0,0,0,0.15)",whiteSpace:"normal",fontWeight:400,textAlign:"left"}}>{text}<div style={{position:"absolute",bottom:-6,left:"50%",transform:"translateX(-50%)",width:0,height:0,borderLeft:"6px solid transparent",borderRight:"6px solid transparent",borderTop:"6px solid #333"}}/></div>}</span>);}
function TipModal({title,children}){const[show,setShow]=useState(false);return(<><span onClick={()=>setShow(true)} style={{cursor:"pointer",border:"1px solid #ccc",borderRadius:"50%",width:18,height:18,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#888",marginLeft:4,verticalAlign:"middle"}}>?</span>{show&&<div onClick={()=>setShow(false)} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",zIndex:10000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:12,padding:24,maxWidth:500,width:"100%",maxHeight:"80vh",overflowY:"auto",position:"relative",wordBreak:"keep-all"}}><button onClick={()=>setShow(false)} style={{position:"absolute",top:12,right:16,background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#666"}}>✕</button><h4 style={{fontWeight:700,marginBottom:16,fontSize:16,paddingRight:30}}>{title}</h4><div style={{fontSize:14,lineHeight:1.8,color:"#333"}}>{children}</div></div></div>}</>);}
