...
/* LTV·대출한도 */
function CalcLTV({isMo=false}){const[pv,sPv]=useState("");const[a,sA]=useState("non");const[h,sH]=useState("1");const[ex,sEx]=useState("");const[first,sFirst]=useState("no");const pvW=tW(pv),exW=tW(ex),n=parseInt(h);let ltv=0.70;if(first==="yes"){ltv=0.70;}else if(a==="spec"){ltv=n===1?0.40:0;}else if(a==="adj"){ltv=n===1?0.50:0.30;}else{ltv=n===1?0.70:0.60;}if(n>=3&&a!=="non")ltv=0;const isMetro=a!=="non";let metroCap=Infinity;if(isMetro){if(pvW<=15e8)metroCap=6e8;else if(pvW<=25e8)metroCap=4e8;else metroCap=2e8;}const ltvLoan=pvW*ltv;const capped=Math.min(ltvLoan,metroCap);const ml=Math.max(0,capped-exW);const isCappedByRegulation=ltv>0&&ltvLoan>metroCap;return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🔑 LTV·대출한도</h3><Inp label="주택 가격 (시세)" value={pv} onChange={sPv} suffix="만원" placeholder="예: 90000"/><Tog label="규제지역" value={a} onChange={sA} options={[{value:"non",label:"비규제"},{value:"adj",label:"조정대상"},{value:"spec",label:"투기과열"}]}/><Sel label="주택 수" value={h} onChange={sH} options={[{value:"1",label:"1주택 (무주택 포함)"},{value:"2",label:"2주택"},{value:"3",label:"3주택 이상"}]}/><Tog label="생애최초 구입자" value={first} onChange={sFirst} options={[{value:"no",label:"아니오"},{value:"yes",label:"예 (LTV 70%)"}]}/><Inp label="기존 담보대출 잔액" value={ex} onChange={sEx} suffix="만원" placeholder="없으면 0"/>{isCappedByRegulation&&<div style={{padding:"12px 16px",background:"#FFF8E1",border:"1px solid #FFE082",borderRadius:10,fontSize:13,color:"#F57F17",marginTop:12,lineHeight:1.6}}>⚠️ LTV 기준 {fW(ltvLoan)} 가능하나, 수도권·규제지역 주담대 한도({metroCap===6e8?"6억":metroCap===4e8?"4억":"2억"})로 제한됩니다. (2025.6.28~ 시행)</div>}</div><div>{pvW>0?<RP title={ltv===0?"대출 제한":"LTV·대출한도 분석"} total={ml} sub={ltv===0?(n>=3&&a!=="non"?"규제지역 3주택 이상 주담대 금지":"규제지역 다주택자 주택담보대출 제한"):"LTV "+fP(ltv*100)+(isCappedByRegulation?" + 주담대 한도":"")+" 적용"} items={[{l:"주택 가격",v:fW(pvW)},{l:"적용 LTV 한도",v:ltv===0?"대출 불가":fP(ltv*100)},{l:"LTV 담보인정 가치",v:fW(ltvLoan)}].concat(isMetro?[{l:"수도권 주담대 한도",v:metroCap===Infinity?"제한없음":fW(metroCap)}]:[]).concat([{l:"기존 대출 차감",v:exW>0?"-"+fW(exW):"없음"},{l:"최대 대출 가능액",v:fW(ml)}])}/>:<Empty icon="🔑"/>}<RateTable title="LTV 규제 비율" headers={["지역","일반","생애최초"]} rows={[["투기지역·투기과열","50%","80%"],["조정대상지역","60%","80%"],["비규제지역","70%","80%"]]}/></div></div>);}
...
/* 연말정산 */
function CalcYearEnd({isMo=false}){const[salary,sSal]=useState("");const[card,sCard]=useState("");const[medical,sMed]=useState("");const[edu,sEdu]=useState("");const[donate,sDon]=useState("");const[insurance,sIns]=useState("");const[pension,sPen]=useState("");
  const[spouse,sSpouse]=useState("0");const[parents,sParents]=useState("0");const[children,sChildren]=useState("0");const[senior,sSenior]=useState("0");const[disabled,sDisabled]=useState("0");
...
function CalcNetSalary({isMo=false}){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  </div>:<Empty icon="💰" msg="연봉을 입력하세요"/>}<RateTable title="4대보험 요율 (2025)" headers={["보험","근로자"]} rows={[["국민연금","4.5%"],["건강보험","3.545%"],["장기요양","건보의 12.95%"],["고용보험","0.9%"],["합계(근로자)","약 9.4%"]]}/></div></div>);}
