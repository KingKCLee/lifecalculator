...
function RP({title,total,sub,items}){
  ...
  return(<div style={{background:"linear-gradient(180deg,#0747A6 0%,#0052CC 100%)",borderRadius:20,padding:"28px 24px",color:"#fff",position:"sticky",top:80}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:isMo?4:8,marginTop:16}}>
...
function CalcAcq({isMo=false}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div>
      ...
      <div style={{marginBottom:16}}>
        <label style={{display:"block",fontSize:12,fontWeight:600,color:"#6b778c",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>부동산 유형<Tip text="주거용 오피스텔인 경우에만 '오피스텔' 선택. 업무용은 '그 외'. 농지는 2년 이상 자경 시 감면 혜택."/></label>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        ...
        {showFirstDist&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={firstDistribution} onChange={e=>setFirstDistribution(e.target.checked)} style={{width:18,height:18}}/> 임대사업자 최초분양<TipModal title="주택임대사업자 최초분양 취득세 면제·감면"><p>일정 규모 이하의 공동주택(아파트·다가구 제외), 오피스텔을 최초 분양받고 임대사업자 등록 시 취득세 면제 또는 감면</p><ul style={{paddingLeft:20}}><li><b>가격:</b> 수도권 6억, 그 외 3억 이하</li><li><b>규모:</b> 60제곱미터 이하</li><li><b>40㎡ 이하:</b> 취득세 면제</li><li><b>40~60㎡:</b> 취득세 50% 감면</li></ul></TipModal></label>}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<TipModal title="조정대상지역"><p>정부가 지정한 부동산 과열 지역입니다.</p><ul style={{paddingLeft:20}}><li>서울 전 지역 (25개 자치구)</li><li>경기 일부 (과천, 성남분당, 하남, 광명 등 12개)</li></ul><p>2주택자 취득세 8%, 3주택 이상 12% 중과가 적용됩니다.</p></TipModal></label>}
        {showMetro&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={metro} onChange={e=>{setMetro(e.target.checked);if(e.target.checked)setPopulationDecline(false)}} style={{width:18,height:18}}/> 수도권<Tip text="서울·인천·경기. 임대사업자 최초분양 감면 시 가격 한도(6억) 판단."/></label>}
        {showPopDecline&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={populationDecline} onChange={e=>{setPopulationDecline(e.target.checked);if(e.target.checked)setMetro(false)}} style={{width:18,height:18}}/> 인구감소지역<Tip text="생애최초 감면 한도 200→300만원 확대. 수도권과 동시 선택 불가."/></label>}
        {showHeavyExclude&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={heavyTaxExclude} onChange={e=>setHeavyTaxExclude(e.target.checked)} style={{width:18,height:18}}/> 중과 배제<TipModal title="주택 유상거래 취득세 중과 제외"><p>지방세법 시행령 제28조의2에 따른 주택은 취득세가 중과세됩니다. 제외 요건을 확인하고 직접 체크해주셔야 합니다.</p><ul style={{paddingLeft:20}}><li>가정어린이집</li><li>노인복지주택</li><li>재개발 부지를 위해 멸실용으로 취득하는 주택</li><li>주택시공자가 공사대금으로 받은 미분양주택</li><li>저당권 실행으로 취득한 주택</li><li>국가등록문화재 주택</li><li>농어촌 주택</li><li>공시가격 1억 원 이하 주택 (재개발 구역 등 제외)</li><li>공공주택사업자(지방공사, LH 등)의 공공임대주택</li><li>주택도시기금 리츠가 환매 조건부로 취득하는 주택</li><li>사원용 주택(60㎡ 이하)</li><li>주택건설사업자가 신축한 미분양된 주택</li></ul></TipModal></label>}
        {showSpouseChild&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={spouseChildGive} onChange={e=>setSpouseChildGive(e.target.checked)} style={{width:18,height:18}}/> 1주택자 배우자·직계비속 증여<Tip text="1세대 1주택자가 배우자·직계존비속에게 증여 시 조정지역 3억 초과여도 3.5% 일반세율."/></label>}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:14}}>
        {showFirstOfLife&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={firstOfLife} onChange={e=>setFirstOfLife(e.target.checked)} style={{width:18,height:18}}/> 생애최초 구입<TipModal title="생애최초 주택구입 취득세 감면"><p>12억원 이하 주택에 한해 적용됩니다. (지방세특례제한법 제36조의3, 2028년 12월 31일까지)</p><p>인구감소지역에서 생애최초 주택을 취득하면 감면 한도만 300만원으로 확대됩니다.</p><ul style={{paddingLeft:20}}><li><b>취득세 200만원 이하:</b> 전액 면제</li><li><b>취득세 200만원 초과:</b> 취득세 - 200만원</li><li><b>인구감소지역, 300만원 이하:</b> 전액 면제</li><li><b>인구감소지역, 300만원 초과:</b> 취득세 - 300만원</li></ul></TipModal></label>}
...
function CalcTrans({isMo=false}){
  ...
  return(<div><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<Tip text="양도 시점에 조정대상지역이면 다주택 중과 대상. 단, 2022.5.10~2026.5.9 양도분은 중과 유예."/></label>}
      {showRealLive&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={realLive} onChange={e=>setRealLive(e.target.checked)} style={{width:18,height:18}}/> 2년 이상 거주<Tip text="조정지역 주택은 2년 보유+2년 거주해야 1세대1주택 비과세. 비조정은 2년 보유만 필요."/></label>}
      {showRentBiz&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={rentBiz} onChange={e=>{setRentBiz(e.target.checked);if(!e.target.checked)setLongRentEx(false)}} style={{width:18,height:18}}/> 임대사업자<Tip text="등록요건·임대기간·임대료 조건 모두 만족한 경우 체크. 소득세법상 혜택 적용."/></label>}
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={unregistered} onChange={e=>setUnregistered(e.target.checked)} style={{width:18,height:18}}/> 미등기양도<Tip text="등기 없이 양도 시 70% 단일세율. 장특공 적용 불가."/></label>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={inherited} onChange={e=>setInherited(e.target.checked)} style={{width:18,height:18}}/> 상속받은 자산<Tip text="상속받은 부동산 양도 시 세율은 피상속인 취득일부터, 장특공은 상속개시일부터 기산."/></label>
      {showNonBizLand&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={nonBizLand} onChange={e=>setNonBizLand(e.target.checked)} style={{width:18,height:18}}/> 비사업용토지<Tip text="소유 기간 중 지목 본래 용도에 사용하지 않은 토지. 기본세율+10%p 추가과세."/></label>}
    </div>
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
    ...
    <div className="calc-inner" style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:12,marginBottom:12}}>
      <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>필요경비 <Tip text="취득세·등기비·중개수수료·인테리어비(자본적지출) 등. 대출이자·재산세는 필요경비 불인정."/></div>
...
const CX=[[3e8,.005],[6e8,.007],[12e8,.010],[25e8,.020],[50e8,.030],[94e8,.040],[Infinity,.050]];
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏘️ 종합부동산세</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>공시가격 <Tip text="매년 6월 1일 기준 공시가격 합계. 공정시장가액비율 60%를 곱한 금액이 과세표준."/></div>
...
// 전역 단일 Tip 열림 관리: 새 Tip 열면 기존 Tip 자동 닫힘
let tipCounter=0;let _openTipId=null;const _tipSubs=new Set();
function _setOpenTipId(id){_openTipId=id;_tipSubs.forEach(f=>f(id));}
function Tip({text}){
  const[myId]=useState(()=>++tipCounter);
  const[openTipId,setLocal]=useState(_openTipId);
  useEffect(()=>{_tipSubs.add(setLocal);return()=>{_tipSubs.delete(setLocal);};},[]);
  const isOpen=openTipId===myId;
...
function CalcNetSalary({isMo=false}){const[salary,sSal]=useState("");const[family,sFamily]=useState("1");const[child,sChild]=useState("0");const[nonTax,sNT]=useState("20");
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>💰 연봉 실수령액</h3>
    ...
    <div style={{fontSize:12,color:"#6b778c",marginBottom:-8,marginLeft:2,display:"flex",alignItems:"center",gap:4,flexWrap:"nowrap"}}>비과세 <Tip text="식대 월 20만원 등 비과세 항목. 비과세 금액은 세금 계산에서 제외됨. 근로계약서/급여명세서에서 확인."/></div>
