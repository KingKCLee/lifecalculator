  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"1fr 1fr",gap:isMo?16:32,alignItems:"start"}}>
    <div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        {showCorp&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={corporation} onChange={e=>setCorporation(e.target.checked)} style={{width:18,height:18}}/> 법인<TipModal title="법인 취득"><p>법인이 주택을 취득하는 경우 주택수와 관계없이 12% 적용됩니다.</p></TipModal></label>}
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
        ...
        {showMetro&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={metro} onChange={e=>{setMetro(e.target.checked);if(e.target.checked)setPopulationDecline(false)}} style={{width:18,height:18}}/> 수도권<TipModal title="수도권"><p>서울·인천·경기. 임대사업자 최초분양 감면 시 가격 한도(6억) 판단에 사용됩니다.</p></TipModal></label>}
        {showPopDecline&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={populationDecline} onChange={e=>{setPopulationDecline(e.target.checked);if(e.target.checked)setMetro(false)}} style={{width:18,height:18}}/> 인구감소지역<TipModal title="인구감소지역"><p>생애최초 감면 한도 200→300만원 확대. 수도권과 동시 선택 불가.</p></TipModal></label>}
        ...
        {showSpouseChild&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}><input type="checkbox" checked={spouseChildGive} onChange={e=>setSpouseChildGive(e.target.checked)} style={{width:18,height:18}}/> 1주택자 배우자·직계비속 증여<TipModal title="1주택자 배우자·직계비속 증여"><p>1세대 1주택자가 배우자·직계존비속에게 증여 시 조정지역 3억 초과여도 3.5% 일반세율 적용.</p></TipModal></label>}
      </div>
...
function CalcTrans({isMo=false}){
  ...
  return(<div><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>📊 양도소득세 시뮬레이션</h3>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      {showConArea&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={conArea} onChange={e=>setConArea(e.target.checked)} style={{width:18,height:18}}/> 조정대상지역<TipModal title="조정대상지역 (양도)"><p>양도 시점에 조정대상지역이면 다주택 중과 대상. 단, 2022.5.10~2026.5.9 양도분은 중과 유예.</p></TipModal></label>}
      {showRealLive&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={realLive} onChange={e=>setRealLive(e.target.checked)} style={{width:18,height:18}}/> 2년 이상 거주<TipModal title="2년 이상 거주"><p>조정지역 주택은 2년 보유+2년 거주해야 1세대1주택 비과세. 비조정은 2년 보유만 필요.</p></TipModal></label>}
      {showRentBiz&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={rentBiz} onChange={e=>{setRentBiz(e.target.checked);if(!e.target.checked)setLongRentEx(false)}} style={{width:18,height:18}}/> 임대사업자<TipModal title="임대사업자"><p>등록요건·임대기간·임대료 조건 모두 만족한 경우 체크. 소득세법상 혜택이 적용됩니다.</p></TipModal></label>}
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={unregistered} onChange={e=>setUnregistered(e.target.checked)} style={{width:18,height:18}}/> 미등기양도<TipModal title="미등기양도"><p>등기 없이 양도 시 70% 단일세율. 장기보유특별공제 적용 불가.</p></TipModal></label>
      <label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={inherited} onChange={e=>setInherited(e.target.checked)} style={{width:18,height:18}}/> 상속받은 자산<TipModal title="상속받은 자산"><p>상속받은 부동산 양도 시 세율은 피상속인 취득일부터, 장기보유특별공제는 상속개시일부터 기산합니다.</p></TipModal></label>
      {showNonBizLand&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:isMo?14:13,cursor:"pointer"}}><input type="checkbox" checked={nonBizLand} onChange={e=>setNonBizLand(e.target.checked)} style={{width:18,height:18}}/> 비사업용토지<TipModal title="비사업용토지"><p>소유 기간 중 지목 본래 용도에 사용하지 않은 토지. 기본세율 + 10%p 추가과세.</p></TipModal></label>}
    </div>
    <div style={{display:"flex",flexWrap:"wrap",gap:isMo?10:16,marginBottom:12}}>
