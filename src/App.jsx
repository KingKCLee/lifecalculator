          ...
          <div style={{position:"absolute",left:"50%",top:0,bottom:0,transform:"translateX(-50%)",display:"flex",alignItems:"center",gap:4}}>
            {CATS.map(c=>{const items=CL.filter(cl=>cl.c===c.id);const active=cat===c.id&&page!=="home";const hot=hoverCat===c.id;const clicked=clickedCat===c.id;return(
              <div key={c.id} onMouseEnter={()=>setHoverCat(c.id)} onMouseLeave={()=>{setHoverCat(null);setClickedCat(prev=>prev===c.id?null:prev);}} style={{position:"relative"}}>
                <button type="button" onMouseEnter={()=>{setHoverCat(c.id);setClickedCat(c.id);}} onClick={(e)=>{e.preventDefault();e.stopPropagation();const next=hoverCat===c.id?null:c.id;setHoverCat(next);setClickedCat(next);}} style={{padding:"0 16px",border:"none",borderRadius:0,background:hot?"#ffffff":"transparent",color:active?"#0747A6":"#6B7280",fontSize:16,fontWeight:hot?500:(active?700:600),cursor:"pointer",fontFamily:"inherit",borderBottom:hot?"none":(active?"3px solid #0747A6":"3px solid transparent"),transition:"transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1), background .15s, color .15s",height:64,display:"flex",alignItems:"center",gap:6,transform:hot?"scale(1.1)":"scale(1)",transformOrigin:"center center",willChange:"transform"}}><span style={{display:"inline-flex"}}>{TAB_ICONS[c.id]}</span>{c.l}</button>
            ...
            <div onMouseEnter={megaOpen} onMouseLeave={megaClose} style={{position:"relative"}}>
              <button type="button" onMouseEnter={()=>{setShowInfoMenu(true);setClickedCat("info");}} onClick={(e)=>{e.preventDefault();e.stopPropagation();const next=!showInfoMenu;setShowInfoMenu(next);setClickedCat(next?"info":null);}} style={{padding:"0 16px",border:"none",borderRadius:0,background:showInfoMenu?"#ffffff":"transparent",color:page==="info"?"#0747A6":"#6B7280",fontSize:16,fontWeight:showInfoMenu?500:(page==="info"?700:600),cursor:"pointer",fontFamily:"inherit",borderBottom:showInfoMenu?"none":(page==="info"?"3px solid #0747A6":"3px solid transparent"),transition:"transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1), background .15s, color .15s",height:64,display:"flex",alignItems:"center",gap:6,transform:showInfoMenu?"scale(1.1)":"scale(1)",transformOrigin:"center center",willChange:"transform"}}><span style={{display:"inline-flex"}}>{TAB_ICONS.info}</span>정보센터</button>
