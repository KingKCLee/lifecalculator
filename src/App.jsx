...
function CalcBond({isMo=false,onNav=()=>{}}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:isMo?"1fr":"3fr 2fr",gap:isMo?16:28,alignItems:"start",minWidth:0}}><div><Inp label="매매 가격" value={p} onChange={sP} suffix="만원" placeholder="예: 90000"/><Tog label="지역" value={area} onChange={sArea} options={[{value:"seoul",label:"서울"},{value:"metro",label:"광역시"},{value:"etc",label:"기타 지역"}]}/><Tog label="용도" value={use} onChange={sUse} options={[{value:"house",label:"주택"},{value:"commercial",label:"상가·오피스텔"}]}/><Inp label="할인율 (실시세)" value={customRate} onChange={setCustomRate} suffix="%" note="증권사 당일 매도가. 일반 4~5%"/></div><RP miss={tW(p)>0?null:MI.bond} expertGuide={CALC_NAV_CONTENT.bond?.items} expertGuide={CALC_NAV_CONTENT.bond?.items} title="채권 할인비용"
