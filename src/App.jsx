...
function CalcComm(){const[ty,sTy]=useState("res");const[tx,sTx]=useState("sale");const[p,sP]=useState("90000");const[dep,sDep]=useState("5000");const[mon,sMon]=useState("50");
  let pW;if(ty==="res"&&tx==="rent"){const depW=tW(dep),monW=tW(mon);pW=Math.max(depW,depW+monW*100);}else{pW=tW(p);}
  ...
  return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignItems:"start"}}><div><h3 style={{fontSize:18,fontWeight:700,color:P.tx,margin:"0 0 20px"}}>🏠 공인중개사 중개보수</h3><Tog label="부동산 유형" value={ty} onChange={sTy} options={[{value:"res",label:"주택"},{value:"com",label:"상가·토지·오피스텔"}]}/>{ty==="res"&&<Tog label="거래 유형" value={tx} onChange={sTx} options={[{value:"sale",label:"매매"},{value:"lease",label:"전세"},{value:"rent",label:"월세"}]}/>}{ty==="res"&&tx==="rent"?(<><Inp label="보증금" value={dep} onChange={sDep} suffix="만원" placeholder="예: 5000"/><Inp label="월세" value={mon} onChange={sMon} suffix="만원" placeholder="예: 50"/><div style={{padding:"10px 14px",background:P.lt,borderRadius:8,fontSize:12,color:"#6b778c",lineHeight:1.6,marginBottom:16}}>거래금액 = 보증금 + (월세 × 100) = {fW(pW)}</div></>):(<Slider label="거래금액" value={p} onChange={sP} min={1000} max={500000} step={500}/>)}</div><RP title="중개보수 (수수료)" total={cm+vt} sub={"상한요율 "+fP(r*100)+" 적용"} items={[{l:"거래금액 기준",v:fW(pW)},{l:"중개보수 (VAT 별도)",v:fW(cm)},{l:"부가가치세 (10%)",v:fW(vt)},{l:"합계 (중개보수+VAT)",v:fW(cm+vt)}]}/></div>);}
