...
export default function AdminDashboard({ session, onLogout }) {
  ...
  const cardSt = { background: "#fff", borderRadius: 14, padding: "22px 24px", border: "1px solid #dfe1e6", flex: "1 1 200px", minWidth: 0 };
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Pretendard',-apple-system,sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #E5E7EB", padding: "14px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: "#0a1628" }}>생활계산기.com</div>
          <div style={{ fontSize: 12, color: "#6b778c" }}>{dateStr} {timeStr}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {session && session.picture && <img src={session.picture} alt="" style={{ width: 28, height: 28, borderRadius: "50%" }} referrerPolicy="no-referrer" />}
          {session && <span style={{ fontSize: 13, color: "#374151", fontWeight: 600 }}>{session.name || session.email}</span>}
          <button onClick={loadStats} style={{ padding: "7px 12px", background: "#f4f5f7", color: "#505f79", border: "1px solid #dfe1e6", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>새로고침</button>
          <button onClick={doLogout} style={{ padding: "7px 12px", background: "#fff", color: "#94a3b8", border: "1px solid #E5E7EB", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>로그아웃</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {stats && <>
          ...
          {/* Top Calcs */}
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", border: "1px solid #dfe1e6", marginBottom: 28 }}>
                ...
                <div key={i} style={{ display: "grid", gridTemplateColumns: "32px 1fr 80px 50px", gap: 12, alignItems: "center", padding: "8px 0", borderBottom: i < stats.topCalcs.length - 1 ? "1px solid #f4f5f7" : "none" }}>
                  ...
                  <div>
                    ...
                    <div style={{ height: 6, background: "#f1f5f9", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: pct + "%", background: i < 3 ? "#0141f9" : "#94a3b8", borderRadius: 3 }} />
                    </div>
                  </div>
            ...
            {stats.dailyVisits && stats.dailyVisits.length > 0 ? (
              <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 160, overflowX: "auto" }}>
                {stats.dailyVisits.map((d, i) => {
                  const h = maxDaily > 0 ? (d.cnt / maxDaily) * 130 : 0;
                  const dayLabel = d.dt ? d.dt.slice(5) : "";
                  return (
                    <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: "1 0 22px", minWidth: 22 }} title={d.dt + ": " + d.cnt + "건"}>
                      <div style={{ fontSize: 9, color: "#6b778c", marginBottom: 2 }}>{d.cnt}</div>
                      <div style={{ width: "70%", height: Math.max(h, 2), background: "#0141f9", borderRadius: "3px 3px 0 0" }} />
                      <div style={{ fontSize: 8, color: "#94a3b8", marginTop: 4, whiteSpace: "nowrap" }}>{i % 3 === 0 ? dayLabel : ""}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 28 }}>
            <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", border: "1px solid #dfe1e6" }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#0a1628", marginBottom: 12 }}>GA4 Analytics</div>
              <a href="https://analytics.google.com/" target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "14px 18px", background: "#f8f9fc", borderRadius: 10, textAlign: "center", color: "#0141f9", fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1px solid #E5E7EB" }}>
                GA4 대시보드 (G-58SYV64E1C)
              </a>
            </div>
            <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", border: "1px solid #dfe1e6" }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#0a1628", marginBottom: 12 }}>AdSense</div>
              <a href="https://www.google.com/adsense/" target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "14px 18px", background: "#f8f9fc", borderRadius: 10, textAlign: "center", color: "#0141f9", fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1px solid #E5E7EB" }}>
                AdSense 대시보드 (ca-pub-8857637137858335)
              </a>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", border: "1px solid #dfe1e6" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {[
                ...
                { l: "Search Console", u: "https://search.google.com/search-console" },
                { l: "PageSpeed", u: "https://pagespeed.web.dev/" },
              ].map((lk, i) => (
