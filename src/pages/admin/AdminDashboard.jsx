...
function DashContent({ stats, cardSt, secSt }) {
  return (<>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
      <div style={secSt}><div style={{ fontSize: 14, fontWeight: 800, color: "#0a1628", marginBottom: 16 }}>{"\uBAA8\uBC14\uC77C vs PC"}</div><DonutChart data={[{ label: "\uBAA8\uBC14\uC77C", value: stats.mobileCount || 65, color: "#0141f9" }, { label: "PC", value: stats.pcCount || 35, color: "#93c5fd" }]} /></div>
      <div style={secSt}><div style={{ fontSize: 14, fontWeight: 800, color: "#0a1628", marginBottom: 16 }}>{"\uC720\uC785 \uACBD\uB85C"}</div><DonutChart data={[{ label: "\uB124\uC774\uBC84", value: stats.refNaver || 40, color: "#00c73c" }, { label: "\uAD6C\uAE00", value: stats.refGoogle || 30, color: "#4285f4" }, { label: "\uC9C1\uC811", value: stats.refDirect || 20, color: "#6b778c" }, { label: "\uAE30\uD0C0", value: stats.refOther || 10, color: "#d1d5db" }]} /></div>
    </div>
    <div style={secSt}>
      <div style={{ fontSize: 14, fontWeight: 800, color: "#0a1628", marginBottom: 16 }}>{"\uC2DC\uAC04\uB300\uBCC4 \uBC29\uBB38 \uBD84\uD3EC"}</div>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 100 }}>
        {(stats.hourlyVisits || Array.from({ length: 24 }, (_, h) => ({ hour: h, cnt: 0 }))).map((d, i) => { const maxH = Math.max(...(stats.hourlyVisits || [{ cnt: 1 }]).map(x => x.cnt), 1); const hPx = maxH > 0 ? (d.cnt / maxH) * 80 : 0; return (
    ...
    <div style={secSt}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        {[{ l: "Cloudflare", u: "https://dash.cloudflare.com/" }, { l: "GitHub", u: "https://github.com/KingKCLee/lifecalculator" }, { l: "Supabase", u: "https://supabase.com/dashboard" }, { l: "Search Console", u: "https://search.google.com/search-console" }, { l: "GA4", u: "https://analytics.google.com/" }, { l: "AdSense", u: "https://www.google.com/adsense/" }].map((lk, i) => (
...
function AdsTab({ authFetch }) {
  ...
  useEffect(() => { authFetch("/api/admin/adsense").then(r => r.json()).then(j => { if (j.ok && j.config) setCfg(j.config); }).catch(() => {}); }, []);
  const toggle = (id) => setCfg(c => ({ ...c, [id]: { ...(c[id] || {}), enabled: !(c[id]?.enabled) } }));
  const setCode = (id, code) => setCfg(c => ({ ...c, [id]: { ...(c[id] || {}), code } }));
  const save = async () => { setSaving(true); setMsg(null); try { const r = await authFetch("/api/admin/adsense", { method: "POST", body: JSON.stringify({ config: cfg }) }); const j = await r.json().catch(() => ({})); setMsg(j.ok ? "\uC800\uC7A5 \uC644\uB8CC" : (j.error || "\uC800\uC7A5 \uC2E4\uD328")); } catch { setMsg("\uB124\uD2B8\uC6CC\uD06C \uC624\uB958"); } finally { setSaving(false); } };
...
function NoticeTab({ authFetch }) {
  ...
  useEffect(() => { authFetch("/api/admin/notice").then(r => r.json()).then(j => { if (j.ok) { setOn(!!j.enabled); setMessage(j.message || ""); setType(j.type || "info"); } }).catch(() => {}); }, []);
...
function ConsultTab() {
  const LC = "https://lc-auth-worker.noble-kclee.workers.dev";
  const [list, setList] = useState([]); const [loading, setLoading] = useState(true);
  const load = async () => { setLoading(true); try { const r = await fetch(LC + "/consult/stats", { headers: { "X-Admin-Key": localStorage.getItem("lc_admin_api_key") || "" } }); const j = await r.json().catch(() => ({})); setList(j.daily_trend || []); } catch {} finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  return (<div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", border: "1px solid #E5E7EB" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><div style={{ fontSize: 14, fontWeight: 700 }}>{"\uC0C1\uB2F4 \uC2E0\uCCAD \uD604\uD669"}</div><button onClick={load} style={{ padding: "6px 14px", background: "#f1f5f9", border: "1px solid #E5E7EB", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{"\uC0C8\uB85C\uACE0\uCE68"}</button></div>
    {loading ? <div style={{ color: "#6b778c", padding: 20 }}>{"\uB85C\uB4DC \uC911..."}</div> : (<table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "2px solid #E5E7EB" }}><th style={{ padding: "10px 12px", textAlign: "left", color: "#6b778c", fontWeight: 600 }}>{"\uB0A0\uC9DC"}</th><th style={{ padding: "10px 12px", textAlign: "right", color: "#6b778c", fontWeight: 600 }}>{"\uC2E0\uCCAD \uC218"}</th></tr></thead><tbody>{list.length === 0 ? <tr><td colSpan={2} style={{ padding: 20, textAlign: "center", color: "#9ca3af" }}>{"\uB370\uC774\uD130 \uC5C6\uC74C"}</td></tr> : list.map((d, i) => (<tr key={i} style={{ borderBottom: "1px solid #f4f5f7" }}><td style={{ padding: "10px 12px" }}>{d.day}</td><td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 700 }}>{fNum(d.count)}</td></tr>))}</tbody></table>)}</div>);
}
